...
*
*/
//=======Ditambah satu group lagi, group 3========modif by mustadio
(function($) {
    ...
    $.configureBoxes = function(options) {
        ...
        if (IsMoveMode(settings[index])) {
			...
			$('#' + group3[index].view).dblclick(function() {
                MoveSelected(group3[index], group1[index]);
            });
			
			//===add by mustadio
            $('#' + settings[index].to12).click(function() {
                MoveSelected(group2[index], group1[index]);
            });
			$('#' + settings[index].to13).click(function() {
                MoveSelected(group3[index], group1[index]);
            });
			
			//===add by mustadio
            $('#' + settings[index].allTo12).click(function() {
                MoveAll(group2[index], group1[index]);
            });
			$('#' + settings[index].allTo13).click(function() {
                MoveAll(group3[index], group1[index]);
            });
        } else {
            $('#' + group2[index].view).dblclick(function() {
			...
			$('#' + group3[index].view).dblclick(function() {
                RemoveSelected(group3[index], group1[index]);
            });
			...
			$('#' + settings[index].allTo13).click(function() {
                RemoveAll(group3[index], group1[index]);
            });
        }
		...
		$('#' + group1[index].view).dblclick(function() {
            MoveSelected(group1[index], group3[index]);
        });
		...
		alert(group2.length);
			...
			{
				alert("Dasar Bundle hanya 1 container");
			}
		...
		*/
		...
		$('#' + settings[index].allTo3).click(function() {
            MoveAll(group1[index], group3[index]);
        });
        ...
        $('#' + group1[index].storage + ',#' + group3[index].storage).css('display', 'none');
		$('#' + group1[index].storage + ',#' + group3[index].storage).css('display', 'none');
		...
		if (settings[index].selectOnSubmit) {
            $('#' + settings[index].box3View).closest('form').submit(function() {
                $('#' + settings[index].box3View).children('option').attr('selected', 'selected');
            });
        }
		
		function tes()
		{
		//var tes = document.forms["frm1"]["box2View"].value;
		//alert(tes);
		alert(group2.length);
		}
    ...
    function UpdateLabel(group) {
		...
		//=====hanya group 1 yg ditampilkan info===modif by mustadio
		//$("#" + group1[index]).text('Showing ' + showingCount + ' of ' + (showingCount + hiddenCount));
    }
