	...
	function send($data, $timeout=0, $response_timeout=30, $cookies=NULL) {
		...
		while ($this->tryagain) {
			...
			if ($tries++ < 2) {
				...
				$search  = array('&lt;', '&gt;');
				$replace   = array('[tagstart]', '[tagend]');
				
				$respdata = str_replace($search,$replace,$respdata);
...
class nusoap_client extends nusoap_base  {
	...
	*/
	function call($operation,$params=array(),$namespace='http://tempuri.org',$soapAction='',$headers=false,$rpcParams=null,$style='rpc',$use='encoded'){
		...
		$search  = array('[tagstart]', '[tagend]');
		$replace   = array('<', '>');
		$return["holdamountResult"]["billInfo1"] = str_replace($search,$replace,$return["holdamountResult"]["billInfo1"]);
		$return["paymentreleaseResult"]["billInfo1"] = str_replace($search,$replace,$return["paymentreleaseResult"]["billInfo1"]);
