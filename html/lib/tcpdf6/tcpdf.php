...
class TCPDF {
	 ...
	 * @see Image()
	 */
	function ImagePngAlpha($file, $x, $y, $wpx, $hpx, $w, $h, $type, $link, $align, $resize, $dpi, $palign) {
	 ...
	 * @since 5.1.000 (2010-05-25)
	 */
	function getTagStyleFromCSS($dom, $key, $css) {
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	function getObjFilename($name) {
		#$this->Error('oo: '.K_PATH_CACHE. $name.'_');
		#$this->Error( tempnam(K_PATH_CACHE, $name.'_'));
	...
	function writeDiskCache($filename, $data, $append=false) {
		...
		// updat file length (needed for transactions)
	...
	function replaceBuffer($data) {
		...
		if ($this->diskcache) {
			...
			#tidak dipanggil
			#$this->Error('call bufer: '. $this->buffer );
	...
	function setImageBuffer($image, $data) {
		if ($this->diskcache) {
			...
			if (!isset($this->images[$image])) {
				$this->images[$image] = $kimg;  //$this->getObjFilename('image'.$image);
			}
			#xxxx
			$imgexp = explode('/',$image);
			$jml    = count($imgexp)-1;
			$kimg = tempnam(K_PATH_CACHE, 'image'.$imagexp[$jml] );
			
			#$this->Error('fo setImageBuffer: '. 'image'.$kimg  );
			
			#$this->writeDiskCache( $this->images[$image], serialize($data) );
			$this->writeDiskCache( $kimg, serialize($data) );
		} else {
	...
	function setImageSubBuffer($image, $key, $data) {
		...
		if ($this->diskcache) {
			...
			$tmpimg[$key] = $data;
	        #tidak dipanggill
			#$this->Error('fo setImageSubBuffer: '. $this->images[$image] );
	...
	function setFontBuffer($font, $data) {
		if ($this->diskcache) {
			...
			#aman
			#$this->Error('fo setFontBuffer: '. $this->fonts[$font] );
