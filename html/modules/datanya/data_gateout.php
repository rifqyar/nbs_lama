...
if(isset($q)) {
		...
		$query = "SELECT * FROM 
				(
				    SELECT A.NO_UKK, 
				            ...
				            B.VOYAGE_IN,
				            B.VOYAGE_OUT, 
				            TO_CHAR(A.TGL_GATE_OUT,'DD-MM-YYYY HH24:MI') TGL_GATE_OUT,
				            TO_CHAR(A.TGL_GATE_OUT,'YYYYMMDDHH24MI') SEQ,
							CASE
	...
	while ($row = $res->fetchRow()) {
		...
		if($q == 'gateout') 
		{
			...
			$responce->rows[$i]['cell']=array($row[NO_UKK],$row[NO_CONTAINER],$sts,$voy,$row[TGL_GATE_OUT],$row[YD_POSISI]);
