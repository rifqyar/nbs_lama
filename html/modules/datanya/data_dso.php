...
if(isset($q)) {
	...
	{		
        ...
        else if($q=='v_baplie_export') //ambil data header
	{		
		$query="SELECT 
						*                                								
                FROM 
	...
	while ($row = $res->fetchRow()) {
		...
		{
			$id_vsb_voyage = $row[ID_VSB_VOYAGE];
			$vessel = urlencode($row[VESSEL]);
			$call_sign_e = $row[CALL_SIGN];
			$voyage_out_export = $row[VOYAGE_OUT];
			$voyage_in_export = $row[VOYAGE_IN];
			
			$detail = "<a href=planning.baplie_export.ajax/download?call_sign=$call_sign_e&voyage_out=$voyage_out_export&voyage_in=$voyage_in_export>Excel</a>";
			
			//Generate File Baplie
			$create = "<button onclick='create_baplie(\"$call_sign_e\",\"$voyage_in_export\",\"$voyage_out_export\");' title='create baplie' ><img src='images/page_attach.png' width=15px height=15px border='0'></button>";
			
			$edi = "<a href=planning.baplie_export.ajax/edi?call_sign=$call_sign_e&voyage_out=$voyage_out_export&voyage_in=$voyage_in_export&id_vsb_voyage=$id_vsb_voyage target=_blank>Gen EDI</a>";			
					
			$filename = $vessel."_".$voyage_in_export;					            
                                        
			$responce->rows[$i]['id']=$row[VOYAGE_IN];
			$responce->rows[$i]['cell']=array($create,$detail,$edi,$row[VESSEL],$row[VOYAGE_IN],$row[VOYAGE_OUT],$row[CALL_SIGN], $row[ATD]);
		}
