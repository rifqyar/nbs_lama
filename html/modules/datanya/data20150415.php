...
if(isset($q)) {
	...
	*/
		...
		$query="SELECT A.ID_REQ,C.NO_NOTA,A.EMKL,A.VESSEL,A.VOYAGE||' - '||A. VOYAGE_OUT AS VOYAGE,C.STATUS, (SELECT COUNT(1) FROM REQ_BATALMUAT_D WHERE ID_REQ = A.ID_REQ) JUMLAH_CONT
		...
		$query="select * from (SELECT b.ID_PROFORMA, a.ID_REQ, a.KODE_PBM, a.VESSEL ,a.VOYAGE_IN, a.VOYAGE_OUT, (SELECT count(c.NO_CONTAINER) from req_receiving_d c WHERE c.ID_REQ= a.ID_REQ) JML_CONT,b.STATUS
   ...
   WHERE NAMA_YARD LIKE 'LAPANGAN 300-305' AND B.TIER != 0
	...
	while ($row = $res->fetchRow()) {
		...
		else if($q == 'l_delivery_p') 
		{
			...
			$responce->rows[$i]['id']=$row[ID_REQ];
			$responce->rows[$i]['cell']=array($aksi1,$aksi2,$status_empty,$aksi_empty, $row[ID_REQ],$row[OLD_REQ],$row[JUMLAH_CONT],$row[EMKL],$vesv, $row[SP2P_KE], $row[TIPE_CONT_REQ]);
		}
		...
		else if($q == 'nota_receiving') 
		{
			if($row[STATUS]=="S")
			{
				$aksi   = "<a href=".HOME."billing.receiving.print/print_nota_lunas?no_req=".$row[ID_REQ]." ><img src='images/printer.png' title='print_nota'></a> <a href='#' onclick='return recalc(\"$row[ID_PROFORMA]\",\"$row[ID_REQ]\")' title='recalculate'><img src='images/money2.png' ></a>";
			...
			$responce->rows[$i]['cell']=array($aksi,$status,$row[ID_PROFORMA],$row[ID_REQ],$row[KODE_PBM],$row[VESSEL].' - '.$row[VOYAGE_IN].'-'.$row[VOYAGE_OUT],$row[JML_CONT]);
		}
		else if($q == 'nota_receiving_tn') 
		{
			if($row[STATUS]=="S")
	...
	else if($q == 'nota_hicoscan') 
	{
		...
		else if($q == 'nota_batalmuatd') 
		{
			...
			if($row[STATUS_PENUMPUKAN]=="S")
			{
				$aksi_penumpukan   = "<a href=".HOME."billing.batalmuat.delivery.print/print_nota_lunas_penumpukan?no_req=".$row[ID_REQ]." ><img src='images/printer.png' title='print_nota'> <a href='#' onclick='return recalc(\"$row[ID_NOTA_PENUMPUKAN]\",\"$row[ID_REQ]\")' title='recalculate'><img src='images/money2.png' ></a></a>";
