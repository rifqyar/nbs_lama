...
";
	...
	while ($row = $res->fetchRow()) {
		...
		if($q == 'nota_bm') 
		{
			...
			if(($_SESSION["ID_GROUP"]=='A') OR ($_SESSION["NAMA_PENGGUNA"] =='276107167') OR ($_SESSION["ID_GROUP"]=='F'))
			{
			...
			if ($row[STS_RPSTV]=='I') {
				...
				$rp="<a title='print' onclick='cetak_nota(\"$row[ID_RPSTV]\")'><img src='images/printer.png' ></a><br><b><font color=#0378c6>".$row[TRX_NMB]."</FONT></b><br/> <i style='color:green'>Transfer Succeed</i>";
			}
            else if ($row[STS_RPSTV]=='G') {
				$rp="<a title='print' onclick='cetak_nota(\"$row[ID_RPSTV]\")'><img src='images/printer.png' ></a>    $jk<br><b><font color=#0378c6>".$row[TRX_NMB]."</FONT></b><br/> <i style='color:red'>Transfer Failed</i>";
			}
		...
		{
			$act_transfer = '<a onclick="transfer(\''.$row[NO_NOTA].'\')" style="cursor:pointer" title="Transfer Ulang"><img src="images/trflfg2.png" ></a>';
			$responce->rows[$i]['id']=$i;
			$responce->rows[$i]['cell']=array($act_transfer,$row[NO_NOTA],$row[DESCRIPTION],$row[NO_REQUEST],$row[CUST_NAME],$row[DATE_CREATED],$row[KREDIT],$row[DATE_PAID],$row[STATUS_AR].' '.$row[STATUS_ARMSG], $row[ARPROCESS_DATE], $row[STATUS_ARMSG], $row[STATUS_RECEIPT].' '.$row[STATUS_RECEIPTMSG],$row[RECEIPTPROCESS_DATE]);
		}
		$i++;
	}
