...
if(isset($q)) {
	...
	if($q=='p_timesheet') {		
		...
		$query ="SELECT COUNT(*) AS NUMBER_OF_ROWS FROM (SELECT * FROM REQ_BATALMUAT_H )";
	}
	else if($q=='nota_bm') {		
		$query ="SELECT COUNT(*) AS NUMBER_OF_ROWS FROM (SELECT * FROM REQ_BATALMUAT_H )";
	}
	else if($q=='print_bm_del') {		
	...
	else if($q=='l_batalmuat') //ambil data header
	{
		$query="SELECT A.ID_BATALMUAT,
					   ...
					   A.STAT_GATE,
					   A.STATUS
					   FROM REQ_BATALMUAT_H A, MASTER_PBM B 
					   WHERE A.KODE_PBM=B.KODE_PBM
					   ORDER BY A.ID_BATALMUAT DESC";
	}
	else if($q=='print_bm_del') //ambil data header
	{
		$query="SELECT A.ID_REQ ID_BATALMUAT,
                       B.NAMA_PERUSAHAAN NAMA,                       
                       A.VESSEL,
                       A.VOYAGE_IN VOYAGE,
                       A.JENIS,
                       '' STAT_GATE,
                       ...
                       FROM REQ_BATALMUAT_H A LEFT JOIN MST_PELANGGAN B ON (A.KODE_PBM=B.KD_PELANGGAN)
                       WHERE A.JENIS='D'
                       ORDER BY A.ID_REQ DESC";
		$row8 = $db->query($query)->getAll();
		$c=0;
		foreach($row8 as $b)
		{
			$id_req[$c] = $b['ID_BATALMUAT'];	 
			$row5[$c] = $db->query("SELECT COUNT(NO_CONTAINER) AS JML_CONT FROM REQ_BATALMUAT_D WHERE ID_REQ = '$id_req[$c]' GROUP BY ID_REQ")->getAll();
	...
	while ($row = $res->fetchRow()) {
		...
		else if($q == 'print_bm_del') 
		{
			...
			if($row[STATUS]=="P")
				{
					$aksi3 = "<a href=".HOME."print.batalmuat_delivery.cetak/?pl=".$row[ID_BATALMUAT]." target='blank'><img src='images/printer.png' title='print_nota'></a>"; 
				}
				else
				{
