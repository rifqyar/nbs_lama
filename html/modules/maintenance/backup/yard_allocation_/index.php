...
-webkit-border-radius: 10px 10px 10px 10px; padding: 10px">
		...
		<table style="font-size: 12px; font-weight: bold;">
                    <tr>
                            <td>Nama Vessel</td><td> : </td><td><input type="text" size="15" id="vessel" name="vessel">
                                &nbsp<input type="text" size="5" id="voyage" name="voyage">
                                <input type="hidden" id="id_vessel" name="id_vessel">
                                 <input type="hidden" id="id_test" name="id_test" value="<?=$id_test;?>">
				</td>
			</tr>
			<tr>
				<td>Ukuran Container</td><td> : </td><td>
			...
			<tr>
				<td colspan="3">Pilih kategori container yang akan dialokasi : <br>
...
//	
	...
	$("#save").click(function(event) {
		...
		for (var j = 0; j < count_block; j++)
		{
			block_str += "<block><size>"+size[j]+"</size><type>"+type[j]+"</type><cell>"+index_block[j].join(",")+"</cell></block>";
		}
	...
	$("#db_link").click(function(event) {
		...
		for (var j = 0; j < count_block; j++)
		{
			block_str += "<block><id_block>"+id_block+"</id_block><kategori>"+kategori+"</kategori><vessel>"+vessel+"</vessel><size>"+size+"</size><type>"+type+"</type><cell>"+index_block[j].join(",")+"</cell></block>";
		}
