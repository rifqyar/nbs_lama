...
function js_created()
{
	var url	= "<?=HOME?>operational.gate_in_delivery_admin.auto/save";
	
	var n_p=$('#no_pol').val();
	var remarkc=$('#remark').val();
    var tgl_gatein=$('#tgl_gate_in').val();
	var gi_jam=$('#gi_jam').val();
	var gi_menit=$('#gi_menit').val();
	...
	//alert(brt);
		...
		$.post(url,{NO_CONT: my_cont, REQ_DEV: my_no_req, NO_POL:n_p,REMARK:remarkc,NO_UKK:my_ukk,TGL_GATE_IN:tgl_gatein,JAM_GATE_IN:gi_jam,MENIT_GATE_IN:gi_menit},function(data) 
		{
			console.log(data);
			/*if (data=='OK')
			{
			...
			*/
			...
			alert(data);
			window.location = "<?=HOME?>operational.gate_in_delivery_admin/";
...
<div class="content" id="finds">
	<p>
	<h2> <img src="<?=HOME?>images/truckyellow1.png" height="5%" width="5%" style="vertical-align:middle">&nbsp;<font color="#0378C6">Delivery</font> Gate-In Administrator</h2></p>
	...
	<table>
		...
		<TR>
			<TD colspan="3" align="right"><b><i><font color="red">click syncrhonize</font> - if container unavailable to gate in</i></b></TD>
		</TR>
	</table>
	...
	<tr>
	 ...
	 <TR>
		...
		<td class="form-field-caption" align="right">:</td>
		<td><input type="text" size="8" name="tgl_gate_in" id="tgl_gate_in" />&nbsp;&nbsp;&nbsp;<input type="text" name="gi_jam" id="gi_jam" size="2" maxlength="2"/>&nbsp;:&nbsp;<input type="text" name="gi_menit" id="gi_menit" size="2" maxlength="2"/>
		</td>
	 </TR>
	 <tr>
