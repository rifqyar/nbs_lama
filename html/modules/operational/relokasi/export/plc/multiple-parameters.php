		...
		foreach ($arr as $p)
		{
			...
			if(($size_=='40')||($size_=='45'))
					{			
												...
												foreach ($jml_hsl as $seq)
												{
													...
													if($n==1)
													{
														...
														$relokasi = "begin relokasi_hh('$id_cell', '$slot_yard', '$row_yard', '$tier_yard','$blok', '$id2_cell', '$nama_blok', '$id_user', '$id_plcmnt', '$no_container','MUAT'); end;";
														$db->query($relokasi);
													...
													else
													{
														...
														$relokasi = "begin relokasi_hh('$id2_cell', '$slot2_yard', '$row_yard', '$tier_yard','$blok', '$id2_cell', '$nama_blok', '$id_user', '$id_plcmnt', '$no_container', 'MUAT'); end;";
														$db->query($relokasi);
					...
					else
					{
											...
											$relokasi = "begin relokasi_hh('$cell_address', '$slot_', '$row_yard', '$tier_yard','$blok', 'NULL', '$nama_blok', '$id_user', '$id_plcmnt', '$no_container','MUAT'); end;";
											$db->query($relokasi);
					...
					if (($date['ID_BLOCKING_AREA'] == "$blok") AND ($date['SLOT_YARD'] == "$slot_") AND ($date['ROW_YARD'] == "$row_yard") AND ($date['TIER_YARD'] == "$tier_yard")){
							...
							if ($kd_status == 54){
								...
								$isws_list = "UPDATE ISWS_LIST_CONTAINER SET KODE_STATUS = '54' WHERE NO_CONTAINER = '$no_container' AND NO_UKK = '$no_ukk'";
								$db->query($isws_list);
