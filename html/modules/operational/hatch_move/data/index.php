...
$db = getDB();
$query ="SELECT COUNT(*) AS NUMBER_OF_ROWS FROM BH_REQUEST ".$wh;
//print_r($query);die;
...
$limit = $limit*$page;
// if($sidx=="")	$sidx=1;
$query="SELECT * FROM (SELECT A.*, (SELECT COUNT(B.NO_CONTAINER) FROM BH_DETAIL_REQUEST B WHERE A.ID_REQUEST=B.ID_REQUEST ) JUMLAH_CONT, ROWNUM rnum FROM BH_REQUEST A ".$wh." AND ROWNUM <= ".$limit." ORDER BY ".$sidx." ".$sord.") where rnum > ".$start;
//echo $query; die;
