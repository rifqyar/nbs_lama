...
jQuery(function() {
 jQuery("#l_sum").jqGrid({
	url:'pymad.stgpymad_nbsopus_pontianak.sum/?year=',
	mtype : "post",
	datatype: "json",
	...
	caption:"&nbsp;&nbsp;&nbsp;&nbsp;SUMMARY PYMAD NBS OPUS PONTIANAK",
	footerrow: true,
 ...
 jQuery("#l_user").jqGrid({
	url:'pymad.stgpymad_nbsopus_pontianak.data/?year=',
	mtype : "post",
	datatype: "json",
	...
	shrinkToFit: false,
	addurl: "maintenance.master.user/add/",
	caption:"&nbsp;&nbsp;&nbsp;&nbsp;STAGING PYMAD NBS OPUS PONTIANAK",
	grouping: true,
	multiselect:true,
	groupingView : { 
 ...
 $.post("<?=HOME?>pymad.stgpymad_nbsopus_pontianak.ajax/update_stat",{AID:s,PYMASTAT:'Y'},function(){
 ...
 var col = $('#col').val();
 var key = $('#key').val();
 $("#l_user").jqGrid('setGridParam',{datatype:'json',url:'pymad.stgpymad_nbsopus_pontianak.data/?col='+col+'&key='+key}).trigger('reloadGrid');
 $("#l_sum").jqGrid('setGridParam',{datatype:'json',url:'pymad.stgpymad_nbsopus_pontianak.sum/?col='+col+'&key='+key}).trigger('reloadGrid');
 });
 ...
 $('#key').val('');
 var year = $('#year').val();
 $("#l_sum").jqGrid('setGridParam',{datatype:'json',url:'pymad.stgpymad_nbsopus_pontianak.sum/?year='+year}).trigger('reloadGrid');
 $("#l_user").jqGrid('setGridParam',{datatype:'json',url:'pymad.stgpymad_nbsopus_pontianak.data/?year='+year}).trigger('reloadGrid');
 });
 jQuery("#m2").click( 
 function() { 
 var s; s = jQuery("#l_user").jqGrid('getGridParam','selarrrow'); 
 $.post("<?=HOME?>pymad.stgpymad_nbsopus_pontianak.ajax/update_stat",{AID:s,PYMASTAT:'X'},function(){
 ...
 var col  = $('#col').val();
 var key  = $('#key').val();
 window.open("<?=HOME?>pymad.stgpymad_nbsopus_pontianak.excel/?col="+col+"&key="+key+"&year="+year,'_blank');
 }

function detail(id,vv,bprp,cust)
{
	$('#add_config').load("<?=HOME?>pymad.stgpymad_nbsopus_pontianak.ajax/detail_trans",{AID:id,}).dialog({modal:true, height:400,width:800, buttons: { "close": function() 
			{ 
				$(this).dialog("close"); 
...
<div class="content">
<table width="980px" cellspacing="0" height="81px" border="0" style="margin-top:-33px;margin-left:-24px">
	<tr>
		<td  class="" style="padding-top:7px">
			<span class="graybrown " style="">
				<img src='images/staging.png' border='0' class="icon" style="width:40px"/> 
...
<tr>
			...
			<a class="myButton" onclick="print()"><img src='images/excel-2.png' border='0'/>&nbsp;Excel</a>
			</td>
		</tr>
		<tr>
