...
SELECT
        CASE
            WHEN ATD = ETD THEN 0 
            WHEN (DURASI > 5 AND ETD < AKHIR_MASA2) AND  ((AKHIR_MASA2 - ETD) <= 5) AND AWAL_MASA2=ATD THEN 0 
            WHEN (DURASI > 5 AND ETD < AKHIR_MASA2) AND  ((AKHIR_MASA2 - ETD) <= 5) AND (AKHIR_MASA2 > ATD) THEN (ATD-AWAL_MASA2)+1     
            WHEN (DURASI > 5 AND ETD < AKHIR_MASA2) AND  (AKHIR_MASA2 - ETD) <= 5 THEN AKHIR_MASA2 - ETD
            WHEN (DURASI > 5 AND ETD < AKHIR_MASA2) AND  ((AKHIR_MASA2 - ETD) > 5) THEN ATD-AKHIR_MASA1
            WHEN (DURASI > 5) AND  (ETD > AWAL_MASA3) THEN 0           
            WHEN DURASI > 5 AND ETD = AKHIR_MASA2 THEN 0
            WHEN DURASI > 5 AND ETD = AWAL_MASA3 THEN 0
        ...
        CASE              
             ...
             WHEN DURASI < 10 THEN 0
             WHEN (DURASI>10) AND (ETD>AWAL_MASA3) AND (ATD=ETD) THEN ATD - AWAL_MASA3
             WHEN (DURASI > 10) AND (ETD > AWAL_MASA3) THEN ATD - ETD
             WHEN (DURASI > 10) AND (ETD > AWAL_MASA3) AND AKHIR_MASA2 = ETD THEN ATD - ETD
             WHEN (DURASI > 10) AND (ETD = AWAL_MASA3) THEN ATD - ETD
             WHEN (DURASI > 10) AND (ETD < AWAL_MASA3) THEN (ATD - AWAL_MASA3)+1
...
<table class="table_alumni">
	...
	?>
	<tr>
		 <td class="td-tengah"><?=$no?></td>
		 <td class="td-kiri"><?=$id_req; ?></td>
		 <td class="td-kiri"><?=$row['NO_CONTAINER']?></td>
		 <td class="td-kiri"><?=$row['SIZE_CONT']?></td>
		 <td class="td-kiri"><?=$row['TYPE_CONT']?></td>
		 <td class="td-kiri"><?=$row['STATUS_CONT']?></td>
		 <td class="td-kiri"><?=$row['HZ']?></td>
		 <td class="td-kiri"><?=$row['GATE_IN_DATE']?></td>
                 <td class="td-kiri"><?=$row['ETD']?></td>                
                 <td class="td-kiri"><?=$row['ATD']?></td>
                 <td class="td-kiri"><?=$row['DURASI']?></td>
                 <td class="td-kiri"><?=$row['KURANG_BAYAR_MASA2']?></td>
                 <td class="td-kiri"><?=$row['KURANG_BAYAR_MASA3']?></td> 
                 <td class="td-kiri"><?=$row['KODE_PBM']?></td>                
