...
setInterval(function()
		{
			window.location = "<?=HOME?>maintenance.monitoring.layout/load_layout?id=81";
		},
		300000);
...
function load_block(block_id,slot_id,row_id,yard_id,tgl,per)
{
	...
	$('#loadx').load("<?=HOME?>report.yard_periodik.sticky_layout/load_row_slot?block_id="+block_id+"&slot_id="+slot_id+"&row_id="+row_id+"&yard_id="+yard_id+"&tgl="+tgl+"&per="+per+"").dialog({modal:true, height:660,width:790});
}
...
function yor_p(tgl,per)
{
	$('#loadd').load("<?=HOME?>report.yard_periodik.yard/yor_performance?tgl="+tgl+"&per="+per+"").dialog({modal:true, height:600,width:500});
}

function vessel_p()
{
	$('#loadv').load("<?=HOME?>report.yard_periodik.yard/vessel_performance").dialog({modal:true, height:600,width:700});
}

function yd_realization(tgl, per)
{
	$('#loadyd').load("<?=HOME?>report.yard_periodik.yard/print_yd?tgl="+tgl+"&per="+per+"").dialog({modal:true, height:200,width:180,title: "Yard Realization Print"});
}
...
function cetak_yard()
{
	...
	{
		window.open('<?=HOME?>report.yard_periodik.print/yard_realization?id_yard='+yd_id+'&id_block='+block_id+'&nm_block='+block_nm+'&slot='+slot_yd,'_blank');
	}
}

function filter_yard(tgl, per)
{
	$('#load_filter').load("<?=HOME?>report.yard_periodik.yard/filter?tgl="+tgl+"&per="+per+"").dialog({modal:true, height:300,width:400});
}
...
<ul id="dock">
        <li id="tools">
            <ul class="free">
                ...
                <li valign="middle"><button id="but_yor" onclick="yor_p('<?=$tgl?>','<?=$per?>')"><img src="<?=HOME?>images/chart-pie.png" width="40" height="40"></button> YOR Performance</li>
				...
				<li valign="top"><button id="but_cont"><img src="<?=HOME?>images/contaier.png" width="40" height="40"></button> Container Perf.</li>
				<li valign="top"><button id="but_yd" onclick="yd_realization('<?=$tgl?>','<?=$per?>')" ><img src="<?=HOME?>images/block_yd.png" width="40" height="40"></button> Yard Realization</li>
				<li valign="top"><button id="but_filter" onclick="filter_yard('<?=$tgl?>','<?=$per?>')"><img src="<?=HOME?>images/filter.png" width="40" height="40"></button> Filter Lapangan</li>
			<!--	<?//if ($_SESSION["ID_GROUP"]==1){?>
					<li valign="top"><button id="rst" onclick="reset_yard('<?=$tgl?>','<?=$per?>')"><img src="<?=HOME?>images/clock_reset.png" width="40" height="40"></button> Reset Yard</li>
					<?//}?>-->
           </ul>
        </li>
		<li id="helps">
...
-webkit-border-radius: 10px 10px 10px 10px;">
	<legend>Block</legend>
        <table align="left" width="100">
                <tr align="center"><td class="grid-header">Color Code</td><td class="grid-header">Block Type</td></tr>
                <tr align="center" bgcolor="#ffffff">
					<td align="center" style="width:15px;height:5px;font-size:8px; font-family:Tahoma; border:1px solid #ffffff;background-color:#86BCFF; "></td>
					<td class="grid-cell">Export</td>
				</tr>
...
-webkit-border-radius: 10px 10px 10px 10px;">
	<legend>Legend Tier</legend>
        <table align="left" width="200">
                <tr align="center"><td class="grid-header">Color Code</td><td class="grid-header">Block Type</td>
					<td class="grid-header">Color Code</td><td class="grid-header">Block Type</td>
				</tr>
...
-webkit-border-radius: 10px 10px 10px 10px; ">
    <center>
		...
		<table width="1315"><tr height="5"bgcolor="#cecdcd" align="center" ><td></td></tr>
			...
			<font color="black"><h2>Layout</font> <font color="black">Lapangan Ex TBB</font><br>
			Tanggal <?=$tgl?> Pukul <?=$per?></h2></td></tr>
         ...
         <?php
			...
			*/
			$query_cell2 = "SELECT a.INDEX_CELL as INDEX_CELL, b.ID AS ID, b.NAME AS NAME, a.SLOT_ AS SLOT_, a.ROW_ AS ROW_, a.STATUS_BM,b.POSISI,a.SIZE_PLAN_PLC FROM 	YD_BLOCKING_AREA b,         
                             (select * from ret_blocking_cell where date_created > TO_DATE('$tgl $per','dd/mm/rrrr hh24:mi:ss') AND date_created < TO_DATE('$tgl $per','dd/mm/rrrr hh24:mi:ss')+4/24)  a WHERE a.ID_BLOCKING_AREA = b.ID AND b.ID_YARD_AREA = '81' AND a.SIZE_PLAN_PLC IS NULL 
                            UNION 
                            SELECT d.INDEX_CELL AS INDEX_CELL, c.ID AS ID, c.NAME AS NAME, d.SLOT_ AS SLOT_, d.ROW_ AS ROW_, d.STATUS_BM,c.POSISI,d.SIZE_PLAN_PLC FROM YD_BLOCKING_AREA c,  (select * from ret_blocking_cell where date_created > TO_DATE('$tgl $per','dd/mm/rrrr hh24:mi:ss') AND date_created < TO_DATE('$tgl $per','dd/mm/rrrr hh24:mi:ss')+4/24)  d WHERE d.ID_BLOCKING_AREA = c.ID AND c.ID_YARD_AREA = '81' AND d.SIZE_PLAN_PLC = '40d'
                            UNION
                            SELECT d.INDEX_CELL AS INDEX_CELL, c.ID AS ID, c.NAME AS NAME, d.SLOT_ AS SLOT_, d.ROW_ AS ROW_,d.STATUS_BM,c.POSISI,d.SIZE_PLAN_PLC FROM YD_BLOCKING_AREA c,  (select * from ret_blocking_cell where date_created > TO_DATE('$tgl $per','dd/mm/rrrr hh24:mi:ss') AND date_created < TO_DATE('$tgl $per','dd/mm/rrrr hh24:mi:ss')+4/24)  d WHERE d.ID_BLOCKING_AREA = c.ID AND c.ID_YARD_AREA = '81' AND d.SIZE_PLAN_PLC = '20'
                           -- UNION 
                           -- SELECT d.INDEX_CELL AS INDEX_CELL, c.ID AS ID, c.NAME AS NAME, d.SLOT_ AS SLOT_, d.ROW_ AS ROW_,d.STATUS_BM,c.POSISI,d.SIZE_PLAN_PLC FROM YD_BLOCKING_AREA c,  (select * from ret_blocking_cell where date_created > TO_DATE('$tgl $per','dd/mm/rrrr hh24:mi:ss') AND date_created < TO_DATE('$tgl $per','dd/mm/rrrr hh24:mi:ss')+4/24)  d WHERE d.ID_BLOCKING_AREA = c.ID AND c.ID_YARD_AREA = '81' AND d.SIZE_PLAN_PLC = '40b'
			...
			//echo $index_cell_;
				...
				{                
					...
					if(isset($_POST["id_vessel"])||isset($_POST["tonase"])||isset($_POST["tuj"])||isset($_POST["size"])||isset($_POST["type"]))
					{
						...
						$query_place = "SELECT COUNT(a.ID_PLACEMENT) JUM FROM  (select * from ret_placement_yard where date_created > TO_DATE('$tgl $per','dd/mm/rrrr hh24:mi:ss') AND date_created < TO_DATE('$tgl $per','dd/mm/rrrr hh24:mi:ss')+4/24)  a, TB_CONT_JOBSLIP b WHERE a.ID_JOBSLIP = b.ID_JOB_SLIP AND a.ID_BLOCKING_AREA = '$id_block' AND a.SLOT_YARD = '$slot_' AND a.ROW_YARD = '$row_' AND a.ID_CELL = '$index_cell_' ". $query_sz . $query_type. $query_idvs . $query_tuj. $query_ton . $query_sta; 
						$filter=1;
                    }
					else
					{
						$query_place = "SELECT COUNT(ID_PLACEMENT) JUM FROM  (select * from ret_placement_yard where date_created > TO_DATE('$tgl $per','dd/mm/rrrr hh24:mi:ss') AND date_created < TO_DATE('$tgl $per','dd/mm/rrrr hh24:mi:ss')+4/24)  WHERE ID_BLOCKING_AREA = '$id_block' AND SLOT_YARD = '$slot_' AND ROW_YARD = '$row_' AND ID_CELL = '$index_cell_' ";
						$filter=0;
					}
                ...
                <?  if (($filter==0)&&($placement <> 0))
				
					{
						...
						?>
							<td data-tooltip="sticky<?=$index_cell_?>" onclick="load_block('<?=$id_block?>','<?=$slot_?>','<?=$row_?>','<?=$yard_id?>','<?=$tgl_ori?>','<?=$per?>')"  onMouseOut="this.style.backgroundColor=<?=$color_plc?>" align="center" style="width:30px;height:10px;font-size:8px; font-family:Tahoma; border:1px solid #ffffff;background-color:<?=$color_plc?>; " <?=$cr?>><blink><font color="white"><?=$placement//echo $row['NAME'];=$index_cell_?><a><? //echo $placement ?><?//=$st_bm;?><?//=$id_block;?></a></font></blink></td>
...
<!--                     </div>-->
				   ...
				   <? 
					if ($st_bm=='Muat')
					{?>
                          <td data-tooltip="sticky<?=$index_cell_?>" onclick="load_block('<?=$id_block?>','<?=$slot_?>','<?=$row_?>','<?=$yard_id?>','<?=$tgl_ori?>','<?=$per?>')" onMouseOver="this.style.backgroundColor='#CCFF33'" onMouseOut="this.style.backgroundColor='#86BCFF'" align="center" style="width:30px;height:10px;font-size:8px; font-family:Tahoma; border:1px solid #ffffff;background-color:#86BCFF; " <?=$cr?>><?echo $placement ;// echo $row['NAME'];// echo $index_cell_ ; // echo $index_cell_ ;//echo $st_bm;?><?//=$id_block;?></td>
					<?}
					else if ($st_bm=='Bongkar')
					{
					?>
							<td data-tooltip="sticky<?=$index_cell_?>" onclick="load_block('<?=$id_block?>','<?=$slot_?>','<?=$row_?>','<?=$yard_id?>','<?=$tgl_ori?>','<?=$per?>')" onMouseOver="this.style.backgroundColor='#CCFF33'" onMouseOut="this.style.backgroundColor='#82ff07'" align="center" style="width:30px;height:10px;font-size:8px; font-family:Tahoma; border:1px solid #ffffff;background-color:#82ff07; ">
        ...
        <div id="mystickytooltip" class="stickytooltip">
                    ...
                    <? 
								...
								//	$per		= $_GET['per'];
                                    ...
                                    $query_cell4 = "SELECT a.INDEX_CELL, b.ID, b.NAME, a.SLOT_, a.ROW_ FROM YD_BLOCKING_AREA b, (select * from ret_blocking_cell where date_created > TO_DATE('$tgl $per','dd/mm/rrrr hh24:mi:ss') AND date_created < TO_DATE('$tgl $per','dd/mm/rrrr hh24:mi:ss')+4/24) a WHERE  a.ID_BLOCKING_AREA = b.ID AND b.ID_YARD_AREA = '$yard_id' AND b.NAME <> 'NULL' ORDER BY a.INDEX_CELL ASC";
                            ...
                            ?>
                                    ...
                                    <? 
                                    $query_place = "SELECT NO_CONTAINER,SLOT_YARD,ROW_YARD FROM (select * from ret_placement_yard where date_created > TO_DATE('$tgl $per','dd/mm/rrrr hh24:mi:ss') AND date_created < TO_DATE('$tgl $per','dd/mm/rrrr hh24:mi:ss')+4/24) WHERE ID_BLOCKING_AREA = ".$dama['ID']." AND SLOT_YARD = ".$dama['SLOT_']." AND ROW_YARD = ".$dama['ROW_']." AND ID_CELL = ".$dama['INDEX_CELL']."";
                                   ...
                                   <font color="red"> <i> -- There is no container heres -- </i></font>
									...
									?>
                                       <?php
                                            $query_index_cell   = "SELECT  NO_CONTAINER, SIZE_, TON,TIER_YARD,TYPE_CONT,STATUS_CONT, ID_PEL_TUJ FROM (select * from ret_placement_yard where date_created > TO_DATE('$tgl $per','dd/mm/rrrr hh24:mi:ss') AND date_created < TO_DATE('$tgl $per','dd/mm/rrrr hh24:mi:ss')+4/24) a WHERE ID_CELL = ".$dama['INDEX_CELL']." ORDER BY TIER_YARD DESC";
                                       ...
                                       ?>
										   ...
										   <td>
                                           <b>No Container : </b> <font color="red"><?=$row['NO_CONTAINER']?></font><br>
										   <b>Size/Type/Status :</b> <font color="red"> <?=$row['SIZE_']?>/<?=$row['TYPE_CONT']?>/<?=$row['STATUS_CONT']?></font><BR>
										   <b>Tonase : </b> <font color="red"><?=$row['TON']?></font><br>
										   <b>Tujuan : </b> <font color="red"><?=$row['ID_PEL_TUJ']?></font><br>
										   </td></tr>
