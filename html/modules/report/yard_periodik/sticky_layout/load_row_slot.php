...
<?
	...
	$query_ = "SELECT NAME, TIER, (select count(1) from (select * from ret_blocking_cell where date_created > TO_DATE('$tgl $per','dd/mm/rrrr hh24:mi:ss') AND date_created < TO_DATE('$tgl $per','dd/mm/rrrr hh24:mi:ss')+4/24)  b WHERE b.ID_BLOCKING_AREA='$block' and b.SLOT_='$slot') AS JML_ROW FROM YD_BLOCKING_AREA a WHERE a.ID_YARD_AREA='$yard' AND a.ID='$block'";
...
<div id="mapping2w2">
	<table>
		<tr>
			<td data-tooltip="stickyz1"><b>BLOK <?=$blok?> SLOT <?=$slot?> <br>
			Tanggal <?=tgl?> Pukul <?=$per?></b></td>
		</tr>
	</table>
	...
	?> <tr>
		...
		<?  for($i=1;$i<=$width;$i++)
			{
				$query2 = "SELECT ID_CELL,NO_CONTAINER, SIZE_, TYPE_CONT, STATUS_CONT, ID_PEL_ASAL, ID_PEL_TUJ   FROM (select * from ret_placement_yard where date_created > TO_DATE('$tgl $per','dd/mm/rrrr hh24:mi:ss') AND date_created < TO_DATE('$tgl $per','dd/mm/rrrr hh24:mi:ss')+4/24)  WHERE ID_BLOCKING_AREA='$block' AND SLOT_YARD='$slot' AND ROW_YARD='$i' AND TIER_YARD='$t'";
