<?php
	...
	$tl =  xliteTemplate('print_nota_pen.htm');
	...
	*/
		...
		$sql_xpi = "BEGIN PROC_BATALMUATDEL_PEN('$no_req','$jenis'); END;";
		// echo $sql_xpi;die;
	...
	$detail_nota  = "SELECT TO_CHAR(a.TARIF, '999,999,999,999') AS TARIF, 
                      ...
                      WHERE a.ID_CONT = b.KODE_BARANG(+) and a.ID_REQ = '$no_req' AND KETERANGAN not in ('ADM','MATERAI')";
				   
	$result       = $db->query($detail_nota);
	$row_detail   = $result->getAll();
	
	
	$total_		  = "SELECT SUM(TOTAL) TOTAL FROM NOTA_BATALMUAT_D_PEN_TMP WHERE ID_REQ = '$no_req' AND KETERANGAN not in ('ADM','MATERAI')";
    ...
    //$adm   			= $jml_cont*10000;
	$query_adm		= "SELECT TO_CHAR(SUM(TOTAL) , '999,999,999,999') AS ADM, SUM(TOTAL) ADM1 FROM NOTA_BATALMUAT_D_PEN_TMP WHERE ID_REQ = '$no_req' AND KETERANGAN = 'ADM'";
	...
	/* gagat modif 12 februai 2020 Menghitung Bea Materai*/
	$query_mtr		= "SELECT  TOTAL AS BEA_MATERAI FROM NOTA_BATALMUAT_D_PEN_TMP 
	...
	$total_bayar        =  $totals + $ppn + $bea_materai;/**GAGAT add + $bea_materai */
