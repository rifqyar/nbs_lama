<?php	
	...
	$query_nota	= "SELECT 
                                    ...
                                    b.ALAMAT, 
                                    b.NPWP, 
                                    to_char(b.TGL_REQ,'dd-mm-yyyy') TGL_REQ,
                                    b.VESSEL, 
                                    b.VOYAGE_IN,b.VOYAGE_OUT,
                                    (SELECT 
                                            KEGIATAN 
                                     FROM 
                                            MASTER_PROSES_KEGIATAN a 
                                     WHERE 
                                     b.JENIS = a.ID_KEG) KEG
									 ,TGL_REQ TGL_REQ2
                           FROM 
                                    req_batalmuat_h b
                           WHERE 
	...
	//echo "BEGIN PROC_BATALMUAT('$no_req','$jenis'); END;";die;
		...
		$sql_xpi = "BEGIN PROC_BATALMUAT_BM('$no_req','$jenis'); END;";				
		$db->query($sql_xpi);            
	...
	$query_tot		= "SELECT TO_CHAR('$totals' , '999,999,999,999') AS TOTAL_ALL FROM DUAL"; //ematerai (Total asli)
...
/*//start Implementasi einvoice ematerai, gagat 14 Oktober 2019*/
	$query_mtr		= "SELECT TOTAL AS BEA_MATERAI FROM 
                            NOTA_BATALMUAT_D_TMP WHERE 
                            ID_REQ='$no_req' 
...
/**gagat end modify materai 17 oktober 2019**/
	...
	$total_bayar        =  $totals + $ppn + $bea_materai; /**gagat modify bea materai 17 oktober 2019 [ +$bea_materai ]**/
	...
	//debug($no_req);die;
        //$tl->assign("jenis_nota_bm",$jenis_nota_bm);
