...
$query="SELECT 
		ID_PROFORMA,
		ID_REQ,
		TO_CHAR(TGL_REQ,'dd-mm-yyyy') TGL_REQUEST
        FROM 
		NOTA_BATALMUAT_H_PENUMPUKAN 
        WHERE 
		TRIM(ID_REQ) = TRIM('$no_req') 
		and status!='X'";
...
$query= "SELECT B.ID_PROFORMA, B.ID_REQ, B.EMKL, B.ALAMAT, B.NPWP, B.TGL_REQ, B.JENIS,
...
//print_r("SELECT * from TB_NOTA_DELIVERY_D A, TB_NOTA_DELIVERY_H B, MASTER_BARANG C WHERE A.ID_NOTA='$p1'  AND A.ID_NOTA=B.ID_NOTA AND C.KODE_BARANG=A.ID_CONT");die;
 ...
 /**Fauzan start modify amount materai 17 Oktober 2019 **/
	$query_materai="SELECT TOTAL FROM NOTA_BATALMUAT_D_PENUMPUKAN WHERE ID_REQ = '$no_req' AND KETERANGAN='MATERAI' ";
...
$query_dtl=" SELECT 
                 ...
                 b.STATUS 
    FROM 
                NOTA_BATALMUAT_D_PEN_TMP a, master_barang b 
    WHERE 
                a.ID_CONT = b.KODE_BARANG 
                AND a.ID_REQ = '$no_req' 
                AND KETERANGAN NOT IN ('ADM','MATERAI')
                AND KETERANGAN LIKE '%PENUMPUKAN%'";/**gagat modif 20 feb 2020 NOT IN ('MATERAI')*/
