<?php
	...
	//echo "BEGIN PROC_BATALMUAT('$no_req','$jenis'); END;";die;
		...
		$sql_xpi = "BEGIN PROC_BATALMUAT_PEN('$no_req','$jenis'); END;";
	...
	$detail_nota  = "SELECT 
                                ...
                                b.STATUS 
                      FROM 
                                NOTA_BATALMUAT_D_PEN_TMP a, 
                                MASTER_BARANG b 
                      WHERE 
                                a.ID_CONT = b.KODE_BARANG 
                                AND a.id_req = '$no_req' 
                                AND KETERANGAN NOT IN  ('ADM','MATERAI')"; /**18 Oktober 2019 fauzan modify and KETERANGAN NOT IN ('MATERAI')**/
	...
	$jml_cont	= "SELECT COUNT(NO_CONTAINER) JML FROM req_batalmuat_d WHERE id_req = '$no_req'";
        ...
        //$adm   			= $jml_cont*10000;
	$query_adm		= "SELECT 
                                            TO_CHAR(SUM(TOTAL) , '999,999,999,999') AS ADM, 
                                            SUM(TOTAL) ADM1 
                                    FROM 
                                            NOTA_BATALMUAT_D_PEN_TMP 
                                    WHERE 
