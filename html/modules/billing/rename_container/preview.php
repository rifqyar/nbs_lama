<?php
	...
	/**GAGAT modify 25 november 2019 add materai */
	$query_materai		= "SELECT SUB_TOTAL AS TOTAL FROM NOTA_RENAME_D_TEMP WHERE ID_REQ='$no_req' AND KETERANGAN = 'MATERAI'";
	$result_materai		= $db->query($query_materai);
	$row_materai		= $result_materai->fetchRow();
	
	if($row_materai['TOTAL'] > 0){
		$bea_materai = $row_materai['TOTAL']; 
	}else{
		$bea_materai = 0;
	}
	...
	$total_bayar        =  $totals + $ppn + $bea_materai;/*gagat modify 24 jan 2020*/
