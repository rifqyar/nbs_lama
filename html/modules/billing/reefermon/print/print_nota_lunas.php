...
$query="SELECT ID_PROFORMA, ID_REQ, TO_CHAR(TGL_REQ,'dd-mm-yyyy') TGL_REQUEST FROM NOTA_MONREEFER_H WHERE TRIM(ID_REQ) = TRIM('$no_req')";
$hasil=$db->query($query);
$hasil_=$hasil->fetchRow();
$notanya=$hasil_['ID_PROFORMA'];
$param_tgl=$hasil_['TGL_REQUEST'];
/*PTP*/
    $query = "select fc_nm_perusahaan('$param_tgl','RFRMON') NM_PERUSAHAAN from dual";
// echo $query; die();
    $rq    = $db->query($query)->fetchRow();
    $nm_perusahaan = $rq['NM_PERUSAHAAN'];
/*PTP*/

$query="UPDATE NOTA_MONREEFER_H SET TGL_CETAK=SYSDATE WHERE ID_REQ='$no_req'";
$db->query($query);
...
//gagat 30 oktober 2019 materai 
$queryMaterai="SELECT  
					  a.SUB_TOTAL AS MATERAI
					  FROM NOTA_MONREEFER_D a 
					  where a.ID_REQ = '$no_req' and KETERANGAN = 'MATERAI'";
$data_materai = $db->query($queryMaterai)->fetchRow();
if($data_materai['MATERAI']>0){
	$bea_materai=$data_materai['MATERAI'];
}else{
	$bea_materai=0;
}
...
$query_dtl="SELECT TO_CHAR(a.TARIF, '999,999,999,999') AS TARIF, 
					  ...
					  b.STATUS,a.JML_SHIFT,a.PLUGIN,a.PLUGOUT
					  FROM NOTA_MONREEFER_D a left join  master_barang b on (a.ID_CONT = b.KODE_BARANG)
					  where a.ID_REQ = '$no_req' and KETERANGAN not in ('ADM','MATERAI')";
	...
	foreach($row2 as $rows) {
		...
		<tr><td colspan="3" width="75"><font size="6">'.'<b>'.$rows[KETERANGAN].'</b>'.'</font></td>
                        <td width="15" align="center"><font size="6">'.'<b>'.$rows[JML_SHIFT].'</b>'.'</font></td>                              
...
$query_jum="SELECT COUNT(1) JUM_DETAIL FROM NOTA_MONREEFER_D A WHERE A.ID_REQ='$no_req' AND KETERANGAN <> 'ADM'";
...
for($pg=1; $pg<=$jum_page; $pg++) {
	...
	$tbl = <<<EOD
		...
		<tr>                    
                ...
                <tr>
                    <td COLSPAN="6"><b>MONITORING REEFER</b></td>                                        
                </tr>                
                ...
                <tr>                              
                    ...
                    <th width="15" align="center"><font size="6"><b>SHIFT</b></font></th>
