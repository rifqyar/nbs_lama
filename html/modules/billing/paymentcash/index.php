...
jQuery(function() {
 jQuery("#l_pay").jqGrid({
	...
	mtype : "post",
	datatype: "json",
	colNames:['','No. Proforma','No. Nota','No. Faktur','No. Request','EMKL','Modul','Vessel - Voy','Save Nota','Payment','Payment VIA'], 
	colModel:[
		{name:'act', width:70, align:"center", search:false},
		{name:'np', width:120, align:"center"},
...
function pay(a,b,c, vessel, voyin, voyout, total)
{
	$('#pay').load("<?=HOME?>billing.paymentcash.ajax/payment?idn="+a+"&req="+encodeURIComponent(b)+"&ket="+c+"&vessel="+encodeURIComponent(vessel)+"&voyin="+encodeURIComponent(voyin)+"&voyout="+encodeURIComponent(voyout)+"&total="+total, function(data){
		$("#paid_via").load("<?=HOME?>billing.paymentcash.ajax/paid_via",{KD_PELUNASAN:1,idn:a,noreq:b,modul:c});	
...
function sync_payment(id_request, id_nota)
{
    ...
    $("#l_pay").jqGrid('setGridParam',{url:"datanya/data_diskon?q=payment_cash&idreq="+id_req, datatype:"json"}).trigger("reloadGrid");
}
</script>
