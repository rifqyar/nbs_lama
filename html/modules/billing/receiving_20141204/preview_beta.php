<?php
	...
	//--------------------------
	$query_nota	= "SELECT b.KODE_PBM AS EMKL, b.ALAMAT, b.NPWP, b.TGL_REQUEST, VESSEL, VOYAGE FROM req_receiving_h b
	...
	//echo "BEGIN nota_anne_v2('$no_req','$tglstack','$tglopenstack'); END;";
	$sql_xpi = "BEGIN nota_anne_v2('$no_req'); END;";
	//echo $sql_xpi;die;
	$db->query($sql_xpi);
            
	$detail_nota  = " SELECT TO_CHAR(a.TARIF, '999,999,999,999') AS TARIF, TO_CHAR(a.SUB_TOTAL, '999,999,999,999') AS BIAYA, a.KETERANGAN, a.HZ, a.JUMLAH_CONT JML_CONT, TO_DATE(a.TGL_START_STACK,'dd/mm/yyyy') START_STACK, TO_DATE(a.TGL_END_STACK,'dd/mm/yyyy') END_STACK, b.UKURAN SIZE_, b.TYPE TYPE_, b.STATUS FROM nota_receiving_d_tmp a, master_barang b WHERE a.ID_CONT = b.KODE_BARANG and a.ID_REQ = '$no_req'";
