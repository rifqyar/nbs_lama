<?php
	...
	//--------------------------
	$query_nota	= "SELECT a.NAMA AS EMKL, a.ALAMAT, a.NPWP FROM tb_req_receiving_h b, master_pbm a
	...
	$query_tgl	= "SELECT TO_CHAR(TGL_STACK,'dd/mon/yyyy') TGL_STACK, TO_CHAR(TGL_OPEN_STACK,'dd/mon/yyyy') TGL_OPEN_STACK, SHIFT_REEFER FROM tb_req_receiving_h 
                             WHERE ID_REQ = '$no_req'
                            ";
	//echo $query_tgl;die;			   
       ...
       // echo $tgl_re;die;
		...
		$sql_xpi = "DECLARE
                            ...
                            BEGIN 
                                 no_req := '$no_req';
                                 tgl_stack := '$tglstack';
                                 tgl_open_stack := '$tglopenstack';
                                 nota_anne(no_req,tgl_stack,tgl_open_stack); 
                            END;";
                //echo $sql_xpi;die;
		$db->query($sql_xpi);
              //  echo $sql_xpi;die;
                
	$detail_nota  = " SELECT TO_CHAR(a.TARIF, '999,999,999,999') AS TARIF, TO_CHAR(a.SUB_TOTAL, '999,999,999,999') AS BIAYA, a.KETERANGAN, a.HZ, a.JUMLAH_CONT JML_CONT, TO_DATE(a.TGL_START_STACK,'dd/mm/yyyy') START_STACK, TO_DATE(a.TGL_END_STACK,'dd/mm/yyyy') END_STACK, b.UKURAN SIZE_, b.TYPE TYPE_, b.STATUS FROM tb_temp_detail_nota a, master_barang b WHERE a.ID_CONT = b.KODE_BARANG and a.ID_REQ = '$no_req'";
