<?php
 ...
 //Akses Prosedure nota_transhipment Untuk create Nota Temporary
 $sql_xpi = "DECLARE no_req VARCHAR2(50); BEGIN no_req :='".$no_req."'; nota_transhipment(no_req); END;";
                //echo $sql_xpi;die;
				
 $db->query($sql_xpi);
		
		
	$query_nota	= "SELECT A.SHIPPING_LINE, A.ALAMAT, A.NPWP, B.TGL_START_STACK, B.TGL_END_STACK, A.VESSEL_OLD, A.VOYAGE_IN_OLD, to_date(A.TGL_REQUEST,'dd/mm/rrrr') TGL_REQUEST
					FROM REQ_TRANSHIPMENT_H A LEFT JOIN NOTA_TRANSHIPMENT_D_TMP B ON A.ID_REQ=B.ID_REQ
					WHERE A.ID_REQ = '$no_req'
					AND B.KETERANGAN <> 'ADM'";
	$result		= $db->query($query_nota);
	$row_nota	= $result->fetchRow();
	$qtglreq 	= "SELECT TGL_REQUEST FROM REQ_TRANSHIPMENT_H WHERE ID_REQ='$no_req'";
	$rtglreq	= $db->query($qtglreq)->fetchRow();
	$param_tgl  = $rtglreq[TGL_REQUEST];
...
$detail_nota  = "SELECT TO_CHAR(SUM(a.TARIF), '999,999,999,999') AS TARIF, TO_CHAR(SUM(a.SUB_TOTAL), '999,999,999,999') AS BIAYA, a.KETERANGAN, a.HZ, a.JUMLAH_CONT JML_CONT, a.JUMLAH_HARI_KENA, b.UKURAN SIZE_, b.TYPE TYPE_, b.STATUS FROM NOTA_TRANSHIPMENT_D_TMP a, master_barang b WHERE a.ID_CONT = b.KODE_BARANG and a.ID_REQ = '$no_req' and a.ID_CONT IS NOT NULL GROUP BY a.KETERANGAN, a.HZ, a.JUMLAH_CONT, a.JUMLAH_HARI_KENA, b.UKURAN, b.TYPE, b.STATUS";
				 //  print_r($detail_nota);die;
	$result       = $db->query($detail_nota);
	$row_detail   = $result->getAll();
	
	
	$total_		  = "SELECT SUM(SUB_TOTAL) TOTAL FROM NOTA_TRANSHIPMENT_D_TMP WHERE ID_REQ = '$no_req'";
	...
	//Biaya Administrasi
	$query_adm		= "SELECT TO_CHAR(SUB_TOTAL , '999,999,999,999') AS ADM FROM NOTA_TRANSHIPMENT_D_TMP WHERE ID_REQ = '$no_req' AND KETERANGAN='ADM'";
	...
	$q_cek="select count(*) as JUM from NOTA_TRANSHIPMENT_H where trim(ID_REQ)=trim('".$no_req."') and STATUS='S'";
	//print_r($q_cek);die;
