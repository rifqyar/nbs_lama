...
/*PTP*/
	/*ipctpk*/
    //$query = "select fc_nm_perusahaan_nota('".$notanya."','STACKEXT') corporate_name from dual";
    // echo $query; die();
	...
	/**/
    $nm_perusahaan = $rq['CORPORATE_NAME'];
/*PTP*/
...
$db->query($query);
$query="UPDATE NOTA_STACKEXT_H SET TGL_CETAK=SYSDATE WHERE ID_NOTA='$notanya'";
$db->query($query);
...
$query="SELECT A.*,B.*, SYSDATE AS TGL_PROSES,UPPER(B.USER_REQ) AS NAMA,to_char(A.INVOICE_DATE,'DD-MM-YYYY') TGL_NOTA2, to_char(A.ADM_NOTA,'999,999,999,999,990.00') ADM_NOTAN,to_char(A.PPN,'999,999,999,999,990.00') PPNN, to_char(A.JUMLAH,'999,999,999,999,999.00') TAGIHANN,to_char(A.TOTAL,'999,999,999,999,999.00') TOTALN
		FROM NOTA_STACKEXT_H A, REQ_STACKEXT_H B WHERE A.ID_NOTA='$p1' AND TRIM(A.ID_REQUEST)=TRIM(B.ID_REQ)";
$data = $db->query($query)->fetchRow();

//start Gagat 04 februari 2020 materai
	$query_mtr="SELECT A.SUB_TOTAL  AS BEA_MATERAI 
				FROM NOTA_STACKEXT_D A WHERE A.ID_NOTA='$p1' AND A.KETERANGAN='MATERAI' ";
	$data_mtr = $db->query($query_mtr)->fetchRow();
	if($data_mtr['BEA_MATERAI']>0){
		$bea_materai=$data_mtr['BEA_MATERAI'];
	}else{
		$bea_materai=0;
	}
//end Gagat 04 Februari 2020 

 $date=date('d M Y H:i:s');
 $query3="SELECT DISTINCT MIN(TGL_START_STACK) TGL_START_STACK,  MAX(TGL_END_STACK)  TGL_END_STACK FROM NOTA_STACKEXT_D WHERE ID_NOTA='$p1' AND ID_CONT IS NOT NULL";
...
$query_dtl="SELECT a.KETERANGAN,
                             a.HZ,
                             sum(a.JUMLAH_CONT) JUMLAH_CONT,
                             a.JUMLAH_HARI,
                             c.UKURAN,
                             c.TYPE,
                             c.STATUS,
                             TO_CHAR (a.TGL_START_STACK, 'dd/mm/yyyy') TGL_START_STACK,
                             TO_CHAR (a.TGL_END_STACK, 'dd/mm/yyyy') TGL_END_STACK,
       TO_CHAR (A.TARIF, '999,999,999,999,999.00') AS TARIFN,
       TO_CHAR (SUM (a.SUB_TOTAL), '999,999,999,999') AS SUB_TOTALN
  FROM NOTA_STACKEXT_D A, NOTA_STACKEXT_H B, MASTER_BARANG C
 WHERE     A.ID_NOTA = '$p1'
       AND A.ID_NOTA = B.ID_NOTA
       AND C.KODE_BARANG = A.ID_CONT
	...
	foreach($row2 as $rows) {
		...
		$detail .= '
		<tr><td colspan="3" width="130"><font size="8">'.$rows[KETERANGAN].'</font></td>
					    <td width="80" align="center"><font size="8">'.$rows[TGL_START_STACK].'</font></td>
                        <td width="80" align="center"><font size="8">'.$rows[TGL_END_STACK].'</font></td>
                        ...
                        <td width="75" align="right"><font size="8">'.$rows[TARIFN].'</font></td>
...
$jum_data_page = 40;	//jumlah data dibatasi per page 18 data
$jum_page = ceil($data_jum['JUM_DETAIL']/$jum_data_page);	//hasil bagi pembulatan ke atas
if(($data_jum[JUM_DETAIL]%$jum_data_page)>50 || ($data_jum[JUM_DETAIL]%$jum_data_page)==0)	$jum_page++;	//jika pada page terakhir jumlah data melebihi 12, tambah 1 page lagi
for($pg=1; $pg<=$jum_page; $pg++) {
	...
	$tbl = <<<EOD
			<table>
                <tr>
                    <td COLSPAN="14" align="left"><b>$nm_perusahaan</b></td>
                </tr>
                <tr>
                ...
                <tr>
                    <td COLSPAN="6"></td>
                    <td COLSPAN="8" align="right"><font size="11"><b>NOTA PERPANJANGAN EXPORT</b></font></td>
                ...
                <tr>
                    <th colspan="3" width="130"><font size="8"><b>KETERANGAN</b></font></th>
					<th width="80" align="center"><font size="8"><b>TGL AWAL</b></font></th>
                    <th width="80" align="center"><font size="8"><b>TGL AKHIR</b></font></th>
                    ...
                    <th width="75" align="center"><font size="8"><b>TARIF</b></font></th>
                    <th width="35" align="center"><font size="8"><b>VAL</b></font></th>
...
<table>
					...
					<!-----04 Februari 2020 gagat----->
					<tr>
                        <td colspan="7"></td>
                        <td width="180" colspan="3" align="right">Bea Materai :</td>
                        <td width="120" colspan="2" align="right">$bea_materai</td>
                    </tr>
					<!------04 Februari 2020 gagat------->
                    <tr>
                        <td colspan="7"></td>
