...
/*PTP*/
    $query = "select fc_nm_perusahaan('$notanya','STE') NM_PERUSAHAAN from dual";
    // echo $query; die();
    $rq    = $db->query($query)->fetchRow();
    $nm_perusahaan = $rq['NM_PERUSAHAAN'];
/*PTP*/
$query="INSERT INTO LOG_NOTA (
								ID_NOTA,NO_REQUEST,
								KODE_MODUL,KETERANGAN,
								ID_USER, TANGGAL
							 )
					VALUES (
								'$notanya','$p01',
...
//print_r($p1);die;
$query="SELECT
				A.*,B.*, SYSDATE AS TGL_PROSES,
				UPPER(B.USER_REQ) AS NAMA,
		...
		FROM
				NOTA_STACKEXT_H A, REQ_STACKEXT_H B
		WHERE
				A.ID_NOTA='$p1'
				AND A.ID_REQUEST=B.ID_REQ";
//echo $query;
//die();
...
$query_dtl="SELECT
			...
			FROM
					NOTA_STACKEXT_D A, NOTA_STACKEXT_H B, MASTER_BARANG C
			WHERE
					A.ID_NOTA='$p1'
...
for($pg=1; $pg<=$jum_page; $pg++) {
	...
	$tbl = <<<EOD
			<table>
                <tr>
                    <td COLSPAN="14" align="left"><b> PT. PELABUHAN INDONESIA II - CABANG PONTIANAK</b></td>
                </tr>
                <tr>
                ...
                <tr>
                    <td COLSPAN="6"></td>
                    <td COLSPAN="8" align="right"><font size="11"><b>PRANOTA PERPANJANGAN EXPORT</b></font></td>
