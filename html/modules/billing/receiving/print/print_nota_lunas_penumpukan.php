...
$query="SELECT ID_PROFORMA, TO_CHAR(TGL_REQ,'dd-mm-yyyy') TGL_REQUEST FROM nota_receiving_h_pen WHERE TRIM(ID_REQ) = TRIM('$no_req') AND STATUS <> 'X'";
...
$query="INSERT INTO LOG_NOTA (ID_NOTA,NO_REQUEST,KODE_MODUL,KETERANGAN,ID_USER) VALUES ('$notanya','$no_req','K','CETAK','$id_user')";
		...
		$db->query($query);
$query="UPDATE NOTA_RECEIVING_H_PEN SET TGL_CETAK=SYSDATE WHERE ID_PROFORMA='$notanya'";
$db->query($query);
$query="SELECT 
				b.PELABUHAN_TUJUAN, 
				b.IPOD, 
				...
				a.ID_USER, TO_CHAR((select c.SUB_TOTAL 
		from nota_receiving_d c where c.ID_PROFORMA=a.ID_PROFORMA AND c.KETERANGAN='ADM' and id_req = '$no_req'),'999,999,999,999') ADM_NOTA, a.EMKL, a.ALAMAT, a.NPWP,TO_CHAR(a.TAGIHAN,'999,999,999,999') TAGIHAN, TO_CHAR(a.PPN,'999,999,999,999') PPN, TO_CHAR(a.TOTAL,'999,999,999,999') TOTAL, a.STATUS, TO_CHAR(a.TGL_REQ,'dd/mm/yyyy') TGL_REQUEST 
        FROM nota_receiving_h_pen a, req_receiving_h b
...
$qcont = "SELECT NO_CONTAINER,STATUS_CONT,SIZE_CONT,TYPE_CONT FROM REQ_RECEIVING_D WHERE ID_REQ = '$no_req'";
    $rcont = $db->query($qcont)->getAll();
    $listcont = "<br/>Daftar Container<br/><b>";
    foreach($rcont as $rc){
        $listcont .= $rc[NO_CONTAINER]."+".$rc[SIZE_CONT]."-".$rc[TYPE_CONT]."-".$rc[STATUS_CONT]." ";
    }
    $listcont.="</b>";

/**gagat start modify amount materai 10 oktober 2019 **/
$query_materai="SELECT TRIM ( TO_CHAR (SUB_TOTAL, '999,999,999,999')) AS SUB_TOTAL FROM NOTA_RECEIVING_D WHERE ID_REQ = '$no_req' AND KETERANGAN='MATERAI' ";
$hasil_materai=$db->query($query_materai);
$hasil_materai_=$hasil_materai->fetchRow();
$bea_materai=$hasil_materai_['SUB_TOTAL']; 
/**gagat end modify amount materai 10 oktober 2019 **/

$query_dtl="SELECT a.KETERANGAN, a.TGL_START_STACK, a.TGL_END_STACK, ROUND(a.JUMLAH_HARI) AS JUMLAH_HARI, a.JUMLAH_CONT, b.UKURAN SIZE_, b.TYPE, b.STATUS, a.HZ, TO_CHAR(a.TARIF,'999,999,999,999') TARIF , TO_CHAR(a.SUB_TOTAL,'999,999,999,999') SUB_TOTAL FROM nota_receiving_d a, master_barang b WHERE a.ID_CONT = b.KODE_BARANG(+) AND a.ID_PROFORMA = '$notanya' AND a.KETERANGAN<>'ADM'";
...
for($pg=1; $pg<=$jum_page; $pg++) {
	...
	$tbl = <<<EOD
		...
		<tr>                    
                ...
                <tr>
                    <td COLSPAN="6"><b>RECEIVING - PENUMPUKAN</b></td>                                        
                </tr>                
