<?php
	...
	$detail_nota  = " SELECT TO_CHAR (SUM(a.TARIF), '999,999,999,999') AS TARIF,
							   TO_CHAR (SUM(a.SUB_TOTAL), '999,999,999,999') AS BIAYA,
							   TXT_VALUE KETERANGAN,
							   a.HZ,
						 ...
						 WHERE a.ID_REQ = '$no_req' AND a.KETERANGAN NOT IN ('ADM','MATERAI') /**18 Oktober 2019 fauzan modify and KETERANGAN NOT IN ('MATERAI')**/
						 GROUP BY TXT_VALUE,HZ,JUMLAH_CONT,TGL_START_STACK,TGL_END_STACK,UKURAN,TYPE,STATUS,JUMLAH_HARI";
	//echo $detail_nota ;die;		   
	$result       = $db->query($detail_nota);
	$row_detail   = $result->getAll();
	
	
	$total_		  = "SELECT SUM(SUB_TOTAL) TOTAL FROM nota_delivery_d_tmp WHERE ID_REQ = '$no_req' AND KETERANGAN NOT IN ('MATERAI')";
	//echo $total_ ;die;
...
/**fauzan modify 17 oktober 2019 add materai */
	$query_mtr		= "SELECT SUB_TOTAL AS MATERAI FROM NOTA_DELIVERY_D_TMP WHERE id_req = '$no_req' 
	and keterangan='MATERAI'";
	$result_mtr		= $db->query($query_mtr);
	$row_mtr		= $result_mtr->fetchRow();
	if($row_mtr['MATERAI']>0){
