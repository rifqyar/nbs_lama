 ...
 $db 	 = getDB("storage");
	   ...
	   CONCAT(TERBILANG(a.TOTAL_TAGIHAN),'rupiah') TERBILANG, m.NAMA_LENGKAP, CASE WHEN TRUNC(TGL_NOTA) < TO_DATE('1/6/2013','DD/MM/RRRR')
		...
		ELSE A.NO_FAKTUR END NO_FAKTUR_
                            FROM NOTA_PNKN_DEL a, request_delivery c, master_user m where
...
class MYPDF extends TCPDF {
	...
	public function Header() {
		...
		$this->SetFont('', 'B', 12);
		$this->SetY(36);
		$this->Cell(0, 15, 'NOTA PENUMPUKAN DELIVERY', 0, false, 'C', 0, '', 0, false, 'M', 'M');
		$this->SetFont('', '', 12);
		$this->SetY(42);
		$this->SetX(5);
	...
	public function Footer() {
		...
		// $this->SetY(-16);
		// $this->SetX(50);
		// $this->Cell(0, 0, 'NOTA PNKN DELIVERY' , 0, false, '', 0, '', 0, false, 'T', 'M');
		// $this->SetFont('Courier', 'B', 11);
		// $this->SetY(-12);
...
EOD;
        ...
        $query_get2	= "SELECT TO_CHAR(a.START_STACK,'dd/mm/yyyy') START_STACK,TO_CHAR(a.END_STACK,'dd/mm/yyyy') END_STACK, 
						...
						AND c.TGL_NOTA = (SELECT MAX(d.TGL_NOTA) 
											...
											WHERE d.NO_REQUEST = '$no_req') and a.KETERANGAN <> 'ADMIN NOTA' ";
