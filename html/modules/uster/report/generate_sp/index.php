...
jQuery(function() {
 jQuery("#l_pay").jqGrid({
	...
	mtype : "post",
	datatype: "json",
	colNames:['No. Request','No. Nota','No. Faktur','Total','Customer','No. SP'], 
	colModel:[
		{name:'nt', width:120, align:"center"},
		{name:'nf', width:120, align:"center"},
		{name:'nr',index:'nr', width:150, align:"center"},
		{name:'em',index:'em', width:100, align:"left"},
		{name:'aem',index:'aem', width:250, align:"left"},
		{name:'vv',index:'vv', width:150, align:"center"}
	],
	rowNum:20,
	width: 865,
	height: "100%",
...
function search_payment(){
    ...
    $("#l_pay").jqGrid('setGridParam',{url:"<?=HOME?><?=APPID?>/data?idreq="+id_req+"&idtime="+id_time, datatype:"json"}).trigger("reloadGrid");
}

function generate_sp(){
		var id_req 	= $("#id_req").val();
		var id_time = $("#id_time").val();
		var url="<?=HOME?><?=APPID?>.ajax/save_generate";
		
		if(id_time == null || id_time == "")
		{
		...
		{	
			$.blockUI({ message: '<h1><br>Please wait... </h1><br><img src=images/loadingbox.gif /><br><br>' });
			$.post( url, {id_req : id_req, id_time : id_time},function(data){
			alert(data);
			$.unblockUI({
				onUnblock: function(){ }
			});
...
function toexcel() {
	var id_req = $("#id_req").val();
    var id_time = $("#id_time").val();
	var url 	= "<?=HOME?><?=APPID?>.toexcel2/toexcel2?id_time="+id_time+"&id_req="+id_req;
...
<div class="content">
	...
	<table border="0">
		...
		<tr>
			<td>Tanggal Awal</td>
			<td>:</td>
			<td><input type="text" id="id_time" name="id_time"/> <input type="button" onclick="search_payment()" value="Cari" /></td>
		</tr>
		<tr>
			<td><input type="button" onclick="generate_sp()" value="Generate" /> <input type="button" onclick="toexcel()" value="Export to Excel" /></td>
			<td></td>
			<td></td>
		</tr>
	</table>
