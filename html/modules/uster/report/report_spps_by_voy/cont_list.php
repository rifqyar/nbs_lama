<?php
	...
	if($kegiatan == "stripping"){
      ...
      $bp_id_ = $rwbp["NO_BOOKING"];
			/*$query = "select * from (select mc.no_container, mc.size_, cs.no_request, rs.tgl_request, mu1.nama_lengkap     , vm.nm_pbm, cs.commodity,
            ...
            left join history_container hc on hc.no_container = cs.no_container and hc.no_request = cs.no_request and kegiatan = 'REALISASI STRIPPING' 
            left join master_user mu1 on rs.id_user = mu1.id
            left join master_user mu2 on cs.id_user_realisasi = mu2.id
            left join  petikemas_cabang.ttd_bp_cont b on b.cont_no_bp = mc.no_container
      ...
      $bp_id_ = $rwbp["NO_BOOKING"];
			/*$query = "select q.*, bg.tgl_in tgl_gate from (select  cs.no_container, mc.size_, rs.no_request, rs.tgl_request, mu1.nama_lengkap nm_request , vm.nm_pbm, 
                        ...
                        union all
                        select c.no_container, mc.size_, r.no_request, r.tgl_request, mu.nama_lengkap nm_request , ns.emkl, c.start_stack tgl_awal, c.start_perp_pnkn tgl_akhir, ns.lunas, 
                        c.tgl_realisasi, mu1.nama_lengkap nm_realisasi , c.commodity, '' no_request_delivery, case when c.type_stuffing = 'GUD_TONGKANG' then 'Gudang Lapangan'
                                                                                                                        ...
                                                                                                                        end as STATUS,to_date('') tgl_gate
