<?php
	...
	//$cont_ungate = array();
	//$row_ungate = array();
	/*function get_detail($no_req_ungate){
			//foreach($row_list as $rows){
				//if($rows["ASAL"] == "TPK"){
					//$no_req_ungate = $rows["NO_REQUEST"];
					$query_cek_ = "SELECT * FROM (
								...
								WHERE NO_REQUEST LIKE '$no_req_ungate'";
					$res_ungate = $db->query($query_cek_);
					$row_ungate = $res_ungate->getAll();				
					foreach($row_ungate as $rowu){
						$cont_ungate[] = array(
												'NO_REQUEST' => $rowu["NO_REQUEST"],
												'NO_CONTAINER' => $rowu["NO_CONTAINER"]
						);
					}			
				//}
			//}
...
$html = '
<br><br>
<p style="font-size:40px;font-weight:bold"> Nota Per Periodik </p>
<hr>
<div>
	<table border="1">
    	<tr>
