...
if($asalcontstuff == "TPK"){
	...
	$cont_plan 		= "SELECT DISTINCT PLAN_CONTAINER_STUFFING.NO_SEAL,PLAN_CONTAINER_STUFFING.BERAT,PLAN_CONTAINER_STUFFING.HZ,PLAN_CONTAINER_STUFFING.DEPO_TUJUAN, PLAN_CONTAINER_STUFFING.COMMODITY, A.KD_SIZE, A.KD_TYPE
...
// ===========================================================  END SIMOP TPK==============================================================================//
	...
	// $query 		= "UPDATE PLAN_CONTAINER_STUFFING SET TGL_APPROVE = TO_DATE('$tgl_approve','dd-mm-rrrr')
	...
	$query_r 	= "SELECT 	NO_REQUEST,
							...
							ID_USER,NO_REQUEST_DELIVERY, KD_CONSIGNEE, 	KD_PENUMPUKAN_OLEH, NM_KAPAL,NO_PEB, NO_NPE, VOYAGE 
	...
	$query_c 	= "SELECT DISTINCT PLAN_CONTAINER_STUFFING.*, 
								   TO_CHAR(PLAN_CONTAINER_STUFFING.START_STACK,'dd-mm-yyyy') STACK,
								   PLAN_CONTAINER_STUFFING.COMMODITY COMMO, A.KD_SIZE, A.KD_TYPE
					   FROM PLAN_CONTAINER_STUFFING INNER JOIN PETIKEMAS_CABANG.TTD_BP_CONT A			
					   ON PLAN_CONTAINER_STUFFING.NO_CONTAINER = A.CONT_NO_BP
					   WHERE PLAN_CONTAINER_STUFFING.NO_REQUEST = '$no_req' AND PLAN_CONTAINER_STUFFING.NO_CONTAINER = '$no_cont'";
	...
	if(count($row_cek_request) > 0 && count($row_cek_cont) > 0){ //jika request telah ada dan container telah ada
		...
		foreach($row_c as $rc){
			...
			$query_ic	= "INSERT INTO CONTAINER_STUFFING (NO_CONTAINER, NO_REQUEST, AKTIF, HZ, COMMODITY, TYPE_STUFFING, START_STACK,
							ASAL_CONT, NO_SEAL, BERAT, KETERANGAN, STATUS_REQ, TGL_APPROVE, TGL_GATE, TGL_MULAI_FULL, TGL_SELESAI_FULL)
							VALUES('$cont',
							'$no_req_stuf',
							'$aktif',
							'$hz',
	...
	$history_del        = "INSERT INTO history_container(NO_CONTAINER, NO_REQUEST, KEGIATAN, TGL_UPDATE, ID_USER, ID_YARD,STATUS_CONT, NO_BOOKING, COUNTER) 
														  VALUES ('$no_cont','$no_req_stuf','REQUEST STUFFING',SYSDATE,'$id_user', '$id_yard','MTY', '$cur_booking3', '$cur_counter3')";
...
$no_req_stuf = str_replace('P', 'S', $no_req);
	...
	$history_stuf        = "INSERT INTO history_container(NO_CONTAINER, NO_REQUEST, KEGIATAN, TGL_UPDATE, ID_USER,STATUS_CONT, NO_BOOKING, COUNTER) 
														  VALUES ('$no_cont','$no_req_stuf','REQUEST STUFFING',SYSDATE,'$id_user','MTY', '$cur_booking4', '$cur_counter4')";
