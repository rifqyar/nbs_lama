...
function create_req()
{
	var url="<?=HOME;?>uster.new_request.batal_stuffing/save_req";
	...
	else
	{
		...
		function(data){
			if(data == 'sukses'){
				$.unblockUI({ 
				onUnblock: function(){ alert('Batal Stuffing Sukses'); } 
				});
				window.location="<?=HOME?>uster.new_request.batal_stuffing/";
			}else {
				$.unblockUI({ 
				onUnblock: function(){ alert('Batal Stuffing Gagal '); } 
				});
			}
		});
	}
...
$(document).ready(function()
{
	...
	$( "#nc" ).autocomplete({
		minLength: 6,
		source: "uster.new_request.batal_stuffing/container",
