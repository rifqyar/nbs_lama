...
if($asalcontstuff == "TPK"){
		...
		$sql = "SELECT NO_REQ_DEL FROM PETIKEMAS_CABANG.TTM_DEL_REQ WHERE NO_REQ_DEL = '$no_req_ict'";
			...
			//utang are_id,are_id2,temp,class			
				$sqldet   = "INSERT INTO PETIKEMAS_CABANG.TTD_CONT_EXBSPL(KD_PMB_DTL,KD_PMB,NO_CONTAINER,KD_SIZE,KD_TYPE_CONT,KD_JENIS_PEMILIK,KD_COMMODITY,GROSS,NO_SEAL,KETERANGAN,STATUS_PMB_DTL,STATUS_KARTU,USER_ID,VIA,HZ,STATUS_PP,ARE_ID,ARE_ID2) 
					...
					// ===========================================  END SIMOP ==============================================//
	$history_rec        = "INSERT INTO history_container(NO_CONTAINER, NO_REQUEST, KEGIATAN, TGL_UPDATE, ID_USER) 
														  VALUES ('$no_cont','$no_req_rec','REQUEST RECEIVING',SYSDATE,'$id_user')";
	...
	$query_cek_request = "SELECT * FROM REQUEST_STUFFING WHERE NO_REQUEST = '$no_req'";
	$result_cek_request = $db->query($query_cek_request);
	$row_cek_request	= $result_cek_request->getAll();

	$query_cek_cont = "SELECT * FROM CONTAINER_STUFFING WHERE NO_REQUEST = '$no_req' AND NO_CONTAINER = '$no_cont'";
	...
	if(count($row_cek_request) > 0 && count($row_cek_cont) > 0){ //jika request telah ada dan container telah ada
		...
		foreach($row_c as $rc){
			...
			$query_ic	= "INSERT INTO CONTAINER_STUFFING 
							VALUES('$cont',
							'$req',
							'$aktif',
							'$hz',
							...
							'',
							'',
						TO_DATE('$start_stack','dd-mm-yy'),
							'$asal',
							'$seal',
							'$berat',
							'$keterangan',
						TO_DATE('$tgl_mulai','yy-mm-dd'),
						'$stat_req',
	...
	$history_del        = "INSERT INTO history_container(NO_CONTAINER, NO_REQUEST, KEGIATAN, TGL_UPDATE, ID_USER) 
														  VALUES ('$no_cont','$no_req','REQUEST STUFFING',SYSDATE,'$id_user')";
