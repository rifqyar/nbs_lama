...
//--------------------------------------------------- End of Interface Ke ICT----------------------------------------------------------------------------------------------
	...
	if(count($row_cek_request) > 0 && count($row_cek_cont) > 0){
		...
		$query_ir = "INSERT INTO REQUEST_STRIPPING(ID_YARD,  KETERANGAN, TGL_REQUEST,
					...
					ID_USER, KD_CONSIGNEE, KD_PENUMPUKAN_OLEH, NO_REQUEST)
						VALUES(
						'$id_yard',
						'$keterangan',
						'$tgl_req',
	...
	$history_stripp        = "INSERT INTO history_container(NO_CONTAINER, NO_REQUEST, KEGIATAN, TGL_UPDATE, ID_USER) 
														  VALUES ('$no_cont','$no_req','REQUEST STRIPPING',SYSDATE,'$id_user')";
