   ...
   //    debug($_POST);
     //  echo count($_POST['select2']);die;
       ...
       // echo "SELECT TO_CHAR(TGL_REQUEST_DELIVERY+1,'dd/mm/yyyy') TGL_END, REQ_AWAL, NVL((PERP_KE+1),1) PERP_KE, ID_YARD, DELIVERY_KE, request_delivery.VESSEL, request_delivery.VOYAGE FROM REQUEST_DELIVERY WHERE NO_REQUEST = '$no_req'";die;
        $query  	= "SELECT TO_CHAR(TGL_APPROVE+3, 'dd/mm/yyyy') START_PERP, KD_CONSIGNEE, NVL((PERP_KE+1),1) PERP_KE, ID_YARD FROM REQUEST_STRIPPING WHERE NO_REQUEST = '$no_req'";
	...
	$query_cek	= "SELECT LPAD((COUNT(1)+1),6,0) AS JUM, TO_CHAR(SYSDATE, 'MM') AS MONTH, TO_CHAR(SYSDATE, 'YY') AS YEAR FROM request_stripping WHERE TGL_REQUEST BETWEEN TRUNC(SYSDATE,'MONTH') AND LAST_DAY(SYSDATE)";
...
//	echo "INSERT INTO request_delivery(NO_REQUEST, KD_EMKL, TGL_REQUEST, REQ_AWAL, TGL_REQUEST_DELIVERY, KETERANGAN, CETAK_KARTU, ID_USER, DELIVERY_KE,VESSEL, VOYAGE, PERALIHAN, ID_YARD, STATUS, PERP_KE, PERP_DARI) 
  //                                                    VALUES ('$no_req_dev', '$id_emkl', SYSDATE, '$req_awal',TO_DATE('".$tgl_perp."','yyyy/mm/dd'),'$KETERANGAN', '0', $id_user, '$dev_ke','$id_vessel','$id_voyage','T','$id_yard','EXT','$perp_ke','$no_req')";die;
        $query_req 	= "INSERT INTO request_stripping(NO_REQUEST, KD_EMKL, TGL_REQUEST, REQ_AWAL, TGL_REQUEST_DELIVERY, KETERANGAN, CETAK_KARTU, ID_USER, DELIVERY_KE,VESSEL, VOYAGE, PERALIHAN, ID_YARD, STATUS, PERP_KE, PERP_DARI) 
                                                      VALUES ('$no_req_dev', '$id_emkl', SYSDATE, '$req_awal',TO_DATE('".$tgl_perp."','yyyy/mm/dd'),'$KETERANGAN', '0', $id_user, '$dev_ke','$id_vessel','$id_voyage','T','$id_yard','EXT','$perp_ke','$no_req')";
        //echo $query_req;die;
        $db->query($query_req);
        $query  	= "SELECT MAX(NO_REQUEST) NO_REQUEST FROM request_stripping";
		$result_cek	= $db->query($query);
        $cek		= $result_cek->fetchRow();
       ...
       // echo 'before';die;
        foreach($data_sp2 as $item){
            //echo 'dama';die;
            $query  	= "SELECT a.STATUS, a.HZ, a.START_STACK FROM container_stripping a WHERE  a.NO_CONTAINER = '$item' AND NO_REQUEST = '$no_req'";
            $result_cek	= $db->query($query);
            $cek	= $result_cek->fetchRow();
            $status	= $cek["STATUS"];
            $hz 	= $cek["HZ"];
