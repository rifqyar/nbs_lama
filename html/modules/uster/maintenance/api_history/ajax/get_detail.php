<?php
	...
	if($act == "handling"){
		...
		$q_placement = "SELECT CEK.*,  CASE WHEN CEK.INSERT_VIA = 'H' THEN 'HANDHELD' ELSE 'DESKTOP' END VIA FROM (SELECT HISTORY_PLACEMENT.*, MASTER_USER.NAME NAMA_LENGKAP, BLOCKING_AREA.NAME, YARD_AREA.NAMA_YARD FROM 
                        ...
                        LEFT JOIN BILLING.TB_USER MASTER_USER ON (HISTORY_PLACEMENT.NIPP_USER = TO_CHAR(MASTER_USER.ID) OR HISTORY_PLACEMENT.NIPP_USER = MASTER_USER.NIPP) WHERE NO_CONTAINER = '$no_cont' AND NO_REQUEST = '$no_req_pl'
                        ...
                        ON HISTORY_PLACEMENT.ID_BLOCKING_AREA = BLOCKING_AREA.ID
                        LEFT JOIN YARD_AREA ON BLOCKING_AREA.ID_YARD_AREA = YARD_AREA.ID
                        LEFT JOIN BILLING.TB_USER MASTER_USER ON HISTORY_PLACEMENT.NIPP_USER = MASTER_USER.ID WHERE NO_CONTAINER = '$no_cont' AND NO_REQUEST = '$no_req_pl'
