<?php
	...
	if($kegiatan == $rw_cek_op[KEGIATAN]){
		if ($kegiatan == 'REALISASI STRIPPING') {
			...
			$db->query("UPDATE CONTAINER_STRIPPING SET AKTIF = 'Y', TGL_REALISASI = '', ID_USER_REALISASI = '' WHERE NO_CONTAINER = '$no_cont' AND NO_REQUEST = '$no_req'");
			...
			$db->query("DELETE FROM HISTORY_CONTAINER WHERE NO_CONTAINER = '$no_cont' AND NO_REQUEST = '$no_req' AND NO_BOOKING = '$no_booking' AND KEGIATAN = '$kegiatan'");
			$db->query("DELETE FROM GATE_IN WHERE NO_CONTAINER = '$no_cont' AND NO_REQUEST = '$no_req'");
