...
if (  ($no_nota_cek != NULL && $nota_status == 'BATAL') || ($no_nota_cek == NULL && $nota_status == NULL)){	
	...
	$row_pass     = $result__->fetchRow();
       ...
       // $adm   = 10000;
        $total_ = $total;
        $ppn   = $total_ppn;
        $tagihan = $total_tagihan;// + $tarif_pass;
	...
	if($koreksi <> 'Y'){
		...
		$faktur		= "SELECT concat(CONCAT(a.no_faktur, b.kode_faktur), s.faktur) FAKTUR, s.SEQ FROM  (SELECT (LPAD (NVL ( (MAX (SEQ_FAKTUR) + 1), 1), 6, 0))
                                ...
                                FROM  NOTA_ALL_H
                                      ...
                                      (SELECT KODE_FAKTUR FROM MASTER_NOTA WHERE ID_NOTA = 11) b";
		...
		$update = "UPDATE NOTA_PNKN_DEL SET STATUS = 'BATAL' WHERE NO_NOTA = '$nota_lama_'";
		$db->query($update);
		// $db4->query($update);
	...
	if($db->query($query_insert_nota))//(TRUE)
	{
		...
		if ($rtpk_flag[FLAG] == 'N') {
			$db->query($query_nota_all);
              ...
              //  echo $query_detail;die;
                ...
                if ($rtpk_flag[FLAG] == 'Y') {
							$sql_xpi = "BEGIN
							  USTER.ITPK_POPULATE_STAGING.INSERT_NOTA_ITPK ( '$no_req', 'UST11', '$faktur', '$no_nota', '$koreksi', '$nipp' ); 
							END; ";
              				 // echo $sql_xpi;die;
							$db->query($sql_xpi);
				} 
		...
		header('Location:'.HOME.APPID.'.print/print_nota_lunas_pnkn?no_nota='.$no_nota."&no_req=".$no_req);	
	}	
