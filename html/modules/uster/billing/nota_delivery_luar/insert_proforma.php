...
if (  ($no_nota_cek != NULL && $nota_status == 'BATAL') || ($no_nota_cek == NULL && $nota_status == NULL)){
	...
	$row_pass     = $result__->fetchRow();
       $tarif_pass   = $row_pass['TARIF'];
        ...
        $total_		= "SELECT SUM(BIAYA) TOTAL, SUM(PPN) PPN, (SUM(BIAYA) + SUM(PPN)) TOTAL_TAGIHAN FROM temp_detail_nota WHERE no_request = '$no_req' AND KETERANGAN NOT IN ('MATERAI')"; /**Fauzan modif 26 AUG 2020 "AND KETERANGAN NOT IN ('MATERAI')"*/
        //echo $total_;die;
       ...
       // $adm   = 10000;
        $total_ = $total;
        $ppn   = round($total_ppn);
        $tagihan = round($total_tagihan) + $materai; /**Fauzan modif 26 AUG 2020 "+ $materai"*/
