 ...
 */
		...
		$result3	= $db->query($no_per);
		$nom	= $result3->fetchRow();
...
class MYPDF extends TCPDF {
	...
	public function Header() {
		...
		$this->SetFont('', 'B', 12);
		$this->SetY(36);
		$this->Cell(0, 15, 'LAMPIRAN NOTA RELOKASI MTY', 0, false, 'C', 0, '', 0, false, 'M', 'M');
		$this->SetFont('', '', 12);
		$this->SetY(42);
		$this->SetX(5);
	...
	$row_detail	= $result->getAll();
        for($i=0;$i<count($row_detail);$i++){
                $tbl1 .=<<< EOD
                        <table>                        
                        <tr>
                        <td colspan="3" width="200">{$row_detail[$i]['KETERANGAN']} (Dari Stripping Area Ke Empty Area)</td>
