...
global $no_request, $dt, $no_nota, $no_nota_, $no_do,$no_nota_mti,$no_faktur_mti;
$no_req = $_GET['no_req'];
...
$_SESSION["PRINTED_BY"] = $nm_lengkap;
...
class MYPDF extends TCPDF
{
    ...
    public function Header()
    {
        ...
        $this->SetY(12);
        $this->SetX(184);
        $this->Cell(0, 16,$no_nota_mti, 0, false, 'L', 0, '', 0, false, 'T', 'M');
        $this->SetFont('', 'L', 8);
        $this->SetY(16);
        $this->SetX(184);
        $this->Cell(0, 16,$no_faktur_mti, 0, false, 'L', 0, '', 0, false, 'T', 'M');
        $this->SetFont('', 'L', 8);
        $this->SetY(20);
        $this->SetX(184);
        ...
        $this->SetFont('', 'B', 12);
        $this->SetY(36);
        $this->Cell(0, 15, 'LAMPIRAN NOTA STUFFING', 0, false, 'C', 0, '', 0, false, 'M', 'M');
        $this->SetFont('', '', 12);
        $this->SetY(42);
        $this->SetX(5);
        ...
        // $this->SetFont('', 'B', 12);
        // $this->SetY(36);
        // $this->Cell(0, 15, 'NOTA STUFFING', 0, false, 'C', 0, '', 0, false, 'M', 'M');
        // $this->SetFont('', 'B', 12);
        // $this->SetY(42);
        // $this->SetX(5);
        // $this->Cell(0, 15, $_SESSION["emkl"], 0, false, '', 0, '', 0, false, 'T', 'M');
    ...
    public function Footer()
    {
        ...
        // $this->SetY(-16);
        // $this->SetX(50);
        // $this->Cell(0, 0, 'NOTA STUFFING', 0, false, '', 0, '', 0, false, 'T', 'M');
        // $this->SetFont('Courier', 'B', 11);
        // $this->SetY(-12);
...
/* $query_get2 = "select partall.jml_hari, TO_CHAR(partall.biaya/partall.jml_cont,'999,999,999,999')  tarif,  TO_CHAR(partall.biaya,'999,999,999,999') biaya, partall.keterangan, partall.hz, 
...
--                             SUM(a.JML_CONT),
                    ...
                    (SELECT a.tekstual,   case a.tekstual
                                  when 'PAKET STUFF LAPANGAN' THEN (select count(*) from container_stuffing where no_request = '$no_request' and type_stuffing = 'STUFFING_LAP')
                                  when 'PAKET STUFF GUDANG EKS TONGKANG' THEN (select count(*) from container_stuffing where no_request = '$no_request' and type_stuffing = 'STUFFING_GUD_TONGKANG')
                                  when 'PAKET STUFF GUDANG EKS TRUCK' THEN (select count(*) from container_stuffing where no_request = '$no_request' and type_stuffing = 'STUFFING_GUD_TRUCK')
                             ELSE 0
                              END AS jml_cont FROM nota_stuffing_d a WHERE no_nota = '$no_nota' 
                              and a.tekstual in ('PAKET STUFF LAPANGAN','PAKET STUFF GUDANG EKS TONGKANG','PAKET STUFF GUDANG EKS TRUCK')
                   group by a.tekstual) partwo
                   where partone.keterangan = partwo.tekstual) partall
                    UNION ALL                                                             
                    select jml_hari, TO_CHAR(TARIF,'999,999,999,999') TARIF, TO_CHAR(BIAYA,'999,999,999,999') BIAYA, keterangan, hz, start_stack, end_stack, size_, type_, status, urut,  jumlah_cont jml_cont
                    from nota_stuffing_d, iso_code
                    where nota_stuffing_d.id_iso = iso_code.id_iso
                    and no_nota = '$no_nota'
...
for ($i = 0; $i < count($row_detail); $i++) {
    $tbl1 .= <<< EOD
                       <tr>
                        ...
                        <td width="25" align="center">{$row_detail[$i]['JUMLAH_CONT']}</td>    
