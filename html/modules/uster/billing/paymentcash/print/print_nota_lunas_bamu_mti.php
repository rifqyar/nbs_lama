 ...
 */
		...
		$db 	 = getDB("storage");
		global $no_request, $dt,$no_faktur_mti,$no_nota_mti;
        $no_req = $_GET['no_req'];
		...
		//untuk kontainer
		// $qcont = "SELECT A.NO_CONTAINER,B.SIZE_,B.TYPE_ FROM CONTAINER_BATAL_MUAT A, MASTER_CONTAINER B WHERE A.NO_CONTAINER = B.NO_CONTAINER AND A.NO_REQUEST = '$no_req'";
		...
		$_SESSION["FAKTUR_MTI"] = $row_nota['NO_FAKTUR'];
...
class MYPDF extends TCPDF {
	...
	public function Header() {
		// Logo
		global $no_request,$no_nota_mti,$no_faktur_mti;
		global $dt;
		...
		$this->SetFont('', 'B', 12);
		$this->SetY(36);
		$this->Cell(0, 15, 'LAMPIRAN NOTA BATAL MUAT', 0, false, 'C', 0, '', 0, false, 'M', 'M');
		$this->SetFont('', '', 12);
		$this->SetY(42);
		$this->SetX(5);
...
//        
        ...
        $tbl = <<<EOD
				...
				<tr>
                    ...
                    <td></td>
                    <td colspan="4"></td>
                    <td width="150" align="left">$teks_baru </td>
                    <td width="10">:</td>
                    <td width="100">$no_req_baru</td>
                </tr>
...
/*end hitung materai Fauzan 23 September 2020*/
	...
	, iso_code b WHERE a.id_iso = b.id_iso and a.ID_NOTA = '$no_nota_' AND A.KETERANGAN NOT IN ('ADMIN NOTA', 'MATERAI')"; /*modify fauzan 23 September 2020*/
	...
	$row_detail	= $result->getAll();
        for($i=0;$i<count($row_detail);$i++){
                $tbl1 .=<<< EOD
                        ...
                        <td width="30" align="center">{$row_detail[$i]['JML_CONT']}</td>    
