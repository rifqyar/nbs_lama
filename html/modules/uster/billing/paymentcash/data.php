...
$id_group = $_SESSION["ID_GROUP"];
	...
	$i=0;
		...
		$query="SELECT * FROM (SELECT * FROM (
			SELECT NO_NOTA, NO_FAKTUR, NO_REQUEST, NO_NOTA_MTI, NO_FAKTUR_MTI, EMKL, 'RECEIVING' KEGIATAN, TGL_NOTA_1,TOTAL_TAGIHAN, STATUS,KD_EMKL FROM nota_receiving WHERE STATUS <> 'BATAL'
			UNION
			SELECT NO_NOTA, NO_FAKTUR, NO_REQUEST, NO_NOTA_MTI, NO_FAKTUR_MTI, EMKL, CASE WHEN STATUS = 'PERP' THEN 'PERP_PNK' ELSE 'STUFFING' END KEGIATAN, TGL_NOTA_1,TOTAL_TAGIHAN,STATUS,KD_EMKL FROM nota_stuffing WHERE  STATUS <> 'BATAL'
			UNION 
			SELECT NO_NOTA, NO_FAKTUR, NO_REQUEST, NO_NOTA_MTI, NO_FAKTUR_MTI, EMKL,  CASE WHEN SUBSTR(NO_NOTA,0,2) ='03' THEN 'STRIPPING' ELSE 'PERP_STRIP' end KEGIATAN, TGL_NOTA_1,TOTAL_TAGIHAN,STATUS ,KD_EMKL FROM nota_stripping WHERE STATUS <> 'BATAL'
			UNION
			SELECT NO_NOTA, NO_FAKTUR, NO_REQUEST, NO_NOTA_MTI, NO_FAKTUR_MTI, EMKL,  CASE WHEN STATUS = 'PERP' THEN 'PERP_DEV' ELSE 'DELIVERY' END  KEGIATAN, TGL_NOTA_1,TOTAL_TAGIHAN ,STATUS,KD_EMKL FROM nota_delivery WHERE STATUS <> 'BATAL'
			UNION
			SELECT NO_NOTA, NO_FAKTUR, NO_REQUEST, NO_NOTA_MTI, NO_FAKTUR_MTI, EMKL,  'RELOKASI' KEGIATAN, TGL_NOTA_1,TOTAL_TAGIHAN ,STATUS,KD_EMKL FROM nota_relokasi WHERE STATUS <> 'BATAL'
	...
	while ($row = $res->fetchRow()) 
	{
			...
			//Menampilkan NO_FAKTUR_MTI 
			$responce->rows[$i]['cell']=array($act,$row[NO_NOTA_MTI],$row[NO_FAKTUR_MTI],$row[NO_REQUEST],$row[EMKL],$row[KEGIATAN],$row[TGL_NOTA_1],number_format($row[TOTAL_TAGIHAN]));
