...
//echo $query_nota;die;
	...
	WHERE a.id_iso = b.id_iso and a.no_request = '$no_req' AND a.KETERANGAN NOT IN ('ADMIN NOTA','MATERAI')";/*gagat modif 09 feb 2020*/

	$result       = $db->query($detail_nota);
	$row_detail   = $result->getAll();


	$total_		  = "SELECT SUM(BIAYA) TOTAL, SUM(PPN) PPN, (SUM(BIAYA) + SUM(PPN)) TOTAL_TAGIHAN FROM temp_detail_nota WHERE no_request = '$no_req' AND KETERANGAN NOT IN ('MATERAI')"; /**modify Fauzan 28 Agustus 2020 'AND KETERANGAN NOT IN ('MATERAI')'*/
	...
	//Menghitung Bea Materai Fauzan modif 24 Agustus 2020 
	...
	$total_bayar        = $total_tagihan + $bea_materai; /**modify Fauzan 28 Agustus 2020 '+ $bea_materai'*/
