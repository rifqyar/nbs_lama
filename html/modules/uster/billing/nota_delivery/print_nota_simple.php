...
//$no_nota	= $_GET["no_nota"];
	...
	$query_nota	= "SELECT c.NM_PBM AS EMKL,
                   ...
                   FROM REQUEST_DELIVERY b INNER JOIN
                            V_MST_PBM c
                            ON b.KD_EMKL = c.KD_PBM AND c.KD_CABANG = '05'
...
//echo $query_nota;die;
	...
	$tl->assign('corporate_name','PT. Multi Terminal Idonesia <br> Cabang Pelabuhan Pontianak');
	...
	$total_		  = "SELECT SUM(BIAYA) TOTAL, SUM(PPN) PPN, SUM(BIAYA)+SUM(PPN) TOTAL_TAGIHAN
						...
						WHERE no_request = '$no_req' AND KETERANGAN NOT IN ('MATERAI')"; /**fauzan modif 26 AUG 2020*/
  ...
  //  $total_ = $total + $adm;
	...
	//Menghitung Bea Materai fauzan modif 28 agustus 2020 
	...
	$total_bayar        = $total_bayar + $bea_materai; //+ $tarif_pass;	/**Fauzan modif 26 AUG 2020 "+ $bea_materai"*/
	...
	/*if ($display == 1) {
		if ($koreksi == N) {
		 	$qnota = "select kapal_prod.counter_nota_xp.invoice_num('USTER', 'DELIVERY','05','050', 'NO',$total_bayar, 'IDR',
		            kapal_prod.counter_nota_xp.group_cust('$kd_pbm')) no_nota from dual";
		}
		else {
			$cek_oldpbm = "SELECT KD_EMKL FROM NOTA_DELIVERY WHERE NO_REQUEST = '$no_req' AND STATUS = 'BATAL'";
			$rcek_		= $db->query($cek_oldpbm)->fetchRow();

			// echo $cek_oldpbm;die;

			if ($rcek_[KD_EMKL] == $row_nota[KD_PBM]) {
				$qnota = "select kapal_prod.counter_nota_xp.cm_invoice_num('USTER', 'DELIVERY','05','050', 'NO',$total_bayar, 'IDR',
		            kapal_prod.counter_nota_xp.group_cust('$kd_pbm')) no_nota from dual";
			} else {
				$qnota = "select kapal_prod.counter_nota_xp.invoice_num('USTER', 'DELIVERY','05','050', 'NO',$total_bayar, 'IDR',
		            kapal_prod.counter_nota_xp.group_cust('$kd_pbm')) no_nota from dual";
