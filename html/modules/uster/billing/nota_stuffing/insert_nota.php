		...
		if($rtpk_flag[FLAG] == 'Y'){
			...
			$faktur		= "SELECT concat(CONCAT(a.no_faktur, b.kode_faktur), s.faktur) FAKTUR, s.SEQ FROM  (SELECT (LPAD (NVL ( (MAX (SEQ_FAKTUR) + 1), 1), 6, 0))
	                                ...
	                                FROM  NOTA_ALL_H
	                                      ...
	                                      (SELECT KODE_FAKTUR FROM MASTER_NOTA WHERE ID_NOTA = 3) b";
		...
		if($db->query($query_insert_nota))
		{
			...
			$row		= $res->getAll();
					...
					foreach ($row as $item){
						...
						if ($rtpk_flag[FLAG] == 'Y') {
							$sql_xpi = "BEGIN
							  USTER.ITPK_POPULATE_STAGING.INSERT_NOTA_ITPK ( '$no_req', 'UST04', '$faktur', '$no_nota', '$koreksi', '$nipp' ); 
							END; ";
              				 // echo $sql_xpi;die;
							$db->query($sql_xpi);
						} 
						...
						header('Location:'.HOME.APPID.'.print/print_nota_lunas_simple?no_nota='.$no_nota_cek."&no_req=".$no_req."&first=1");
