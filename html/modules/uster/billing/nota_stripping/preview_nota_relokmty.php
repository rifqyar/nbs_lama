<?php
	...
	if( $st_nota == 'Y'){
		echo 'sudah di cetak';
		header('Location: '.HOME.APPID.'.print/print_proforma_relok?no_req='.$no_req);
	} else {
...
/*echo "DECLARE
                            ...
                            END;";die;*/
		$sql_xpi = "DECLARE id_nota NUMBER; tgl_req DATE; no_request VARCHAR2(100); jenis VARCHAR2 (100); err_msg VARCHAR2(100); BEGIN  id_nota := 2; tgl_req := '$tgl_re'; no_request := '$no_req'; err_msg := 'NULL'; jenis := 'relokasimty'; pack_get_nota_stripping.create_detail_nota(id_nota,tgl_req,no_request,jenis, err_msg); END;";
                //echo $sql_xpi;die;
		$db->query($sql_xpi);
              //  echo $sql_xpi;die;
	...
	$total_		  = "SELECT SUM(BIAYA) TOTAL, SUM(PPN) PPN FROM temp_detail_nota_i WHERE  no_request = '$no_req' AND KETERANGAN NOT IN ('MATERAI')"; /**end modify fauzan 28 aug 2020 "AND KETERANGAN NOT IN ('MATERAI')"*/
	...
	/*if ($display == 1) {
		...
		if ($koreksi == N) {
		 	$qnota = "select kapal_prod.counter_nota_xp.invoice_num('USTER', 'RELOK_MTY','05','050', 'NO',$total_bayar, 'IDR',
		            kapal_prod.counter_nota_xp.group_cust('$kd_pbm')) no_nota from dual";
		}
		else {
			$cek_oldpbm = "SELECT KD_EMKL FROM NOTA_RELOKASI_MTY WHERE NO_REQUEST = '$no_request' AND STATUS = 'BATAL'";
			$rcek_		= $db->query($cek_oldpbm)->fetchRow();

			if ($rcek_[KD_EMKL] == $row_nota[KD_PBM]) {
				$qnota = "select kapal_prod.counter_nota_xp.cm_invoice_num('USTER', 'RELOK_MTY','05','050', 'NO',$total_bayar, 'IDR',
		            kapal_prod.counter_nota_xp.group_cust('$kd_pbm')) no_nota from dual";
			} else {
				$qnota = "select kapal_prod.counter_nota_xp.invoice_num('USTER', 'RELOK_MTY','05','050', 'NO',$total_bayar, 'IDR',
		            kapal_prod.counter_nota_xp.group_cust('$kd_pbm')) no_nota from dual";
	...
	//echo $qnota; die();
	$rnota = $db2->query($qnota)->fetchRow();
	$tl->assign("rnota",$rnota['NO_NOTA']);
	$tl->assign("display",$display);
	}*/

	//debug($no_req);die;
	$tl->assign("url_ins","insert_proforma_relokasimty");
	$tl->assign("kegiatan","RELOKASI MTY");
