<?php
	...
	$query_nota	= "SELECT c.NM_PBM AS EMKL,
                   ...
                   FROM REQUEST_RELOKASI b INNER JOIN
                            V_MST_PBM c ON B.KD_EMKL = c.KD_PBM
	...
	$query_tgl	= "SELECT TO_CHAR(TGL_REQUEST,'dd/mon/yyyy') TGL_REQUEST FROM REQUEST_RELOKASI 
	...
	$detail_nota  = "SELECT TO_CHAR(a.TARIF, '999,999,999,999') AS TARIF, TO_CHAR(a.BIAYA, '999,999,999,999') AS BIAYA, a.KETERANGAN, a.HZ, a.JML_CONT, TO_DATE(a.START_STACK,'dd/mm/yyyy') START_STACK, TO_DATE(a.END_STACK,'dd/mm/yyyy') END_STACK, b.SIZE_, b.TYPE_, b.STATUS, a.JML_HARI FROM temp_detail_nota a, iso_code b WHERE a.id_iso = b.id_iso and a.no_request = '$no_req'";
	...
	$jum		  = "SELECT COUNT(NO_CONTAINER) JUMLAH FROM container_relokasi WHERE no_request = '$no_req'";
