 ...
 */
 ...
 $db 	 = getDB("storage");
      ...
      $query_get	= " SELECT c.NO_REQUEST, a.NO_NOTA, TO_CHAR(a.ADM_NOTA,'999,999,999,999') ADM_NOTA, TO_CHAR(a.PASS,'999,999,999,999') PASS, 
                        a.EMKL NAMA, a.ALAMAT, a.NPWP, a.LUNAS,a.NO_FAKTUR, a.NOTA_LAMA,
                        TO_CHAR(a.TAGIHAN,'999,999,999,999') TAGIHAN, TO_CHAR(a.PPN,'999,999,999,999') PPN, 
                        TO_CHAR(a.TOTAL_TAGIHAN,'999,999,999,999') TOTAL_TAGIHAN, a.STATUS, 
                        TO_CHAR(c.TGL_REQUEST,'dd/mm/yyyy') TGL_REQUEST, CONCAT(TERBILANG(a.TOTAL_TAGIHAN),'rupiah') TERBILANG 
                            FROM NOTA_RELOKASI a, REQUEST_RELOKASI c where
                            a.NO_REQUEST = c.NO_REQUEST
							AND a.TGL_NOTA = (SELECT MAX(d.TGL_NOTA) FROM NOTA_RELOKASI d WHERE d.NO_REQUEST = '$no_req' )";
...
class MYPDF extends TCPDF {
	...
	public function Header() {
		...
		$this->SetFont('Courier', 'B', 14);
		$this->SetY(36);
		$this->Cell(0, 15, 'NOTA RELOKASI', 0, false, 'R', 0, '', 0, false, 'T', 'M');
	...
	public function Footer() {
		...
		$this->SetY(-16);
		$this->SetX(50);
		$this->Cell(0, 0, 'NOTA RELOKASI' , 0, false, '', 0, '', 0, false, 'T', 'M');
		$this->SetFont('Courier', 'B', 11);
		$this->SetY(-12);
...
EOD;
          ...
          $query_get2	= "SELECT a.KETERANGAN, a.JML_CONT, a.JML_HARI, b.SIZE_, b.TYPE_, b.STATUS, a.HZ,
		TO_CHAR(a.TARIF,'999,999,999,999') TARIF , TO_CHAR(a.BIAYA,'999,999,999,999') BIAYA 
		FROM nota_relokasi_d a, iso_code b, nota_relokasi c WHERE a.ID_ISO = b.ID_ISO(+) AND a.NO_NOTA = c.NO_NOTA AND c.TGL_NOTA = (SELECT MAX(d.TGL_NOTA) FROM NOTA_RELOKASI d WHERE d.NO_REQUEST = '$no_req') ";
...
EOD;
        ...
        $pdf->Output('nota_relokasi.pdf', 'I');
