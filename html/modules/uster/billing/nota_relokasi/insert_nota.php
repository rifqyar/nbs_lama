	...
	*/
	//Insert ke tabel nota
        ...
        $query_cek	= "SELECT NVL(LPAD(MAX(TO_NUMBER(SUBSTR(NO_NOTA,10,15))+1),6,0), '000001') JUM_,
                              ...
                              TO_CHAR(SYSDATE, 'YY') AS YEAR 
                        FROM NOTA_RELOKASI 
                       WHERE NOTA_RELOKASI.TGL_NOTA BETWEEN TRUNC(SYSDATE,'MONTH') AND LAST_DAY(SYSDATE)";
	...
	$no_nota	= "0805".$month.$year.$jum;


        //select master pbm
    $query_master	= "SELECT b.KD_PBM, b.NM_PBM, b.ALMT_PBM, b.NO_NPWP_PBM, TO_CHAR(a.TGL_REQUEST,'dd/mm/yyyy') TGL_REQUEST,
	COUNT(c.NO_CONTAINER) JUMLAH FROM REQUEST_RELOKASI a, v_mst_pbm b, CONTAINER_RELOKASI c 
	WHERE a.KD_EMKL = b.KD_PBM AND a.NO_REQUEST = c.NO_REQUEST AND a.NO_REQUEST = '$no_req' 
	GROUP BY  b.KD_PBM, b.NM_PBM, b.ALMT_PBM, b.NO_NPWP_PBM, TO_CHAR(a.TGL_REQUEST,'dd/mm/yyyy') 
 ";
    ...
    $tarif_pass   = $row_pass['TARIF'];
		...
		$query_adm		= "SELECT TO_CHAR(a.TARIF , '999,999,999,999') AS ADM, a.TARIF FROM MASTER_TARIF a, 
						  GROUP_TARIF b WHERE a.ID_GROUP_TARIF = b.ID_GROUP_TARIF AND 
						  b.KATEGORI_TARIF = 'ADMIN_NOTA'";
		...
		$faktur	 	= "SELECT CONCAT(a.NO_FAKTUR ,(LPAD(NVL((MAX(SEQ_FAKTUR)+1),1),6,0))) FAKTUR, NVL((MAX(SEQ_FAKTUR)+1),1) SEQ 
					  FROM NOTA_ALL_H, (SELECT NO_FAKTUR FROM FAKTUR WHERE TAHUN =  to_char(sysdate, 'RRRR') AND 
					  KETERANGAN = 'NEW') a GROUP BY a.NO_FAKTUR";
		$result		= $db->query($faktur);
        $faktur_     = $result->fetchRow();
		$faktur		= $faktur_['FAKTUR'];
		$seq		= $faktur_['SEQ'];

	$query_insert_nota	= "INSERT INTO NOTA_RELOKASI(NO_NOTA, 
										NO_FAKTUR,
                                        TAGIHAN,
                                        PPN,
                                        TOTAL_TAGIHAN,
		...
		// $db4->query($query_nota_all);
              ...
              //  echo $query_detail;die;
               ...
               //     $jml    = $item['JML_HARI'];
                    ...
                    $query_insert	= "INSERT INTO NOTA_RELOKASI_D
                                            (
                                             ID_ISO,
                                             TARIF,
                                             BIAYA,
					...
					$i++;
					 ...
					 $update_nota = "UPDATE NOTA_RELOKASI SET CETAK_NOTA = 'Y' WHERE NO_NOTA = '$no_nota'";
                    $db->query($update_nota);
					// $db4->query($update_nota);
					 $update_nota1 = "UPDATE REQUEST_RELOKASI SET NOTA = 'Y' WHERE NO_REQUEST = '$no_req'";
