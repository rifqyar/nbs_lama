		...
		if($row_req1["DELIVERY_KE"] == "TPK")
		{
			if(count($row) > 0)
			{
				...
				if($row_nota["LUNAS"] == "YES" && $row_req["CETAK_KARTU"] == 0)
				{
					...
					echo  '<a href="'.HOME.APPID.'.print/print_card_praya?no_req='.$no_req.'" target="_blank"> CETAK KARTU </a> | ';
					$petik = "'";
				...
				else if($row_req["CETAK_KARTU"] > 0 && $row_nota["LUNAS"] == "YES")
				{
					...
					echo  '<a href="'.HOME.APPID.'.print/print_card_praya?no_req='.$no_req.'" target="_blank"> CETAK ULANG </a> | ';
					$petik = "'";
					echo  '<a onclick="print_by_container('.$petik.''.$no_req.''.$petik.')"> Cetak Per Container </a>';
				}
				else
