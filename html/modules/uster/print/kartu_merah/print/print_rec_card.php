...
if($rec_dari=="LUAR")
{
 ...
 WHERE     TTM_BP_CONT.BP_ID = TTD_BP_CONT.BP_ID
	 ...
	 $query_nota =  "SELECT NOTA_RECEIVING.NO_NOTA, TO_CHAR(NOTA_RECEIVING.TGL_NOTA, 'DD-MM-YYYY') TGL_NOTA, V_MST_PBM.NM_PBM EMKL, REQ.NO_REQUEST, CONTAINER_RECEIVING.NO_CONTAINER,
				MASTER_CONTAINER.SIZE_, MASTER_CONTAINER.TYPE_, HISTORY_CONTAINER.STATUS_CONT STATUS, REQ.CETAK_KARTU, 
				REQ.RECEIVING_DARI, YARD_AREA.NAMA_YARD, MASTER_CONTAINER.NO_BOOKING NM_KAPAL, CASE WHEN CONTAINER_RECEIVING.VIA ='darat' THEN 'DARAT' ELSE 'TONGKANG' END VIA, CASE WHEN MASTER_CONTAINER.MLO = 'MLO' THEN '03'
				ELSE '02' END AREA_
				...
				INNER JOIN HISTORY_CONTAINER ON MASTER_CONTAINER.NO_CONTAINER = HISTORY_CONTAINER.NO_CONTAINER AND
				MASTER_CONTAINER.NO_BOOKING = HISTORY_CONTAINER.NO_BOOKING AND
				MASTER_CONTAINER.COUNTER = HISTORY_CONTAINER.COUNTER
				LEFT JOIN V_MST_PBM V_MST_PBM ON REQ.KD_CONSIGNEE = V_MST_PBM.KD_PBM and V_MST_PBM.kd_cabang = '05'
