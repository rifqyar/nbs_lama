...
$query_dtl="SELECT a.KETERANGAN, a.TGL_START_STACK, a.TGL_END_STACK, ROUND(a.JUMLAH_HARI) AS JUMLAH_HARI, a.JUMLAH_CONT, b.UKURAN SIZE_, b.TYPE, b.STATUS, a.HZ, TO_CHAR(a.TARIF,'999,999,999,999') TARIF , TO_CHAR(a.SUB_TOTAL,'999,999,999,999') SUB_TOTAL FROM nota_receiving_d a, master_barang b WHERE a.ID_CONT = b.KODE_BARANG(+) AND a.ID_REQ = '$no_req' AND a.KETERANGAN<>'ADM'";
	...
	foreach($row2 as $rows) {
		...
		<tr><td colspan="3" width="100"><font size="6">'.'<b>'.$rows[KETERANGAN].'</b>'.'</font></td>
                        ...
                        <tr>
		<td colspan="8"><font size="6"><i><b>('.$rows[TGL_START_STACK].' s/d '.$rows[TGL_END_STACK].')'.$rows[JUMLAH_HARI].'hari</i></b></font></td>
                         </tr>
		';
                        }else
                            $detail .= '
...
for($pg=1; $pg<=$jum_page; $pg++) {
	...
	$tbl = <<<EOD
		...
		<tr>                    
                 ...
                 <td width="100"><font size="5"><b>$date</b></font></td>
                </tr> 
                <tr>                    
