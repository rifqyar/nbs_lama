...
$query="SELECT A.*,
  ...
  FROM NOTA_REEXPORT_H A, REQ_REEXPORT_H B, TB_USER C
 WHERE B.ID_REQ = TRIM('$p1') AND A.ID_REQUEST = B.ID_REQ AND C.ID = A.USER_ID";
...
$query_dtl="SELECT A.*,C.*, to_char(A.TARIF,'999,999,999,999,999.00') AS TARIFN, to_char(A.SUB_TOTAL,'999,999,999,999,999.00') AS SUB_TOTALN 
            FROM NOTA_REEXPORT_D A, NOTA_REEXPORT_H B, MASTER_BARANG C WHERE A.ID_NOTA='$nota_trans'  AND A.ID_NOTA=B.ID_NOTA AND C.KODE_BARANG=A.ID_CONT";
...
$query_jum="SELECT COUNT(1) JUM_DETAIL FROM NOTA_REEXPORT_D A WHERE A.ID_NOTA='$nota_trans'";
