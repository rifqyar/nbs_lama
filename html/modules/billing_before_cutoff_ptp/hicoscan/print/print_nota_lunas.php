...
$query="SELECT ID_NOTA  FROM nota_hicoscan_h WHERE TRIM(ID_REQUEST) = TRIM('$p01')";
...
$query="INSERT INTO LOG_NOTA (ID_NOTA,NO_REQUEST,KODE_MODUL,KETERANGAN,ID_USER) VALUES ('$notanya','$p01','$kode','CETAK','$id_user')";
		$db->query($query);
$query="UPDATE nota_hicoscan_h SET TGL_CETAK=SYSDATE WHERE ID_REQUEST='$p01'";
$db->query($query);
$query="SELECT ID_NOTA FROM nota_hicoscan_h WHERE ID_REQUEST='$p01'";
$res=$db->query($query);
$res_=$res->fetchRow();
$p1=$res_['ID_NOTA'];
//print_r($p1);die;
$query="SELECT A.*,B.*,SYSDATE AS TGL_PROSES,UPPER(C.NAME) AS NAMA,to_char(A.INVOICE_DATE,'DD-MM-YYYY') TGL_NOTA2, to_char(A.PPN,'999,999,999,999,990.00') PPNN,
		to_char(A.JUMLAH,'999,999,999,999,999.00') TAGIHANN,to_char(A.TOTAL,'999,999,999,999,999.00') TOTALN,UPPER(B.TIPE_REQ) TIPE_REQUEST
		,TO_CHAR((select d.sub_total from nota_hicoscan_d d where trim(d.ID_NOTA)=trim(A.ID_NOTA) and d.KETERANGAN='ADM'),'999,999,999,999,999.00') ADMN
		
		FROM NOTA_HICOSCAN_H A, REQ_HICOSCAN B, TB_USER C WHERE A.ID_NOTA='$p1' AND A.ID_REQUEST=B.ID_REQUEST AND C.ID=A.USER_ID_INVOICE";
$data = $db->query($query)->fetchRow();
 $date=date('d M Y H:i:s');
 $query3="SELECT NO_CONTAINER from REQ_HICOSCAN_D A, NOTA_HICOSCAN_H B WHERE B.ID_NOTA='$p1'  AND A.ID_REQUEST=B.ID_REQUEST";
...
for($pg=1; $pg<=$jum_page; $pg++) {
	...
	$tbl = <<<EOD
			<table>
                ...
                <tr>
                    <td COLSPAN="6"></td>
                    <td COLSPAN="8" align="right"><font size="11"><b>PRANOTA HI CO SCAN $data[TIPE_REQUEST]</b></font></td>
...
<table>                    
                    ...
                    <tr>
                        ...
                        <td width="120" colspan="2" align="right">$data[ADMN]</td>
                    </tr>
                    <tr>
                        <td colspan="7"></td>
