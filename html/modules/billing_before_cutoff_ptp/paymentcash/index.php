...
{
	$('#pay').load("<?=HOME?>billing.paymentcash.ajax/payment?idn="+a+"&req="+b+"&ket="+c+"&vessel="+encodeURIComponent(vessel)+"&voyin="+encodeURIComponent(voyin)+"&voyout="+encodeURIComponent(voyout)+"&total="+total, function(data){
		$("#paid_via").load("<?=HOME?>billing.paymentcash.ajax/paid_via",{KD_PELUNASAN:1});	
