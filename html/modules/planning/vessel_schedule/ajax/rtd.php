...
{
	//alert('coba');
	$('#rtd').dialog('destroy').remove();
	$('#mainform').append('<div id="rta"></div>');
}

function update_rtd()
{
	var url="<?=HOME;?>planning.vessel_schedule.ajax/edit_rtd";
	var id_ukk='<?=$nukk;?>';
	var rtd=$( "#rtdt" ).val();
	
	if(rtd=='')
	{
		alert ('Please Insert RTD');
		return false;
	}
	else
	{
		$.post(url,{UKK: id_ukk,RTD:rtd},function(data){	
			//alert(data);
			
			$('#rtd').dialog('destroy').remove();	
			$('#mainform').append('<div id="rtd"></div>');
...
<table>
	...
	<tr>
		<td>RTD</td>
		<td>:</td>
		<td><input type='text' id='rtdt' size='15' value="<?=$rtd;?>"/></td>
	</tr>
		<tr>
	...
	<tr>
		<td></td>
		<td></td>
		<td><button onclick='update_rtd()'>Save</button>
