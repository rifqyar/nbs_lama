...
for ($i = 2; $i <= $baris; $i++) {
	...
	$oheight = $data->val($i,18);
	$oleft = $data->val($i,19);
	$oright = $data->val($i,20);
	$ofront = $data->val($i,21);
	$oback = $data->val($i,22);
...
$param_b_var= array(	
							...
							"v_unno"=>"$un_no",
                            "v_msg"=>"");
$query = "declare begin proc_add_cont_baplie('$id_vessel',:v_disc_port,:v_load_port,:v_bay,
                                             :v_container,:v_weight,:v_iso_code,:v_type,:v_hz,:v_fm,:v_temp,
											 :v_oheight,:v_oleft,:v_oright,:v_ofront,:v_oback,
											 :v_unno,:v_msg); end;";         
