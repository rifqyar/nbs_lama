...
foreach ($block as $block_)
{
	...
	//	echo "UPDATE YD_BLOCKING_CELL SET SIZE_PLAN_ALLO='$sizea' WHERE ID_BLOCKING_AREA = '$id_block' AND INDEX_CELL='$index_cell'";
		...
		$query_size      = "SELECT SIZE_PLAN_ALLO_MIX FROM YD_BLOCKING_CELL WHERE INDEX_CELL = $index_cell AND ID_BLOCKING_AREA = '$id_block'";			
		//print_R($query_slot_row.'<br>');
		$result2         = $db->query($query_size);
        $hasil2          = $result2->fetchRow();
        $mix          	 = $hasil2['SIZE_PLAN_ALLO_MIX'];
		
		if ($mix == NULL) {
			$query_b     = "UPDATE YD_BLOCKING_CELL SET SIZE_PLAN_ALLO='$sizea' WHERE ID_BLOCKING_AREA = '$id_block' AND INDEX_CELL='$index_cell'";
			$az=$db->query($query_b);
			$query_b     = "UPDATE YD_BLOCKING_CELL SET SIZE_PLAN_ALLO_MIX='$sizea' WHERE ID_BLOCKING_AREA = '$id_block' AND INDEX_CELL='$index_cell'";
			$az=$db->query($query_b);
		} else {
				$data1	= explode('.',$mix);
				$satu	= $data1[0];
				...
				if ($mix==$sizea){
					...
					$query_b     = "UPDATE YD_BLOCKING_CELL SET SIZE_PLAN_ALLO='$sizea' WHERE ID_BLOCKING_AREA = '$id_block' AND INDEX_CELL='$index_cell'";
					$az=$db->query($query_b);
					$query_b     = "UPDATE YD_BLOCKING_CELL SET SIZE_PLAN_ALLO_MIX='$satu' WHERE ID_BLOCKING_AREA = '$id_block' AND INDEX_CELL='$index_cell'";
					$az=$db->query($query_b);
				}
		}
		...
		//	echo "select id_placement from yd_placement_yard where id_cell = '$index_cell' and tier_yard = '$i'";
			...
			if ($place == NULL){
					...
					$query_end="UPDATE TB_BOOKING_CONT_AREA SET PLAN_STATUS=0 WHERE trim(ID_BOOK)=TRIM('$idb')";
					$db->query($query_end);
...
$query_end="UPDATE TB_BOOKING_CONT_AREA SET PLAN_STATUS=1 WHERE trim(ID_BOOK)=TRIM('$idb')";
$db->query($query_end);
$query_end2="UPDATE RBM_H SET FLAG_YAI=1 WHERE trim(NO_UKK)=TRIM('$id_vs')";
$db->query($query_end2);
