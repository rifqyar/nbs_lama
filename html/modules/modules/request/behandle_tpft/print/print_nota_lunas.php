...
$query="SELECT ID_NOTA FROM bh_nota WHERE TRIM(ID_REQUEST) = TRIM('$p01')";
...
$query="INSERT INTO LOG_NOTA (ID_NOTA,NO_REQUEST,KODE_MODUL,KETERANGAN,ID_USER) VALUES ('$notanya','$p01','$kode','CETAK','$id_user')";
		$db->query($query);
$query="UPDATE BH_NOTA SET TGL_CETAK=SYSDATE WHERE ID_REQUEST='$p01'";
$db->query($query);

$p1=$notanya;
//print_r($p1);die;
$query="SELECT A.*,B.*,SYSDATE AS TGL_PROSES,UPPER(C.NAME) AS NAMA,to_char(A.INVOICE_DATE,'DD-MM-YYYY') TGL_NOTA2, to_char(A.ADM_NOTA,'999,999,999,999,990.00') ADM_NOTAN,to_char(A.PPN,'999,999,999,999,990.00') PPNN,
		...
		FROM BH_NOTA A, BH_REQUEST B, TB_USER C WHERE A.ID_NOTA='$p1' AND A.ID_REQUEST=B.ID_REQUEST AND C.ID=A.USER_ID";
...
$query_dtl="SELECT A.*,C.*, to_char(A.TARIF,'999,999,999,999,999.00') AS TARIFN, to_char(A.SUB_TOTAL,'999,999,999,999,999.00') AS SUB_TOTALN 
			from BH_DETAIL_NOTA A, BH_NOTA B, MASTER_BARANG C WHERE A.ID_NOTA='$p1'  AND A.ID_NOTA=B.ID_NOTA AND C.KODE_BARANG=A.ID_BARANG";
