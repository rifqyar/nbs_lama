...
jQuery(function() {
 jQuery("#l_vessel").jqGrid({
	...
	, editurl:"<?=HOME;?>request.batalmuat.delivery_p.ajax/edit_row?id_req='<?=$req;?>'"	
 });
...
{
	var url="<?=HOME;?>request.batalmuat.delivery_p.ajax/save_req_d";	
	$.post(url,{REQ:a, OLDREQ: b, KE: j}, function(data){
		//alert('Request telah disimpan');
		alert(data);
		window.open("<?=HOME?>request.batalmuat.delivery_p/", "_SELF");
	});
}
