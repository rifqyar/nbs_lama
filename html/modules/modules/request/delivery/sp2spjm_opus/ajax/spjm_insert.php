...
$('#loadspjm').load("<?=HOME?>request.delivery.sp2spjm_opus.ajax/load_spjm?idplp="+0+"&setoption=0");
...
$(document).ready(function(){	
				$("#plp_no").autocomplete({		
					minLength: 3,
					source: "request.delivery.sp2spjm_opus.auto/plp_no",
				...
				$("#vessel").autocomplete({		
					minLength: 3,
					source: "request.delivery.sp2spjm_opus.auto/vessel",
					focus: function( event, ui ) 
					{
					...
					.data( "autocomplete" )._renderItem = function( ul, item ) 
					{
						return $( "<li></li>" )
						.data("item.autocomplete", item )
						.append("<a>" + item.VESSEL + "<br/>" + item.VOYAGE_IN + " " + item.VOYAGE_OUT + "</a>")
						.appendTo(ul);
					};					
...
$(document).ready(function(){
	...
	$("#emkl").autocomplete({
		minLength: 3,
		source: "request.delivery.sp2spjm_opus.auto/pelanggan",
		focus: function( event, ui ) 
		{
...
function load_detail()
{
	...
	$('#loadspjm').load("<?=HOME?>request.delivery.sp2spjm_opus.ajax/load_spjm?idplp="+v_idplp+"&noukk="+v_noukk+"&setoption=1");
}
...
function save_spjm()
{
	...
	{	
		var url = "<?=HOME?>request.delivery.sp2spjm_opus.ajax/app2_plp";
		...
		if(r == true)
		{
			...
			$.post(url,{
				...
				if(data == "OK")
				{
					...
					alert("Success...");
					window.location = "<?=HOME?>request.delivery.sp2spjm_opus/";
				} else if (data == "EXIST"){
					$.unblockUI({
					onUnblock: function(){ }
					});
