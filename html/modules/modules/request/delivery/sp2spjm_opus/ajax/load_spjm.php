...
<?
	...
	$id_plp = TRIM($_GET['idplp']);
	$noukk = $_GET['noukk'];
	$db = getDB('dbportal');
	$db2 = getDB('dbint');
	
	$string_query = "";
	$q_sppud = "SELECT CONT_NO FROM SPPUD WHERE TRIM(PIB_NO) = '$id_plp'";
	...
	$jml_cek="SELECT count(A.NO_CONTAINER) JML
		   ...
		   WHERE A.CONT_LOCATION = 'Yard'
				 AND 
				 B.ID_VSB_VOYAGE LIKE '$noukk'
				 AND A.NO_CONTAINER IN ($string_query)";
				 //echo($jml_cek);
	$datacont = $db2->query($jml_cek)->fetchRow();
		...
		<table>
			<tr>
				...
				Filter: <input type="text" id="box1Filter" /><button type="button" id="box1Clear">X</button><br/><br/>
						...
						<?php								
								$query = "SELECT A.NO_CONTAINER NO_CONT
			FROM M_CYC_CONTAINER A INNER JOIN M_VSB_VOYAGE B ON (A.VESSEL = B.VESSEL AND A.VOYAGE_IN = B.VOYAGE_IN AND A.VOYAGE_OUT = B.VOYAGE_OUT)
		   WHERE A.CONT_LOCATION = 'Yard'
				 AND 
				 B.ID_VSB_VOYAGE LIKE '$noukk'
