...
function create_req()
{
	var url="<?=HOME;?>request.transhipment.ajax/save_req";
	...
	{
		//alert('e');
		$.post(url,{TIPE_OI: tipe_oi,EMKL: emkl,IDEMKL:idemkl, INST:inst, BC12:bc12, OVES:oves, OVOY:ovoy, OUKK:oukk, NVES:nves, NVOY:nvoy, NUKK:nukk, KET: ket, OVOYO:ovoyo, NVOYO:nvoyo},function(data){	
			//alert(data);
			var row_data = data;
			...
			else
			{
				...
				$('#detail_req').load("<?=HOME?>request.transhipment.ajax/detail_req?id="+v_req+"&oukk="+oukk+"&nukk="+nukk+"&oi="+tipe_oi);
...
{	
	var url="<?=HOME;?>request.transhipment.ajax/add_detail_req";
	var arr = [];
...
<div class="content">
	...
	<tr>
	 ...
	 <tr>
		...
		<input type="text" size="5" name="ovoy" id="ovoy" readonly="readonly"/> <input type="text" size="5" name="ovoyo" id="ovoyo" readonly="readonly"/> <input type="hidden" name="oukk" id="oukk" /> 
		</td>
	</tr>
	 <tr>
		...
		<input type="text" size="5" name="nvoy" id="nvoy" readonly="readonly"/> <input type="text" size="5" name="nvoyo" id="nvoyo" readonly="readonly"/> <input type="hidden" name="nukk" id="nukk" /> 
		</td>
	</tr>
	 <tr>
