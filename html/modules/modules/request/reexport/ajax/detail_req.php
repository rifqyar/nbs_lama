...
(SELECT NO_CONTAINER FROM REQ_REEXPORT_D WHERE ID_REQ='$req') AND NO_CONTAINER NOT IN (SELECT NO_CONTAINER FROM REQ_DELIVERY_D WHERE NO_UKK='$oukk')";
echo $query;
...
<?php
$query="SELECT NO_CONTAINER, SIZE_, TYPE_, STATUS_, HZ, HEIGHT, E_I, ID_CONT FROM REQ_REEXPORT_D WHERE ID_REQ='$req' ORDER BY URUT";
if($res = $db->query($query))
while ($row = $res->fetchRow()) {
