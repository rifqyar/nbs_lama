...
<?php
	$db=getDb();
	$req=$_GET['no_req'];
	$qr="SELECT SHIPPING_LINE, ALAMAT, NPWP, VESSEL, VOYAGE, E_I, TGL_REQUEST, NO_UKK, USER_REQ, STATUS, NO_BC_12, NO_SURAT_TRS, VESSEL_DT, VOYAGE_DT, NO_UKK_DT, KD_PELANGGAN, OI, DECODE(OI,'O','Ocean Going','I','Intersuler') AS KET_OI, KETERANGAN from REQ_REEXPORT_H A WHERE ID_REQ='$req'";
	//print_r($qr);die;
	$er=$db->query($qr);
	$hr=$er->fetchRow();
?>
