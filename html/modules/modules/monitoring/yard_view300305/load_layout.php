...
setInterval(function()
		{
			window.location = "<?=HOME?>monitoring.yard_view300305/load_layout?id=81";
		},
		300000);
...
-webkit-border-radius: 10px 10px 10px 10px; ">
    <center>
		...
		<table width="1315"><tr height="5"bgcolor="#cecdcd" align="center" ><td></td></tr>
			...
			<font color="black"><h2>Layout</font> <font color="black">Lapangan Ex TBB</font></h2></td></tr>
	...
	<table width="200%" cellspacing="0" border="0" bgcolor="#F6F4E4">
	<tbody>
		 ...
		 //echo "SELECT a.INDEX_CELL, b.ID, b.NAME, a.SLOT_, a.ROW_,a.STATUS_BM FROM YD_BLOCKING_AREA b, YD_BLOCKING_CELL a WHERE a.ID_BLOCKING_AREA = b.ID AND b.ID_YARD_AREA = '$yard_id' ORDER BY a.INDEX_CELL ASC";die;
			...
			*/
			$query_cell2 = "SELECT a.INDEX_CELL as INDEX_CELL, b.ID AS ID, b.NAME AS NAME, a.SLOT_ AS SLOT_, a.ROW_ AS ROW_, a.STATUS_BM,b.POSISI,a.SIZE_PLAN_PLC FROM YD_BLOCKING_AREA b,         
                            YD_BLOCKING_CELL a WHERE a.ID_BLOCKING_AREA = b.ID AND b.ID_YARD_AREA = '81' AND a.SIZE_PLAN_PLC IS NULL 
                            UNION 
                            SELECT d.INDEX_CELL AS INDEX_CELL, c.ID AS ID, c.NAME AS NAME, d.SLOT_ AS SLOT_, d.ROW_ AS ROW_, d.STATUS_BM,c.POSISI,d.SIZE_PLAN_PLC FROM YD_BLOCKING_AREA c, YD_BLOCKING_CELL d WHERE d.ID_BLOCKING_AREA = c.ID AND c.ID_YARD_AREA = '81' AND d.SIZE_PLAN_PLC = '40d'
                            UNION
                            SELECT d.INDEX_CELL AS INDEX_CELL, c.ID AS ID, c.NAME AS NAME, d.SLOT_ AS SLOT_, d.ROW_ AS ROW_,d.STATUS_BM,c.POSISI,d.SIZE_PLAN_PLC FROM YD_BLOCKING_AREA c, YD_BLOCKING_CELL d WHERE d.ID_BLOCKING_AREA = c.ID AND c.ID_YARD_AREA = '81' AND d.SIZE_PLAN_PLC = '20'
                           -- UNION 
                           -- SELECT d.INDEX_CELL AS INDEX_CELL, c.ID AS ID, c.NAME AS NAME, d.SLOT_ AS SLOT_, d.ROW_ AS ROW_,d.STATUS_BM,c.POSISI,d.SIZE_PLAN_PLC FROM YD_BLOCKING_AREA c, YD_BLOCKING_CELL d WHERE d.ID_BLOCKING_AREA = c.ID AND c.ID_YARD_AREA = '81' AND d.SIZE_PLAN_PLC = '40b'
...
<!--                     </div>-->
				   ...
				   <? 
					...
					{?>
						  ...
						  &nbsp &nbsp
						  <?//echo //echo $placement ;// echo $row['NAME'];// echo $index_cell_ ; // echo $index_cell_ ;//echo $st_bm;?><?//=$id_block;?></td>
					<?}
					else //if ($st_bm=='Bongkar')
					{
					?>
							<td data-tooltip="sticky<?=$index_cell_?>" onclick="('<?=$id_block?>','<?=$slot_?>','<?=$row_?>','<?=$yard_id?>')" onMouseOver="this.style.backgroundColor='#CCFF33'" onMouseOut="this.style.backgroundColor='#82ff07'" align="center" style="width:30px;height:10px;font-size:8px; font-family:Tahoma; border:1px solid #ffffff;background-color:#82ff07; ">&nbsp &nbsp
