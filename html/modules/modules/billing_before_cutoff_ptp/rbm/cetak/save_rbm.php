...
for($pg=1; $pg<=$jum_page; $pg++) {
	...
	$tbl = <<<EOD
			...
			<table border="0">
					...
					<tr><td colspan="4" align="center"><font size="12pt"><b><i>REALISASI BONGKAR MUAT VESSEL $ves_id VOYAGE $voy_in/$voy_out</i></b></font></td></tr>
...
$query_i 	= "SELECT NVL(MAX(COUNTER),0) COUNT FROM LOG_RBM where NO_UKK='$no_ukk' AND KETERANGAN = 'RBM_ICT'";
...
$date = date('dMYh:i:s');
ob_clean();
   $pdf->Output('./report/pdf_rbm/RBM_ICT_'.$ves_id.'_'.$voy_in.$voy_out.'_'.$count.'.pdf', 'F');
