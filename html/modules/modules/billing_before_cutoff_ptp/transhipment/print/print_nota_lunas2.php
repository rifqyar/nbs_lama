...
$query="SELECT ID_NOTA FROM NOTA_TRANSHIPMENT_H WHERE TRIM(ID_REQUEST) = TRIM('$p01')";
...
$query="INSERT INTO LOG_NOTA (ID_NOTA,NO_REQUEST,KODE_MODUL,KETERANGAN,ID_USER) VALUES ('$notanya','$p01','$kode','CETAK','$id_user')";
		$db->query($query);
$query="UPDATE NOTA_TRANSHIPMENT_H SET TGL_CETAK=SYSDATE WHERE ID_NOTA='$notanya'";
$db->query($query);
...
$query="SELECT A.*,B.*,DECODE(B.OI, 'O', 'Ocean Going', 'I', 'Intersuler') as KET_OI, SYSDATE AS TGL_PROSES,UPPER(C.NAME) AS NAMA,to_char(A.INVOICE_DATE,'DD-MM-YYYY') TGL_NOTA2, to_char(A.ADM_NOTA,'999,999,999,999,990.00') ADM_NOTAN,to_char(A.PPN,'999,999,999,999,990.00') PPNN, to_char(A.JUMLAH,'999,999,999,999,999.00') TAGIHANN,to_char(A.TOTAL,'999,999,999,999,999.00') TOTALN
		FROM NOTA_TRANSHIPMENT_H A, REQ_TRANSHIPMENT_H B, TB_USER C WHERE A.ID_NOTA='$p1' AND A.ID_REQUEST=B.ID_REQ AND C.ID=A.USER_ID";
$data = $db->query($query)->fetchRow();
 $date=date('d M Y H:i:s');
 $query3="SELECT DISTINCT TGL_START_STACK, TGL_END_STACK FROM NOTA_TRANSHIPMENT_D WHERE ID_NOTA='$p1' AND ID_CONT IS NOT NULL";
...
$query_dtl="SELECT A.*,C.*, to_char(A.TARIF,'999,999,999,999,999.00') AS TARIFN, to_char(A.SUB_TOTAL,'999,999,999,999,999.00') AS SUB_TOTALN 
			FROM NOTA_TRANSHIPMENT_D A, NOTA_TRANSHIPMENT_H B, MASTER_BARANG C WHERE A.ID_NOTA='$p1'  AND A.ID_NOTA=B.ID_NOTA AND C.KODE_BARANG=A.ID_CONT";
...
$query_jum="SELECT COUNT(1) JUM_DETAIL FROM NOTA_TRANSHIPMENT_D A WHERE A.ID_NOTA='$p1'";
...
for($pg=1; $pg<=$jum_page; $pg++) {
	...
	$tbl = <<<EOD
			<table>
                ...
                <tr>
                    <td COLSPAN="6"></td>
                    <td COLSPAN="8" align="right"><font size="11"><b>PRANOTA TRANSHIPMENT</b></font></td>
