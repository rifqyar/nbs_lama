...
jQuery(function() {
 jQuery("#nota_delivery").jqGrid({
	url:'<?=HOME?>datanya/data_cont?q=nota_delivery_tn',
	mtype : "post",
	datatype: "json",
	colNames:['Req Date','','Status','Jenis Request','No Nota','No Request','Qty','EMKL','Vessel'],
	colModel:[
...
function confirm_no_charge(id_req)
{
	...
	if(answer != 0){
		var url="<?=HOME;?>billing.delivery_admin/no_charge";
			$.post(url,{no_req:id_req},function(data){	
			alert(data);
			window.location="<?=HOME?>billing.delivery_admin";			
		});	
	}
}

function cek_save(tipe_req,id_req){
	var url="<?=HOME?>billing.delivery.ajax/cek_save";
	$.post(url,{noreq:id_req},function (data){
		if(data=="1"){
			alert("Request belum disave. Silahkan save terlebih dahulu!");
			return(false);
		} else {
			window.location="<?=HOME?>billing.delivery/preview?tipereq="+tipe_req+"&id="+id_req;
		}
	});
}
...
<div class="content">
	<div class="main_side">
	<p>
	<h2> <img src="<?=HOME?>images/delivery.png" height="5%" width="5%" style="vertical-align:middle"> Billing SP2 Delivery</h2></p>
