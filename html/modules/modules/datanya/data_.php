...
if(isset($q)) {
	...
	if($q=='pranota_bprp') //ambil data header
		...
		$query="SELECT A.ID_REQUEST,C.ID_NOTA,A.EMKL,A.VESSEL,A.VOYAGE,(SELECT COUNT(B.NO_CONTAINER) FROM BH_DETAIL_REQUEST B WHERE A.ID_REQUEST=B.ID_REQUEST) JUMLAH_CONT,C.STATUS FROM BH_REQUEST A left join BH_NOTA C  on  A.ID_REQUEST=C.ID_REQUEST ORDER BY A.TGL_REQUEST DESC";	
...
NM_PELABUHAN_TUJUAN, STATUS, EMAIL, JML_BONGKAR,to_char(OPEN_STACK,'DD MON YYYY HH24:Mi') OPEN_STACK, to_char(CLOSING_TIME,'DD MON YYYY HH24:Mi') CLOSING_TIME,
JML_MUAT, JML_SHIFT, JML_HATCH FROM RBM_H ORDER BY TGL_JAM_TIBA DESC";
	else if($q=='rbm') //ambil data header
		$query="SELECT NO_UKK ID_VS, NM_KAPAL NAMA_VESSEL , VOYAGE_IN ,VOYAGE_OUT, TGL_JAM_TIBA ETA, TGL_JAM_BERANGKAT ETD, NM_PELABUHAN_ASAL, NM_PELABUHAN_TUJUAN, STATUS, EMAIL, JML_BONGKAR, JML_MUAT, JML_SHIFT, JML_HATCH FROM RBM_H ORDER BY TGL_JAM_TIBA DESC";
		...
		$query="SELECT NOTA, NO_UKK ID_VS, NM_KAPAL NAMA_VESSEL , NM_PEMILIK NAMA_PEMILIK, VOYAGE_IN ,VOYAGE_OUT, TGL_JAM_TIBA ETA, TGL_JAM_BERANGKAT ETD, NM_PELABUHAN_ASAL, NM_PELABUHAN_TUJUAN, PRANOTA FROM RBM_H ORDER BY TGL_JAM_TIBA DESC";
	...
	///print_r($query);die;
		...
		$query="SELECT NO_UKK ,BAY,ALAT, ACTIVITY, MOVE_TIME_OPEN,MOVE_TIME_CLOSE, OI, OPERATOR, JUMLAH FROM RBM_HM WHERE NO_UKK= '$no_ukks'";
	$res = $db->query($query);
	//debug($res);die;
	//ociexecute($query);
	...
	while ($row = $res->fetchRow()) {
		...
		else if($q == 'booking') 
		{
			...
			$idvs=$row[ID_VS];
			$aksi2 = "<button onclick='tambah(\"$idvs\")'><img border='0' src='images/add1.png' height='20px' width='20px'></button>&nbsp;<button onclick='op_st(\"$idvs\")'><img border='0' src='images/booking_open.png' height='20px' width='20px'></button>&nbsp;<button onclick='cl_tm(\"$idvs\")'><img border='0' src='images/booking_closed.png' height='20px' width='20px'></button>";
			$responce->rows[$i]['id']=$row[ID_VS];
			$responce->rows[$i]['cell']=array($aksi2,$row[NAMA_VESSEL],$row[VOYAGE_IN],$row[VOYAGE_OUT], $row[OPEN_STACK],$row[CLOSING_TIME],$row[ETA],$row[ETD],$row[NM_PELABUHAN_ASAL],$row[NM_PELABUHAN_TUJUAN]);
		}
		else if($q == 'rbm') 
		{
			$pw=$row[ID_VS];
			if ($row[STATUS] == "CLOSED"){
				if ($row[EMAIL] == "Y"){
					$status = "<img src='images/zimbra.png' width=15px height=15px border='0' title='Email Sent'>&nbsp;&nbsp;<a onclick='final_rbm(\"$pw\")' target='blank'><img src='images/save2.png' width=20px height=20px border='0' title='Final RBM'></a>&nbsp;&nbsp;<a onclick='koreksi_rbm(\"$pw\")' target='blank'><img src='images/koreksi.jpg' width=20px height=20px border='0' title='Koreksi RBM'></a>";
				} else {
					$status = "<a href=".HOME."billing.rbm/sent_email/?id_vessel=".$row[ID_VS]." target='blank'><img src='images/sent.png' width=22px height=22px border='0' title='Send Email'></a>";
				}
				$sc		= "<button onclick='sync_rbm(\"$pw\")' title='update RBM'><img src='images/Refresh2.png' width=15px height=15px border='0'></button>&nbsp;<button disabled='disabled' onclick='get_data(\"$pw\")' title='get data ict'><img src='images/Refresh4.png' disabled='disabled' width=15px height=15px border='0'></button>";
				$list = "<div><a>LOCKED!</a></div>";
				
			} else if ($row[STATUS] == "FINAL"){
					$status = "<font color='red'><i><b>FINAL RBM</b></i></font>";
					$sc		= "<button onclick='sync_rbm(\"$pw\")' title='update RBM' disabled='disabled'><img src='images/Refresh4.png' disabled='disabled' width=15px height=15px border='0'></button>&nbsp;<button disabled='disabled' onclick='get_data(\"$pw\")' title='get data ict'><img src='images/Refresh4.png' disabled='disabled' width=15px height=15px border='0'></button>";
				...
				$status = "<a onclick='closed_rbm(\"$pw\")' target='blank'><img src='images/submit2.png' width=20px height=20px border='0' title='Submit RBM'></a>";
			...
			$responce->rows[$i]['id']=$row[ID_VS];
			$responce->rows[$i]['cell']=array($status,$sc,$aksi,$row[ID_VS],$list,$row[NAMA_VESSEL],$voy,$row[JML_BONGKAR],$row[JML_MUAT], $row[ETA],$row[ETD],$row[NM_PELABUHAN_ASAL],$row[NM_PELABUHAN_TUJUAN]);
		}
		else if($q == 'vs_fix') 
		{
			...
			$voy	= $row[VOYAGE_IN].' - '.$row[VOYAGE_OUT];
			$responce->rows[$i]['id']=$row[ID_VS];
			$responce->rows[$i]['cell']=array($row[ID_VS],$row[NAMA_VESSEL],$voy,$row[JML_BONGKAR],$row[JML_MUAT], $row[ETA],$row[ETD],$row[NM_PELABUHAN_ASAL],$row[NM_PELABUHAN_TUJUAN]);
		}
		else if($q == 'm_rbm') 
		{
			$pw=$row[ID_VS];
		...
		else if($q == 'bm') 
		{
			...
			{
				$aksi = "<a href=".HOME."billing.bm/sent_email_nota/?id_vessel=".$row[ID_VS]." target='blank'><img src='images/email.jpg' width=25px height=25px border='0' title='Sent Email'></a> | <a href=".HOME."billing.bm.cetak/cetak_nota/?id_vessel=".$row[ID_VS]." target='blank'><img src='images/print.png' width=25px height=25px border='0' title='cetak Nota Bongkar Muat'></a>";
			}
			else
			{
				//$aksi = "<a href=".HOME."billing.bm/sent_email_nota/?id_vessel=".$row[ID_VS]." target='blank'><img src='images/email.jpg' width=25px height=25px border='0' title='Sent Email'></a> | <a href=".HOME."billing.bm.cetak/cetak_nota/?id_vessel=".$row[ID_VS]." target='blank'><img src='images/print.png' width=25px height=25px border='0' title='cetak Nota Bongkar Muat'></a>";
			...
			$responce->rows[$i]['id']=$row[ID_VS];
			$responce->rows[$i]['cell']=array($aksi,$row[ID_VS],$row[NAMA_VESSEL],$row[VOYAGE_IN].'/'.$row[VOYAGE_OUT],$row[NAMA_PEMILIK],$row[ETA],$row[ETD],$row[NM_PELABUHAN_ASAL],$row[NM_PELABUHAN_TUJUAN]);
		}
		else if($q == 'hatch') 
		{
