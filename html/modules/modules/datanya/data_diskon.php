...
if(isset($q)) 
{
	...
	{	
		$db = getDB();
		$query ="select z.ID_NOTA,z.NO_FAKTUR, z.ID_REQ, z.EMKL, z.STATUS,z.ALAMAT, z.VESSEL, z.VOYAGE_IN, z.VOYAGE_OUT, z.TGL_SIMPAN, z.TGL_PAYMENT, z.PAYMENT_VIA,z.TOTAL,z.COA,z.KD_MODUL,z.KET, y.ARPROCESS_DATE TANGGAL_RECEIPT
        from 
        (select ID_NOTA,NO_FAKTUR, ID_REQ, EMKL, STATUS,ALAMAT, VESSEL, VOYAGE_IN, VOYAGE_OUT, TGL_SIMPAN, TGL_PAYMENT, PAYMENT_VIA,TOTAL,COA,KD_MODUL,'ANNE' KET from nota_receiving_h
        union
        select ID_NOTA,NO_FAKTUR, ID_REQ, EMKL, STATUS,ALAMAT, VESSEL, VOYAGE_IN, VOYAGE_OUT, TGL_NOTA AS TGL_SIMPAN, TGL_PAYMENT, PAYMENT_VIA, TOTAL,COA,KD_MODUL, 'SP2' KET from nota_delivery_h
        union
        select NO_NOTA AS ID_NOTA, NO_FAKTUR, ID_BATALMUAT AS ID_REQ, EMKL, STATUS, ALAMAT, VESSEL , ' ' AS VOYAGE_IN, VOYAGE AS VOYAGE_OUT, TGL_NOTA AS TGL_SIMPAN,TGL_PAYMENT, PAYMENT_VIA, BAYAR TOTAL, COA, JENIS AS KD_MODUL,'BM' KET from nota_batalmuat_h 
        union
        ...
        union
        select ID_NOTA, NO_FAKTUR, ID_REQUEST AS ID_REQ, CUSTOMER NM_PEMILIK, STATUS, ALAMAT, VESSEL ,VOYAGE VOYAGE_IN, VOYAGE VOYAGE_OUT, TGL_CETAK TGL_SIMPAN, TGL_PAYMENT, PAYMENT_VIA, TOTAL, COA, '' AS KD_MODUL, 'STACKEXT' KET from NOTA_STACKEXT_H
        union
        ...
        ) z left join tth_nota_all2 y on z.ID_NOTA=y.NO_NOTA
...
";
	...
	{
		$query="select * from (select z.ID_NOTA,z.NO_FAKTUR, z.ID_REQ, z.EMKL, z.STATUS,z.ALAMAT, z.VESSEL, z.VOYAGE_IN, z.VOYAGE_OUT, z.TGL_SIMPAN, z.TGL_PAYMENT, z.PAYMENT_VIA,z.TOTAL,z.COA,z.KD_MODUL,z.KET, y.ARPROCESS_DATE TANGGAL_RECEIPT
        from 
        (select ID_NOTA,NO_FAKTUR, ID_REQ, EMKL, STATUS,ALAMAT, VESSEL, VOYAGE_IN, VOYAGE_OUT, TGL_SIMPAN, TGL_PAYMENT, PAYMENT_VIA,TOTAL,COA,KD_MODUL,'ANNE' KET from nota_receiving_h
        union
        select ID_NOTA,NO_FAKTUR, ID_REQ, EMKL, STATUS,ALAMAT, VESSEL, VOYAGE_IN, VOYAGE_OUT, TGL_NOTA AS TGL_SIMPAN, TGL_PAYMENT, PAYMENT_VIA, TOTAL,COA,KD_MODUL, 'SP2' KET from nota_delivery_h
        ...
        select A.ID_NOTA, A.NO_FAKTUR, A.ID_REQUEST AS ID_REQ, A.CUSTOMER AS EMKL, A.STATUS, A.ALAMAT, B.VESSEL AS VESSEL , ' ' AS VOYAGE_IN, B.VOYAGE AS VOYAGE_OUT, A.TGL_CETAK AS TGL_SIMPAN,A.TGL_PAYMENT, A.PAYMENT_VIA, A.TOTAL, A.COA, '' AS KD_MODUL, 'RXP' KET from NOTA_REEXPORT_H A LEFT JOIN REQ_REEXPORT_H B ON B.ID_REQ=A.ID_REQUEST
        union
        select ID_NOTA, NO_FAKTUR, ID_REQUEST AS ID_REQ, CUSTOMER NM_PEMILIK, STATUS, ALAMAT, VESSEL ,VOYAGE VOYAGE_IN, VOYAGE VOYAGE_OUT, TGL_CETAK TGL_SIMPAN, TGL_PAYMENT, PAYMENT_VIA, TOTAL, COA, '' AS KD_MODUL, 'REEX' KET from NOTA_STACKEXT_H
        union
        select ID_NOTA, NO_FAKTUR, NO_RENAME AS ID_REQ, PBM AS NM_PEMILIK, STATUS, ALAMAT, VESSEL ,'' AS  VOYAGE_IN, VOYAGE AS VOYAGE_OUT, TGL_SIMPAN, TGL_PAYMENT, PAYMENT_VIA, TOTAL, COA, '' AS KD_MODUL, 'RNM' KET from NOTA_RENAME_CONTAINER
        UNION
        select ID_NOTA,ID_NOTA AS NO_FAKTUR ,ID_REQUEST AS ID_REQ, EMKL AS NM_PEMILIK, STATUS, ALAMAT_EMKL AS ALAMAT, VESSEL ,VOYAGE AS VOYAGE_IN, NULL AS VOYAGE_OUT, TGL_CETAK AS TGL_SIMPAN, TGL_PAYMENT, PAYMENT_VIA, TOTAL, COA, '' AS KD_MODUL, 'HICO' KET from NOTA_HICOSCAN_H
        ) z left join tth_nota_all2 y on z.ID_NOTA=y.NO_NOTA
        order by z.TGL_SIMPAN DESC) where rownum < 5000";
	}
	else if($q=='l_transhipment')
	{
		$query="select ID_REQ, SHIPPING_LINE, VESSEL, VOYAGE, NO_UKK, NO_BC_12, NO_SURAT_TRS, VESSEL_DT, VOYAGE_DT, NO_UKK_DT, OI, 	STATUS,
	...
	while ($row = $res->fetchRow()) 
	{
		...
		else if($q == 'payment_cash') 
		{
			...
			{
				$act="<a href='#' onclick='return print(\"$idn\",\"$req\",\"$remark\",\"$tglct\")' title='cetak nota'><img src='images/document-excel.png'></A>";
				$act.="<button title='Sync Payment' onclick='sync_payment(\"$req\",\"$idn\")'><img width=\"15\" height=\"15\" src='images/sync.png'></button>";
			}	
			$vv=$row[VESSEL].' '.$row[VOYAGE_IN].' - '.$row[VOYAGE_OUT];
			$responce->rows[$i]['id']=$row[ID_NOTA];
			$responce->rows[$i]['cell']=array($act,$row[ID_NOTA],$row[NO_FAKTUR],$row[ID_REQ],$row[EMKL],$row[KET],$vv,$row[TGL_SIMPAN],$row[TGL_PAYMENT],$row[PAYMENT_VIA],$row[TOTAL]);
		}
		else if($q == 'l_transhipment') 
		{	
			$vv=$row[VESSEL].' - '.$row[VOYAGE].'<br>'.$row[NO_UKK];
			$vv2=$row[VESSEL_DT].' - '.$row[VOYAGE_DT].'<br>'.$row[NO_UKK_DT];
			if($row[STATUS]=='N' || $row[STATUS]=='S')
			{
				//belum disave
				$act = "<a href=".HOME."request.transhipment/edit_req?no_req=".$row[ID_REQ]."><img src='images/edit.png' title='edit request'></a>";
			}
			else
			{
				//save pre performance
				$act ="<font color='red'><blink><b>sudah dibayar</b></blink></font>";
			}
		...
		{	
			...
			{
				//belum disave
				$act = "<a href=".HOME."request.reexport/edit_req?no_req=".$row[ID_REQ]."><img src='images/edit.png' title='edit request'></a>";
			}
			else
			{
				//save pre performance
