...
ORDER BY cd.NAME,ab.ID_VS,ab.ID_PEL_TUJ,ab.SIZE_,ab.TYPE_  DESC";
	...
	else if($q=='list_cont_st') 
	{
		if($kg=='I')
		{
			$query ="SELECT NO_CONTAINER, TRIM(SIZE_CONT) AS SIZE_, TRIM(TYPE_CONT) AS TYPE_, TRIM(STATUS) AS STATUS, NVL(TRIM(HZ),'N') AS HZ, WEIGHT AS BERAT ,TRIM(POD) AS POD ,TRIM(POL) AS POL,
                    CASE
                        WHEN E_I='I' AND ACTIVITY=NULL THEN '01'
                        WHEN E_I='I' AND ACTIVITY='DISCHARGE' THEN '02'
                        WHEN E_I='I' AND ACTIVITY='PLACEMENT IMPORT' THEN '03'
                        WHEN E_I='I' AND ACTIVITY='GATE OUT DELIVERY' THEN '10'
                        WHEN E_I='I' AND ACTIVITY='GATE IN DELIVERY (TRUCK IN)' THEN '09'
                    END AS KODE_STATUS, 
                    TO_CHAR(TO_DATE(GATE_IN_DATE,'YYYYMMDDHH24MISS'),'DD-MM-YYYY HH24:MI') TGL_GATE_IN,
                    TO_CHAR(TO_DATE(GATE_OUT_DATE,'YYYYMMDDHH24MISS'),'DD-MM-YYYY HH24:MI') TGL_GATE_OUT,
                    TO_CHAR(TO_DATE(YARD_CONFIRM,'YYYYMMDDHH24MISS'),'DD-MM-YYYY HH24:MI')  TGL_PLACEMENT, 
                    YD_BLOCK BLOCK,
                    YD_SLOT SLOT , 
                    YD_ROW ROW_,
                    YD_TIER TIER, 
                    BAYPLAN_POSITION AS BAY,
                    BAYPLAN_POSITION AS BAY_REAL, 
                    TO_CHAR(TO_DATE(vessel_CONFIRM,'YYYYMMDDHH24MISS'),'DD-MM-YYYY HH24:MI') DATE_CONFIRM
                     FROM m_cyc_container
                    WHERE vessel='$ves' 
                    and voyage_in='$vin' and voyage_out= '$vot'
                    and E_I='$kg'
		...
		else
		{
			$query="SELECT NO_CONTAINER, TRIM(SIZE_CONT) AS SIZE_, TRIM(TYPE_CONT) AS TYPE_, TRIM(STATUS) AS STATUS, NVL(TRIM(HZ),'N') AS HZ, WEIGHT AS BERAT ,TRIM(POD) AS POD ,TRIM(POL) AS POL,
                    CASE
                        WHEN E_I='E' AND ACTIVITY='INSPECTION RECEIVING' THEN '50'
                        WHEN E_I='E' AND ACTIVITY='PLACEMENT IMPORT' THEN '51'
                        WHEN E_I='E' AND ACTIVITY='GATE IN RECEIVING' THEN '50'
                        WHEN E_I='E' AND ACTIVITY='GATE OUT RECEIVING' THEN '50'
                        WHEN E_I='E' AND ACTIVITY='LOADING' THEN '56'
                    END AS KODE_STATUS, 
                    TO_CHAR(TO_DATE(GATE_IN_DATE,'YYYYMMDDHH24MISS'),'DD-MM-YYYY HH24:MI') TGL_GATE_IN,
                    TO_CHAR(TO_DATE(GATE_OUT_DATE,'YYYYMMDDHH24MISS'),'DD-MM-YYYY HH24:MI') TGL_GATE_OUT,
                    TO_CHAR(TO_DATE(YARD_CONFIRM,'YYYYMMDDHH24MISS'),'DD-MM-YYYY HH24:MI')  TGL_PLACEMENT, 
		...
		else
		{
			$query="SELECT NO_CONTAINER, TRIM(SIZE_CONT) AS SIZE_, TRIM(TYPE_CONT) AS TYPE_, TRIM(STATUS) AS STATUS, NVL(TRIM(HZ),'N') AS HZ, WEIGHT AS BERAT ,TRIM(POD) AS POD ,TRIM(POL) AS POL,
                    CASE
                        ...
                        WHEN E_I='E' AND ACTIVITY='GATE OUT RECEIVING' THEN '50'
                        WHEN E_I='E' AND ACTIVITY='LOADING' THEN '56'
						WHEN E_I='E' AND ACTIVITY=NULL THEN '49	'
                    END AS KODE_STATUS, 
                    TO_CHAR(TO_DATE(GATE_IN_DATE,'YYYYMMDDHH24MISS'),'DD-MM-YYYY HH24:MI') TGL_GATE_IN,
                    TO_CHAR(TO_DATE(GATE_OUT_DATE,'YYYYMMDDHH24MISS'),'DD-MM-YYYY HH24:MI') TGL_GATE_OUT,
                    TO_CHAR(TO_DATE(YARD_CONFIRM,'YYYYMMDDHH24MISS'),'DD-MM-YYYY HH24:MI')  TGL_PLACEMENT, 
	...
	while ($row = $res->fetchRow()) {
		...
		else if($q == 'list_cont_st') 
		{
			if($kg=='I')
			{
				...
				$responce->rows[$i]['id']=$i;
				$responce->rows[$i]['cell']=array($row[NO_CONTAINER],$kategori,$row[BERAT],$row[POL],$row[POD],$row[KODE_STATUS],$row[BAY],$row[DATE_CONFIRM],$row[TGL_PLACEMENT],$yd,$row[TGL_GATE_IN],$row[TGL_GATE_OUT]);
			}
			else
			{
				$kategori = $row[SIZE_]." ".$row[TYPE_]." ".$row[STATUS]." ".$row[HZ];
				$yd=$row[BLOCK]." - ".$row[SLOT]." - ".$row[ROW_]." - ".$row[TIER];
				$responce->rows[$i]['id']=$i;
				$responce->rows[$i]['cell']=array($row[NO_CONTAINER],$kategori,$row[BERAT],$row[POL],$row[POD],$row[KODE_STATUS],$row[TGL_GATE_IN],$row[TGL_PLACEMENT],$yd,$row[BAY],$row[BAY_REAL],$row[DATE_CONFIRM]);
			}
		}
		else if($q == 'list_cont_sti') 
		{
