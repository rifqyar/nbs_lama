	...
	else if($q=='list_cont_st') 
	{
		...
		else
		{
			$query="SELECT 
                                        ...
                                        a.WEIGHT AS BERAT ,TRIM(a.POD) AS POD ,
                                        TRIM(a.POL) AS POL,
                                        a.BILLING_REQUEST_ID as NO_REQUEST,
                                        b.NO_TRX as NO_NOTA,
                                        case when b.STATUS_PAID = 1 THEN 'LUNAS' ELSE 'BELUM LUNAS' 
                    ...
                    TO_CHAR(TO_DATE(a.vessel_CONFIRM,'YYYYMMDDHH24MISS'),'DD-MM-YYYY HH24:MI') DATE_CONFIRM
                    FROM 
                        m_cyc_container@dbint_link a left join m_billing@dbint_link b
                         on a.billing_request_id = b.no_request and a.vessel_code=b.vessel and a.voyage_in=b.voy_in and A.voyage_out=b.voy_out and a.no_container=b.no_container
                    WHERE 
                        a.vessel='$ves' 
                        and a.voyage_in='$vin' and a.voyage_out= '$vot'
	...
	else if($q=='list_cont_st') //ambil data
	{
		...
		else
		{
			$query="SELECT a.NO_CONTAINER,
                         ...
                         d.WEIGHT_NPE, 
                          c.NO_TRX as NO_NOTA,
                        case when c.STATUS_PAID = 1 THEN 'LUNAS' ELSE 'BELUM LUNAS' 
                        END  as STATUS_PAYMENT
					FROM    m_cyc_container@dbint_link a
						 LEFT JOIN
							req_receiving_h b
						 ON (a.billing_request_id = b.id_req)
                         LEFT JOIN m_billing@dbint_link c
                         on a.billing_request_id = c.no_request and a.vessel_code=c.vessel and a.voyage_in=c.voy_in and A.voyage_out=c.voy_out and a.no_container=c.no_container
                         LEFT JOIN
                            req_receiving_d d 
                         ON (d.no_req_anne = b.id_req and d.no_container = a.no_container)
				   WHERE     a.vessel = '$ves'
						 AND a.voyage_in = '$vin'
	...
	while ($row = $res->fetchRow()) {
		...
		else if($q == 'list_cont_st') 
		{
			if($kg=='I')
			{
                ...
                $req=$row[NO_REQUEST];
                $db1=getDb();
                $querydb="select EMKL, NO_SPPB from req_delivery_h where id_req='$req'";
                $exqr=$db1->query($querydb);
                $drow=$exqr->fetchRow();
                $emkl=$drow[EMKL];
                $nosppb=$drow[NO_SPPB];

				$responce->rows[$i]['id']=$i;
				$responce->rows[$i]['cell']=array($row[NO_CONTAINER],$row[NO_REQUEST],$kategori,$row[BERAT],$row[POL],$row[POD],$row[KODE_STATUS],$row[BAY],$row[DATE_CONFIRM],$row[TGL_PLACEMENT],$yd,$row[TGL_GATE_IN],$row[TGL_GATE_OUT], $row[NO_NOTA], $row[STATUS_PAYMENT], $emkl, $nosppb);
			}
			else
			{
				$kategori = $row[SIZE_]." ".$row[TYPE_]." ".$row[STATUS]." ".$row[HZ];
				$yd=$row[BLOCK]." - ".$row[SLOT]." - ".$row[ROW_]." - ".$row[TIER];
                /*$db1=getDb();
                $querydb="select KODE_PBM, PEB,NPE from req_receiving_h where id_req='$req'";
                $exqr=$db1->query($querydb);
                $drow=$exqr->fetchRow();
                $emkl=$drow[KODE_PBM];
                $peb=$drow[PEB];
                $npe=$drow[NPE];*/
				$responce->rows[$i]['id']=$i;
				$responce->rows[$i]['cell']=array($row[NO_CONTAINER],$row[NO_REQUEST],$kategori,$row[BERAT],$row[POL],$row[POD],$row[KODE_STATUS],$row[TGL_GATE_IN],$row[TGL_PLACEMENT],$yd,$row[BAY],$row[BAY_REAL],$row[DATE_CONFIRM],$row[WEIGHT_NPE],$row[NPE],$row[PEB],$row[KODE_PBM],$row[NO_NOTA], $row[STATUS_PAYMENT]);
			}
		}
		else if($q == 'list_cont_sti') 
		{
