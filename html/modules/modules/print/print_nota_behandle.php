...
$p1 = $_GET['pl'];
/*PTP*/
    $query = "select fc_nm_perusahaan('$p1','BH') NM_PERUSAHAAN from dual";
    // echo $query; die();
    $rq    = $db->query($query)->fetchRow();
    $nm_perusahaan = $rq['NM_PERUSAHAAN'];
/*PTP*/

$query="SELECT A.*,B.*,SYSDATE AS TGL_PROSES,to_char(A.INVOICE_DATE,'DD-MM-YYYY') TGL_NOTA2, to_char(A.ADM_NOTA,'999,999,999,999,990.00') ADM_NOTAN,to_char(A.PPN,'999,999,999,999,999.00') PPNN,
        ...
        FROM BH_NOTA A join BH_REQUEST B on A.ID_NOTA='$p1' AND trim(A.ID_REQUEST)=trim(B.ID_REQUEST) left join tb_user c on  C.ID=A.USER_ID";
