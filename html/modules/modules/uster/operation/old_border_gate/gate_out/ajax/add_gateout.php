...
// IF untuk kondisi DELIVERY KE  LUAR DEPO (bukan TPK)
   ...
   // echo $query_insert;die;
    ...
    $history  = "INSERT INTO history_container(NO_CONTAINER, NO_REQUEST, KEGIATAN, TGL_UPDATE, ID_USER, ID_YARD, STATUS_CONT) 
                                                      VALUES ('$no_cont','$no_req','BORDER GATE OUT',SYSDATE,'$id_user','$id_yard','$status')";
           // echo $history;die;
              $db->query($history);
...
// IF untuk kondisi delivery KE TPK 
...
else if ( ($peralihan <> 'RELOKASI') && ($selisih_tgl == NULL) && ($delivery_ke == 'TPK') ){
