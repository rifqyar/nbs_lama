...
$nota_lunas 	= $db->query("select case when lunas = 'YES' THEN 'OK'
				 ...
				 from nota_stripping where no_request = '$no_req'
				  AND STATUS <> 'BATAL'");
...
//cek apakah container tersebut status strippingnya aktif
	...
	if($row_cek2["JUM"] > 0)
	{
		...
		if($row_cek1["STATUS"] == "OK")
		{
			...
			//update status aktif
			$query_update		= "UPDATE CONTAINER_STRIPPING SET AKTIF = 'T', TGL_REALISASI = SYSDATE, ID_USER_REALISASI = '$id_user', PEMAKAIAN_ALAT = '$alat' WHERE NO_CONTAINER = '$no_cont' AND NO_REQUEST = '$no_req'";
			$query_update_plan	= "UPDATE PLAN_CONTAINER_STRIPPING SET AKTIF = 'T' WHERE NO_CONTAINER = '$no_cont' AND NO_REQUEST = REPLACE('$no_req','S','P')";
			if($db->query($query_update))
			{
				...
				$hist = "INSERT INTO history_container(NO_CONTAINER, NO_REQUEST, KEGIATAN, TGL_UPDATE, ID_USER, ID_YARD, NO_BOOKING, COUNTER, STATUS_CONT, WHY) 
						 VALUES ('$no_cont','$no_req','REALISASI STRIPPING',SYSDATE,'$id_user','$id_yard', '$cur_booking1', '$cur_counter1','MTY','$keterangan')";
				$db->query($hist);
				echo "OK";
			}
		}
