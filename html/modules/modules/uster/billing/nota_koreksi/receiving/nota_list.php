	...
	$db = getDB("storage");
			...
			if((isset($_POST["NO_REQ"])) && ($from == NULL) && ($to == NULL))
		{
			$query_list		= "SELECT a.NO_REQUEST,
								    ...
								    FROM request_receiving a, v_mst_pbm b, container_receiving c, koreksi_nota_receiving d
...
"; }
	...
	{
		$db 		= getDB("storage");
		$query_cek	= "SELECT a.NOTA, b.LUNAS FROM request_receiving a, nota_receiving b WHERE a.NO_REQUEST = b.NO_REQUEST AND a.NO_REQUEST = '$no_req' AND b.NO_NOTA = (SELECT MAX(c.NO_NOTA) FROM nota_receiving c WHERE no_request = '$no_req') ";
                //echo $query_cek;
		...
		if(count($row_cek) > 0)
		{
			...
			$no_nota	= $row_cek[0]["LUNAS"];
			
			if (($row_cek[0]["NOTA"] <> 'Y') && ($row_cek[0]["LUNAS"] == 'NO'))
			{
			...
			else if (($row_cek[0]["NOTA"] == 'Y') && ($row_cek[0]["LUNAS"] == 'YES'))
			{
			...
			else if (($row_cek[0]["NOTA"] == 'Y') && ($row_cek[0]["LUNAS"] == 'NO'))
			{
				echo '<a href="'.HOME.APPID.'.print/print_nota_lunas?no_nota='.$no_nota.'&no_req='.$no_req.'"  target="_blank"><b><i> CETAK ULANG </i></b> </a> <br>';
                               //  echo '<font color="red"><i>SDH LUNAS</i></font>';
			}
