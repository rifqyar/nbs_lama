...
//------------------------	
	 ...
	 //if (($_SESSION["ID_ROLE"] == '1') OR ($_SESSION["ID_ROLE"] == '2')){
            	...
            	if((isset($_POST["NO_REQ"])) && ($from == NULL) && ($to == NULL))
		{
			$query_list		= "SELECT NVL(nota_delivery.LUNAS, 0) LUNAS, NVL(nota_delivery.NO_FAKTUR, '-') NO_NOTA, request_delivery.NO_REQUEST, TO_CHAR( request_delivery.TGL_REQUEST,'dd/mm/yyyy') TGL_REQUEST, TO_DATE(request_delivery.TGL_REQUEST_DELIVERY,'dd/mm/yyyy') TGL_REQUEST_DELIVERY, emkl.NM_PBM AS NAMA_EMKL,  yard_area.NAMA_YARD, COUNT(container_delivery.NO_CONTAINER) JML_CONT, request_delivery.PERP_DARI
                        FROM request_delivery, nota_delivery, KAPAL_CABANG.MST_pbm emkl, yard_area, container_delivery
                     ...
                     GROUP BY  NVL(nota_delivery.LUNAS, 0), NVL(nota_delivery.NO_FAKTUR, '-'),request_delivery.NO_REQUEST, TO_CHAR( request_delivery.TGL_REQUEST,'dd/mm/yyyy'), TO_DATE(request_delivery.TGL_REQUEST_DELIVERY,'dd/mm/yyyy'), emkl.NM_PBM,yard_area.NAMA_YARD,request_delivery.PERP_DARI,request_delivery.TGL_REQUEST
      ...
      /*              if(isset($_POST["cari"]) ) 
            	{   
                        ...
                        ORDER BY REQUEST_DELIVERY.NO_REQUEST DESC";
                                        ...
                                        {
                                                $query_list = "SELECT NVL(NOTA_DELIVERY.LUNAS, 0) LUNAS, REQUEST_DELIVERY.NO_REQUEST, TO_CHAR( REQUEST_DELIVERY.TGL_REQUEST,'dd/mm/yyyy') TGL_REQUEST, TO_DATE(TGL_REQUEST_DELIVERY,'dd/mm/yyyy') TGL_REQUEST_DELIVERY, emkl.NAMA AS NAMA_EMKL, VOYAGE.VOYAGE, VESSEL.NAMA_VESSEL, yard_area.NAMA_YARD
                        FROM REQUEST_DELIVERY, NOTA_DELIVERY,  KAPAL_CABANG.MST_pbm  emkl, VESSEL, VOYAGE, yard_area
                        WHERE  REQUEST_DELIVERY.ID_EMKL = emkl.ID AND emkl.KD_CABANG = '05'
                        AND REQUEST_DELIVERY.ID_YARD = YARD_AREA.ID
                        AND REQUEST_DELIVERY.ID_VOYAGE = VOYAGE.NO_BOOKING 
                        AND VOYAGE.KODE_VESSEL = VESSEL.KODE_VESSEL
                        AND NOTA_DELIVERY.NO_REQUEST(+) = REQUEST_DELIVERY.NO_REQUEST
                ...
                $query_list     = "SELECT request_delivery.*, NVL(nota_delivery.NO_NOTA, '') NO_NOTA, emkl.NAMA AS NAMA_EMKL, vessel.NAMA_VESSEL, voyage.VOYAGE 
                                FROM request_delivery, KAPAL_CABANG.MST_pbm emkl, nota_delivery, vessel, voyage
                                where request_delivery.ID_EMKL = emkl.ID AND emkl.KD_CABANG = '05'
                                and request_delivery.ID_VOYAGE = voyage.NO_BOOKING
                                and voyage.KODE_VESSEL = vessel.KODE_VESSEL
                                and nota_delivery.NO_REQUEST(+) = request_delivery.NO_REQUEST
	...
	function cek_nota($no_req)
	{
		...
		if(count($row_cek) > 0)
		{
			...
			{
				echo '<a href="'.HOME.APPID.'/print_nota?no_req='.$no_req.'&koreksi=N" target="_blank"> Preview Nota </a> ';
			...
			{
				echo '<a href="'.HOME.APPID.'.print/print_nota_lunas?no_req='.$no_req.'" target="_blank" > Cetak Ulang </a> ';
			}
			else if(($row_cek[0]["NOTA"] == 'Y') AND ($row_cek[0]["KOREKSI"] == 'Y'))
			{
				echo '<a href="'.HOME.APPID.'.print/print_nota_lunas?no_req='.$no_req.'" target="_blank" > Cetak Ulang </a> ';	
			...
			{
				echo '<a href="'.HOME.APPID.'/print_nota?no_req='.$no_req.'&koreksi=Y"" target="_blank">Preview Nota</a> ';		
			}
