<?php
	...
	$query_nota	= "SELECT c.NM_PBM AS EMKL,
                   ...
                   FROM REQUEST_STRIPPING b INNER JOIN
                            KAPAL_CABANG.MST_PBM c ON b.KD_CONSIGNEE = c.KD_PBM
	...
	if( $st_nota == 'Y'){
		echo 'sudah di cetak';
		header('Location: '.HOME.APPID.'.print/print_nota_lunas_simple?no_req='.$no_req);
	} else {
...
/*echo "DECLARE
	...
	$total_		  = "SELECT SUM(BIAYA) TOTAL, SUM(PPN) PPN FROM temp_detail_nota_i WHERE  no_request = '$no_req'";
	...
	//debug($no_req);die;
	$tl->assign("url_ins","insert_nota_relokasimty");	
