...
/*echo "DECLARE
	...
	if ($display == 1) {
		...
		if ($koreksi == N) {
		 	$qnota = "select kapal_prod.counter_nota_xp.invoice_num('USTER', 'STRIPPING','05','050', 'NO',$total_bayar, 'IDR',
			...
			if ($rcek_[KD_EMKL] == $row_nota[KD_PBM]) {
				$qnota = "select kapal_prod.counter_nota_xp.cm_invoice_num('USTER', 'STRIPPING','05','050', 'NO',$total_bayar, 'IDR',
		            kapal_prod.counter_nota_xp.group_cust('$kd_pbm')) no_nota from dual";	
			} else {
				$qnota = "select kapal_prod.counter_nota_xp.invoice_num('USTER', 'STRIPPING','05','050', 'NO',$total_bayar, 'IDR',
		            kapal_prod.counter_nota_xp.group_cust('$kd_pbm')) no_nota from dual";		
