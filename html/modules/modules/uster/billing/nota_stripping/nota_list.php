...
//------------------------	
	...
	else
	{
		...
		AND REQUEST_STRIPPING.TGL_REQUEST BETWEEN TRUNC (ADD_MONTHS (SYSDATE, -1), 'MM') AND LAST_DAY (SYSDATE)
	...
	{
		$db 		= getDB("storage");
		$query_cek	= "SELECT NOTA, KOREKSI, NOTA_PNKN, KOREKSI_PNKN FROM REQUEST_STRIPPING WHERE NO_REQUEST = '$no_req'";
        //echo $query_cek;
		...
		if(count($row_cek) > 0)
		{
			...
			$lunas1		= $row_cek["KOREKSI_PNKN"];
				...
				{
					echo '<a href="'.HOME.APPID.'/preview_nota_simple?no_req='.$no_req.'&n=999&koreksi=N" target="_blank"> <b><i> Preview Nota Stripping</i></b></a> <br/>';
					echo '<a href="'.HOME.APPID.'/preview_nota_relokmty?no_req='.$no_req.'&n=999&koreksi=N" target="_blank"> <b><i> Preview Nota Relokasi MTY</i></b></a> ';
				...
				{
					echo '<a href="'.HOME.APPID.'.print/preview_nota_simple?no_req='.$no_req.'" target="_blank"><b><i> Cetak Ulang Nota Stripping </i></b></a> <br>';
					echo '<a href="'.HOME.APPID.'./preview_nota_relokmty?no_req='.$no_req.'&n=999&koreksi=N" target="_blank"> <b><i> Preview Nota Relokasi MTY</i></b></a> ';
				}
				else if(($row_cek["NOTA"] <> 'Y') AND ($row_cek["KOREKSI"] <> 'Y') AND ($row_cek["NOTA_PNKN"] == 'Y') AND ($row_cek["KOREKSI_PNKN"] <> 'Y'))
				{
					echo '<a href="'.HOME.APPID.'/preview_nota_simple?no_req='.$no_req.'&n=999&koreksi=N" target="_blank"> <b><i> Preview Nota Stripping</i></b></a> <br/>';
					echo '<a href="'.HOME.APPID.'.print/print_nota_lunas_simple_relokasi?no_req='.$no_req.'" target="_blank"><b><i> Cetak Ulang Nota Relokasi MTY </i></b></a> '; 
				}
				else if(($row_cek["NOTA"] == 'Y') AND ($row_cek["KOREKSI"] <> 'Y') AND ($row_cek["NOTA_PNKN"] == 'Y') AND ($row_cek["KOREKSI_PNKN"] <> 'Y')  )
				{
					echo '<a href="'.HOME.APPID.'.print/print_nota_lunas_simple?no_req='.$no_req.'" target="_blank"><b><i> Cetak Ulang Nota Stripping </i></b></a> <br/>';
					echo '<a href="'.HOME.APPID.'.print/print_nota_lunas_simple_relokasi?no_req='.$no_req.'" target="_blank"><b><i> Cetak Ulang Nota Relokasi MTY </i></b></a> '; 
				}
				else if(($row_cek["NOTA"] <> 'Y') AND ($row_cek["KOREKSI"] == 'Y') AND ($row_cek["NOTA_PNKN"] <> 'Y') AND ($row_cek["KOREKSI_PNKN"] == 'Y')  )
				{
					echo '<a href="'.HOME.APPID.'/preview_nota_simple?no_req='.$no_req.'&n=999&koreksi=Y" target="_blank"> <b><i> Preview Nota Stripping</i></b></a> <br/>';
					echo '<a href="'.HOME.APPID.'/preview_nota_relokmty?no_req='.$no_req.'&n=999&koreksi=Y" target="_blank"> <b><i> Preview Nota Relokasi MTY</i></b></a>';
				}
				else if(($row_cek["NOTA"] <> 'Y') AND ($row_cek["KOREKSI"] == 'Y') AND ($row_cek["NOTA_PNKN"] <> 'Y') AND ($row_cek["KOREKSI_PNKN"] <> 'Y')  )
				{
					echo '<a href="'.HOME.APPID.'/preview_nota_simple?no_req='.$no_req.'&n=999&koreksi=Y" target="_blank"> <b><i> Preview Nota Stripping</i></b></a> <br/>';
			...
			{
				$q_cek_relokasi = $db->query("SELECT COUNT(*) JUM FROM NOTA_RELOKASI_MTY WHERE NO_REQUEST='$no_req' AND STATUS='BATAL'");
				$r_cek_relokasi	= $q_cek_relokasi->fetchRow();
				$rwcek 			= $r_cek_relokasi[JUM];
				if($rwcek == 0){
						echo '<a> <b><i> Nota Relokasi Belum Batal</i></b></a> <br/>';
				}
				else {
					echo '<a href="'.HOME.APPID.'/preview_nota_simple?no_req='.$no_req.'&n=999&koreksi=Y" target="_blank"> <b><i> Preview Nota</i></b></a> <br/>';
				}
			}*/
