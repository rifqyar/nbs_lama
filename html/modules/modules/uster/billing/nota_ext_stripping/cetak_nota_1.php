...
if($nota == 999)
{
	...
	$no_faktur	= '010.010.-'.$year.'.'.$jum;
        ...
        $cek_delivery   = "SELECT DELIVERY_KE, PERALIHAN FROM request_delivery WHERE NO_REQUEST = '$no_req'";
...
//	$peralihan	= $cek["PERALIHAN"];
	...
	$peralihan	= $cek["PERALIHAN"];
      ...
      //  } else if (($delivery_ke == 'TPK') && ($peralihan == 'T')) {
...
//                    $tgl_req_out     = $rows[$i]['TGL_REQ_OUT'];
                    ...
                    $tarif1     = " SELECT a.TARIF, b.ID_ISO 
                                        ...
                                        AND c.KATEGORI_TARIF = 'PENUMPUKAN'
                                        AND b.SIZE_ = '$size' 
                                        AND b.TYPE_ = '$type'
                                        AND b.STATUS = '$status'
                                        AND TO_DATE('".$tgl_placement."','dd/mon/yy') BETWEEN c.START_PERIOD AND c.END_PERIOD";
                   // echo $tarif1; die;
                     ...
                     $jml_hari1  = " SELECT TO_DATE('".$tgl_req_out."', 'dd/mm/yyyy') - (TO_DATE('".$tgl_placement."','dd/mon/yy')+4) jumlah_hari FROM dual";
                   //  echo $jml_hari1;
...
//                     echo 'biaya masa2 '.$biayamasa2;die;
                     ...
                     if (($masa12 <> 0) && ($masa2 <> 0)){
                         $endstack2    = "SELECT TO_DATE('".$tgl_placement."','dd/mm/yyyy')+4+$masa12 ENDSTACK1 FROM DUAL";
                       ...
                       //  die;
                         ...
                         $endstack1    = "SELECT TO_DATE('".$tgl_placement."','dd/mm/yyyy')+$masa12 ENDSTACK1 FROM DUAL";
                         $end_stack_   = $db->query($endstack1);
                         $row_         = $end_stack_->fetchRow();
                         $end_stack1   = $row_['ENDSTACK1'];
                         
                         $query1 = "INSERT INTO nota_delivery_d (ID_ISO, TARIF, BIAYA, KETERANGAN, ID_NOTA, JML_CONT, HZ, START_STACK, END_STACK, JML_HARI) VALUES ('$id_iso','$tarifmasa12','$biayamasa12','PENUMPUKAN MASA I.2','$no_nota','$jml_cont','$hz',TO_DATE('".$tgl_placement."','dd/mm/yyyy')+5,TO_DATE('".$end_stack1."','dd/mm/yyyy'), TO_DATE('".$end_stack1."','dd/mm/yyyy')-TO_DATE('".$tgl_placement."','dd/mm/yyyy')+1)";
//                          echo $query1;
//                         die;
                         $db->query($query1);
                     } 
                ...
                $lolo     = " select b.id_group_tarif, count(b.id_group_tarif) jml_cont, e.id_iso, d.tarif, e.size_, e.type_, e.status, TO_DATE(c.tgl_placement)+4 TGL_START_NOTA
                                        ...
                                        and TO_DATE(c.tgl_placement)+4 between b.START_PERIOD and b.END_PERIOD 
