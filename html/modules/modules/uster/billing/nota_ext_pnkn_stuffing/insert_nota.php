...
$koreksi = $_GET["koreksi"];
	...
	*/
	...
	$no_nota	= "0705".$month.$year.$jum;
        ...
        //select master pbm
        $query_master	= "SELECT b.KD_PBM, b.NM_PBM, b.ALMT_PBM, b.NO_NPWP_PBM FROM request_stuffing a, KAPAL_CABANG.MST_PBM b WHERE a.KD_CONSIGNEE = b.KD_PBM AND b.KD_CABANG = '05' AND a.NO_REQUEST = '$no_req'";
	...
	$query_nota_all			= "INSERT INTO NOTA_ALL_H
		...
		$update = "UPDATE NOTA_STUFFING SET STATUS = 'BATAL' WHERE NO_NOTA = '$nota_lama_'";
		$db->query($update);
	...
	if($db->query($query_insert_nota))
	{
		...
		$row		= $res->getAll();
			...
			$ppn_d   = $item['PPN'];
                ...
                if ($rtpk_flag[FLAG] == 'Y') {
							$sql_xpi = "BEGIN
							  USTER.ITPK_POPULATE_STAGING.INSERT_NOTA_ITPK ( '$no_req', 'UST07', '$faktur', '$no_nota', '$koreksi', '$nipp' ); 
							END; ";
              				 // echo $sql_xpi;die;
							$db->query($sql_xpi);
				} 
...
{		//echo HOME;
		header('Location:'.HOME.APPID.'/print_nota_lunas?no_nota='.$no_nota);		
