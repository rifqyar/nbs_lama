...
//------------------------	
	...
	else
	{
		...
		$query_list		= "SELECT REQUEST_RECEIVING.*, 
								  emkl.NAMA AS NAMA_EMKL, 
								  pnmt.NAMA AS NAMA_PNMT 
						   FROM REQUEST_RECEIVING 
						   INNER JOIN MASTER_PBM emkl ON REQUEST_RECEIVING.ID_EMKL = emkl.ID 
						   JOIN MASTER_PBM pnmt ON REQUEST_RECEIVING.ID_PEMILIK = pnmt.ID 
						   WHERE REQUEST_RECEIVING.TGL_REQUEST BETWEEN TRUNC(SYSDATE,'MONTH') AND LAST_DAY(SYSDATE) 
						   ORDER BY REQUEST_RECEIVING.NO_REQUEST DESC";
