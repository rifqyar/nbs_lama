<?php
	...
	$rec_dari	= "TPK";//$_POST ["REC_DARI"];
	$no_request = $_POST["NO_REQ"];
	$no_do		= $_POST["NO_DO"];
	$no_bl		= $_POST["NO_BL"];
	$no_sppb	= $_POST["NO_SPPB"];
	$tgl_sppb	= $_POST["TGL_SPPB"];	
	$kd_consignee	= $_POST["KD_PENUMPUKAN_OLEH"];
	//$kd_penumpukan_oleh	= $_POST["KD_PENUMPUKAN_OLEH"];
	$nm_consignee	= $_POST["PENUMPUKAN_OLEH"];
//	$nm_penumpukan	= $_POST["PENUMPUKAN_OLEH"];
	$voyage		= $_POST["VOYAGE"];
	$no_booking	= $_POST["NO_BOOKING"];	
	...
	$no_req2	= substr($no_request,3);	
	$no_req2	= "UREQ".$no_req2;	
	$db = getDB("storage");
	...
	$query_update	= "UPDATE REQUEST_RECEIVING 
					   SET KD_CONSIGNEE 		= '$kd_consignee',
							KD_PENUMPUKAN_OLEH	= '$kd_consignee',
