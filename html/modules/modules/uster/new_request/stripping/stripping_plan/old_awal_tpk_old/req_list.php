...
//------------------------	
	...
	if(isset($_POST["CARI"]))
	{
		$query_list		= " SELECT a.*, 
                           ...
                           AND a.TGL_REQUEST BETWEEN TO_DATE ( '$from', 'YYYY-MM-DD ')
												 ...
												 AND a.NO_REQUEST_RECEIVING IS NOT NULL
	...
	{
		$query_list		= "SELECT REQUEST_STRIPPING.*, NVL(REQUEST_STRIPPING.NO_REQUEST_RECEIVING,' - ') AS REQ_REC, emkl.NAMA AS NAMA_EMKL FROM REQUEST_STRIPPING INNER JOIN MASTER_PBM emkl ON REQUEST_STRIPPING.ID_PEMILIK = emkl.ID WHERE REQUEST_STRIPPING.TGL_REQUEST BETWEEN TRUNC(SYSDATE,'MONTH') AND LAST_DAY(SYSDATE) AND REQUEST_STRIPPING.ID_YARD = '$id_yard' AND REQUEST_STRIPPING.NO_REQUEST_RECEIVING IS NOT NULL ORDER BY REQUEST_STRIPPING.NO_REQUEST DESC";
	}
