...
//	debug ($_POST);die;		
	...
	$sqlcekclosing = " SELECT
					 (SELECT TO_DATE (TGL_JAM_CLOSE,'DD-MM-YYYY HH24:MI:SS') TGL_JAM_CLOSE  FROM PETIKEMAS_CABANG.TTH_CONT_BOOKING WHERE NO_BOOKING = 'BSH051200009')- 
					 (SELECT TO_DATE (SYSDATE,'DD-MM-YYYY HH24:MI:SS')HARINI FROM DUAL) A
					  FROM DUAL";
			...
			if($db->query($sqlict_del)){ 		//eksekusi auto request delivery ICT
				if($db->query($query_req_rec)){ // eksekusi auto request receiving USTER
					...
					{	
						...
						$no_req_s		= "STF".$month.$year.$no_req;
