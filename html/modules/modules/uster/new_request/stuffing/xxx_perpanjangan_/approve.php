...
if($asalcontstuff == "TPK"){
	...
	$query_cont_rec_uster	= "INSERT INTO CONTAINER_RECEIVING(NO_CONTAINER, 
		...
		$history_rec        = "INSERT INTO history_container(NO_CONTAINER, NO_REQUEST, KEGIATAN, TGL_UPDATE, ID_USER, ID_YARD, NO_BOOKING, COUNTER) 
								VALUES ('$no_cont','$no_req_rec','REQUEST RECEIVING',SYSDATE,'$id_user', '$id_yard', '$cur_booking1', '$cur_counter1')";
	//end history
	...
	if(count($row_cek_request) > 0 && count($row_cek_cont) > 0){ //jika request telah ada dan container telah ada
		...
		foreach($row_c as $rc){
			...
			$query_ic	= "INSERT INTO CONTAINER_STUFFING (NO_CONTAINER, NO_REQUEST, AKTIF, HZ, COMMODITY, TYPE_STUFFING, START_STACK,
							ASAL_CONT, NO_SEAL, BERAT, KETERANGAN, TGL_MULAI, STATUS_REQ, TGL_APPROVE, TGL_GATE)
							VALUES('$cont',
							'$no_req_stuf',
							'$aktif',
							'$hz',
							'$comm',
							'$type_st',
							to_date('".$start_stack."','mm-dd-yyyy'),
							'$asal',
							'$seal',
							'$berat',
							'$keterangan',
	...
	$query_del_uster  = "INSERT INTO CONTAINER_DELIVERY(NO_CONTAINER, NO_REQUEST, STATUS, AKTIF, 
						  ...
						  '$no_request', TO_DATE('$start_stack','mm-dd-yyyy'), '$asal_cont')";
	...
	$history_del        = "INSERT INTO history_container(NO_CONTAINER, NO_REQUEST, KEGIATAN, TGL_UPDATE, ID_USER, ID_YARD, NO_BOOKING, COUNTER) 
														  VALUES ('$no_cont','$no_req_stuf','REQUEST STUFFING',SYSDATE,'$id_user', '$id_yard', '$cur_booking3', '$cur_counter3')";
	...
	if($db->query($query_del_uster)){
				...
				$history_delivery        = "INSERT INTO history_container(NO_CONTAINER, NO_REQUEST, KEGIATAN, TGL_UPDATE, ID_USER, ID_YARD, NO_BOOKING, COUNTER) 
											VALUES ('$no_cont','$no_req_del','REQUEST DELIVERY',SYSDATE,'$id_user', '$id_yard', '$cur_booking3', '$cur_counter3')";
