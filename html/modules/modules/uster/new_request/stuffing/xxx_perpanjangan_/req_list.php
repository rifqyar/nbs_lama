...
//------------------------	
	...
	$jumlah_req = "SELECT COUNT(NO_REQUEST) JUMLAH FROM PLAN_REQUEST_STUFFING WHERE REPLACE(NO_REQUEST,'P','S') NOT IN (SELECT NO_REQUEST FROM REQUEST_STUFFING)";
	$result_list2	= $db->query($jumlah_req);
	$jumlah_			= $result_list2->fetchRow(); 
	
	$jumlah       = $jumlah_['JUMLAH'];
	
	$total_req = "SELECT COUNT(NO_REQUEST) TOTAL FROM REQUEST_STUFFING WHERE TGL_REQUEST = SYSDATE";
