<?php	
	...
	// $sqlcek3 = "SELECT TO_CHAR (TGL_JAM_CLOSE-2,'YYYY-MM-DD HH24:MI:SS')TGL_JAM_CLOSE  FROM V_BOOKING_STACK_TPK WHERE NO_BOOKING = '$NO_BOOKING'";
	...
	// SELECT LPAD(NVL(MAX(SUBSTR(NO_REQUEST,8,13)),0)+1,6,0) AS JUM ,
	  $query_cek_del	= "SELECT LPAD(NVL(MAX(SUBSTR(NO_REQUEST,8,13)),0)+1,6,0) AS JUM , 
							  TO_CHAR(SYSDATE, 'MM') AS MONTH, 
							  TO_CHAR(SYSDATE, 'YY') AS YEAR 
...
//END CARI NO_REQ_DEL
	...
	//Insert Ke Header ICT Request Receiving ICT == update terbaru, tidak auto delivery ke tpk
	...
	//Insert ke header request stuffing 
			...
			if($db2->query($sqlict_del)){ 		//eksekusi auto request delivery ICT
				if($db->query($query_req_rec)){ // eksekusi auto request receiving USTER
					...
					{	
						...
						$no_req_p		= "STF".$month.$year.$no_req;
						...
						$query_ir = "INSERT INTO REQUEST_STUFFING(NO_REQUEST, ID_YARD, CETAK_KARTU_SPPS, KETERANGAN, NO_BOOKING,
										TGL_REQUEST, NO_DOKUMEN, NO_JPB, 
										BPRP, ID_PEMILIK, ID_EMKL, NO_REQUEST_RECEIVING, ID_USER,
										NO_REQUEST_DELIVERY, KD_CONSIGNEE, KD_PENUMPUKAN_OLEH, NM_KAPAL, NO_PEB, NO_NPE, VOYAGE, STUFFING_DARI, NOTA,
										NO_DO, NO_BL,ID_PENUMPUKAN) 
										VALUES('$no_req_p',
										'$id_yard',
										0,
										'$KETERANGAN',
										...
										'$NPE',
										'$VOYAGE_IN',
										'TPK','T','$NO_DO','$NO_BL','$ID_PNKN_BY')";
						
						//echo $query_req_s;exit;
						//Insert ke header request delivery,;  update terbaru, tidak auto delivery ke tpk
						...
						if(($db->query($query_req_s)) AND ($db->query($query_ir)))
						{
