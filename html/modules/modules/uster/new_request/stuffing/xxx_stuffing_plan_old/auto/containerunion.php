...
$result =   /* " SELECT MASTER_CONTAINER.NO_CONTAINER NO_CONTAINER, MASTER_CONTAINER.SIZE_ KD_SIZE, MASTER_CONTAINER.TYPE_ KD_TYPE, 
			...
			"
			SELECT DISTINCT MASTER_CONTAINER.NO_CONTAINER, MASTER_CONTAINER.SIZE_ KD_SIZE, MASTER_CONTAINER.TYPE_ KD_TYPE,
						'' TGL_BONGKAR, HISTORY_CONTAINER.STATUS_CONT, '' BP_ID, '' NO_UKK, 'DEPO' ASAL_CONT, blocking_area.NAME BLOK_, placement.ROW_ ROW_, placement.SLOT_ SLOT_, placement.TIER_ TIER_,
						'' VOYAGE_IN, MASTER_CONTAINER.NO_BOOKING NM_KAPAL, '' NM_AGEN, '' TGL_STACK
			FROM MASTER_CONTAINER
			INNER JOIN HISTORY_CONTAINER
			ON MASTER_CONTAINER.NO_CONTAINER = HISTORY_CONTAINER.NO_CONTAINER
			AND HISTORY_CONTAINER.TGL_UPDATE = (SELECT MAX(HISTORY_CONTAINER.TGL_UPDATE) FROM HISTORY_CONTAINER WHERE NO_CONTAINER = '$no_cont' )
