...
//------------------------	
	...
	if(isset($_POST["CARI"]))
	{
		if((isset($_POST["NO_REQ"])) && ($from == NULL) && ($to == NULL))
		{
			$query_list = "SELECT * FROM (SELECT PLAN_REQUEST_STUFFING.TGL_REQUEST, PLAN_REQUEST_STUFFING.APPROVE,PLAN_REQUEST_STUFFING.NO_REQUEST,
							...
							WHERE PLAN_REQUEST_STUFFING.NO_REQUEST LIKE '%$no_req%'
	...
	{
		$query_list		= "SELECT * 
								FROM (SELECT 	PLAN_REQUEST_STUFFING.TGL_REQUEST, PLAN_REQUEST_STUFFING.APPROVE,PLAN_REQUEST_STUFFING.NO_REQUEST,
												PLAN_REQUEST_STUFFING.NO_PEB, PLAN_REQUEST_STUFFING.NO_NPE,PLAN_REQUEST_STUFFING.NM_KAPAL, PLAN_REQUEST_STUFFING.VOYAGE,
												NVL (REQUEST_STUFFING.NO_REQUEST, 'blm di approve') NO_REQUEST_APP,
												REQUEST_STUFFING.NOTA, REQUEST_STUFFING.KOREKSI, emkl.NM_PBM AS NAMA_PEMILIK
										FROM PLAN_REQUEST_STUFFING 
												LEFT JOIN 
												REQUEST_STUFFING ON  PLAN_REQUEST_STUFFING.NO_REQUEST =  REPLACE(REQUEST_STUFFING.NO_REQUEST,'S', 'P')
												LEFT JOIN  
												V_MST_PBM emkl ON PLAN_REQUEST_STUFFING.KD_CONSIGNEE = emkl.KD_PBM
												LEFT JOIN 
												PLAN_CONTAINER_STUFFING ON PLAN_REQUEST_STUFFING.NO_REQUEST = PLAN_CONTAINER_STUFFING.NO_REQUEST
										WHERE PLAN_REQUEST_STUFFING.EARLY_STUFFING ='Y'
										GROUP 	BY PLAN_REQUEST_STUFFING.APPROVE, PLAN_REQUEST_STUFFING.NO_REQUEST,
												PLAN_REQUEST_STUFFING.NO_PEB, PLAN_REQUEST_STUFFING.NO_NPE,PLAN_REQUEST_STUFFING.NM_KAPAL, PLAN_REQUEST_STUFFING.VOYAGE,
												NVL (REQUEST_STUFFING.NO_REQUEST, 'blm di approve'), REQUEST_STUFFING.NOTA, REQUEST_STUFFING.KOREKSI, emkl.NM_PBM, PLAN_REQUEST_STUFFING.TGL_REQUEST,
												PLAN_REQUEST_STUFFING.KD_CONSIGNEE
										ORDER BY PLAN_REQUEST_STUFFING.TGL_REQUEST DESC)";
	...
	$jumlah_req = "SELECT COUNT(NO_REQUEST) JUMLAH 
					FROM PLAN_REQUEST_STUFFING 
					WHERE REPLACE(NO_REQUEST,'P','S') NOT IN (SELECT NO_REQUEST FROM REQUEST_STUFFING )
					AND EARLY_STUFFING='Y'";
	$result_list2	= $db->query($jumlah_req);
	$jumlah_			= $result_list2->fetchRow(); 
	
	$jumlah       = $jumlah_['JUMLAH'];
	
	$total_req = "SELECT COUNT(NO_REQUEST) TOTAL FROM REQUEST_STUFFING WHERE TGL_REQUEST = SYSDATE AND EARLY_STUFFING='Y' ";
	...
	{
		$db 		= getDB("storage");
		$query_cek	= "SELECT NOTA, KOREKSI FROM request_stuffing WHERE NO_REQUEST = REPLACE ('$no_req','P', 'S')";	
		...
		$jum_cont = $r_cek_count["jumlah_cont"];
			...
			{
				echo '<a href="'.HOME.APPID.'/view?no_nota='.$no_nota.'" target="_blank"><blink style="color:red"><b> EDIT </b></blink> </a> ';
