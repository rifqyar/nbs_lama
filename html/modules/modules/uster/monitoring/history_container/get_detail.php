	...
	if($act == "handling"){
		if($no_booking == "VESSEL_NOTHING"){
			$q_detail = "SELECT MC.NO_CONTAINER, HC.STATUS_CONT, HC.ID_YARD ,YAR.NAMA_YARD, HC.KEGIATAN, 
                ...
                END TGL_UPDATE, HC.NO_BOOKING NO_BOOKING, HC.ID_USER, MU.NAMA_LENGKAP, RR.NO_REQUEST NO_REQ_REC, 
		...
		$q_get_req_str = "SELECT HC.NO_REQUEST FROM HISTORY_CONTAINER HC
				WHERE HC.NO_CONTAINER = '$no_cont' AND HC.NO_BOOKING = '$no_booking' AND HC.KEGIATAN = 'REQUEST STRIPPING'";
		...
		$q_strip = "SELECT DISTINCT REQUEST_STRIPPING.NO_REQUEST, 
				...
				END LUNAS
                FROM REQUEST_STRIPPING INNER JOIN KAPAL_CABANG.MST_PBM emkl ON REQUEST_STRIPPING.KD_CONSIGNEE = emkl.KD_PBM AND emkl.KD_CABANG = '05'
                JOIN KAPAL_CABANG.MST_PBM pnmt ON REQUEST_STRIPPING.KD_PENUMPUKAN_OLEH = pnmt.KD_PBM AND pnmt.KD_CABANG = '05'
                ...
                LEFT JOIN MASTER_USER MUS ON CONTAINER_STRIPPING.ID_USER_REALISASI = MUS.ID
                ...
                WHERE CONTAINER_STRIPPING.NO_CONTAINER = '$no_cont' AND HC.NO_BOOKING = '$no_booking'
		...
		$q_get_req_rec = "SELECT HC.NO_REQUEST FROM HISTORY_CONTAINER HC
			...
			$q_get_req_rec = "SELECT HC.NO_REQUEST FROM HISTORY_CONTAINER HC
				WHERE HC.NO_CONTAINER = '$no_cont' AND HC.NO_BOOKING = '$no_booking' AND HC.KEGIATAN = 'REQUEST RECEIVING'";
		}
		...
		if($no_booking != "VESSEL_NOTHING"){
			$q_rec = "SELECT DISTINCT a.NO_REQUEST AS NO_REQUEST,
								  ...
								  A.NO_DO, A.NO_BL, A.NO_SPPB, A.TGL_SPPB, cr.AKTIF,
								  ...
								  CASE WHEN A.PERALIHAN = 'STRIPPING' OR A.PERALIHAN = 'STUFFING'
								  THEN 'AUTO_RECEIVE'
								  ELSE CASE WHEN STATUS = 'BATAL' THEN 'BATAL' ELSE nr.LUNAS END
								  END LUNAS,
								  cr.STATUS_REQ
		...
		$q_get_req_stu = "SELECT HC.NO_REQUEST FROM HISTORY_CONTAINER HC
				WHERE HC.NO_CONTAINER = '$no_cont' AND HC.NO_BOOKING = '$no_booking' AND HC.KEGIATAN = 'REQUEST STUFFING'";
		...
		if($no_booking == "VESSEL_NOTHING"){
			...
			$q_get_req_del = "SELECT HC.NO_REQUEST FROM HISTORY_CONTAINER HC
				WHERE HC.NO_CONTAINER = '$no_cont' AND HC.NO_BOOKING = '$no_booking' AND HC.KEGIATAN = 'REQUEST DELIVERY'";
			$add_q = "";
		}
		...
		$q_del = " SELECT distinct 
                    ...
                    LEFT JOIN kapal_cabang.mst_pbm emkl ON REQUEST_DELIVERY.KD_EMKL = emkl.KD_PBM and emkl.kd_cabang = '05'
                    ...
                    WHERE CONTAINER_DELIVERY.NO_CONTAINER = '$no_cont' AND history_container.no_booking = '$no_booking' ".$add_q." 
		...
		$q_placement = "SELECT CEK.*,  CASE WHEN CEK.INSERT_VIA = 'H' THEN 'HANDHELD' ELSE 'DESKTOP' END VIA FROM (SELECT HISTORY_PLACEMENT.*, MASTER_USER.NAMA_LENGKAP, BLOCKING_AREA.NAME, YARD_AREA.NAMA_YARD FROM 
                        ...
                        LEFT JOIN MASTER_USER ON (HISTORY_PLACEMENT.NIPP_USER = TO_CHAR(MASTER_USER.ID) OR HISTORY_PLACEMENT.NIPP_USER = MASTER_USER.NIPP) WHERE NO_CONTAINER = '$no_cont'
                        AND HISTORY_PLACEMENT.INSERT_VIA IS NULL
                        UNION ALL
                        SELECT HISTORY_PLACEMENT.*, MASTER_USER.NAMA_LENGKAP, BLOCKING_AREA.NAME, YARD_AREA.NAMA_YARD FROM 
                        HISTORY_PLACEMENT INNER JOIN BLOCKING_AREA
                        ON HISTORY_PLACEMENT.ID_BLOCKING_AREA = BLOCKING_AREA.ID
                        LEFT JOIN YARD_AREA ON BLOCKING_AREA.ID_YARD_AREA = YARD_AREA.ID
                        LEFT JOIN MASTER_USER ON HISTORY_PLACEMENT.NIPP_USER = MASTER_USER.ID WHERE NO_CONTAINER = '$no_cont'
