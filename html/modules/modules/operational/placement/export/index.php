...
<script type="text/javascript">
	...
	function preview_blok(block_id,slot_id,row_id,tier_id,id_jobslip)
	{
		...
		$('#load_table_cont').load("<?=HOME?>operational.placement.export.table/find_cont?no_cont="+my_cont);
	...
	function koreksi_placement(block_id,slot_id,row_id,tier_id,id_jobslip)
	{
		var e = document.getElementById("id_alat");
		var id_alat = e.options[e.selectedIndex].value;
		var id_user = document.getElementById('nama_yoa').value;
		...
		$('#dialog_table_cont').load("<?=HOME?>operational.placement.export.correction/koreksi?block_id="+block_id+"&slot_id="+slot_id+"&row_id="+row_id+"&yard_id=23&jobslip="+id_jobslip+"&tier_id="+tier_id).dialog({modal:true, height:600,width:600, buttons: { "close": function() 
			{ 
				$(this).dialog("close"); 
	...
	$(document).ready(function() 
	{
        ...
        $( "#NO_CONTAINER" ).autocomplete({
			minLength: 3,
			source: "operational.placement.export.auto/parameter",
			...
			{
				$( "#NO_CONTAINER" ).val( ui.item.NO_CONT);
				my_cont[i]="'"+ui.item.NO_CONT+"'";
		...
		.data( "autocomplete" )._renderItem = function( ul, item ) 
		{
			...
			.append( "<a>" + item.NO_CONT + " <br> " + item.SIZE_ + " / "+item.TYPE_+" / "+item.STATUS_+" / "+item.BERAT+"<br>"+item.VESSEL+" / "+item.VOYAGE+" </a>")
			.appendTo( ul );
...
<html lang="en">
	...
	<span class="graybrown"><img src='images/cont_placement.png' align="absmiddle" width="80" border='0' class="icon"/>
		<font color="#DE7E21"> Form Multi Placement After Received </font><font color="#0066CC"> [ EXPORT ]</font></span>
	<br>
	<br>
	...
	<tr>
	<td>
	<!--<fieldset style="border: 1px solid #94969b;-moz-border-radius:5px;width:430px;" id="bwh_layout">
			<legend align="center"><font color='#0482d5'>Preview Blok Plan</font></legend>
			<br>
			<div id='load_preview' align="center"></div>
			<BR>
			&nbsp;
			<BR>
			&nbsp;			
