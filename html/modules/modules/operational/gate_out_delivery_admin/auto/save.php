...
$tgl_go		= $_POST['TGL_GATE_OUT'];
$jam_go		= $_POST['JAM_GATE_OUT'];
$menit_go	= $_POST['MENIT_GATE_OUT'];

$gate_out_time = $tgl_go." ".$jam_go.":".$menit_go.":00";		
...
else
{
		...
		$sql_erp = "begin gateout_delivery_erp_admin('$no_ukk','$no_cont','$user','$no_seal','$gate_out_time','$nm_user'); end;";
		$db->query($sql_erp);	
		...
		$update_ict	= "update ttd_bp_cont@prodlinkx set
						gate_out = '$nm_user',
						gate_out_date = TO_DATE('$gate_out_time','DD/MM/YYYY HH24:MI:SS'),
						ht_no = '$no_pols',
						status_cont = '09'
						where trim(bp_id) = trim('$bp_id') and trim(cont_no_bp) = trim('$no_cont')";
		$db->query($update_ict);
		/*$sql_ict = "begin gateout_delivery_ict('$no_ukk','$no_cont'); end;";
		$db->query($sql_ict);*/
		//========= PATCH ICT ============//
