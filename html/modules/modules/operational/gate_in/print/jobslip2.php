<?php
	$db=getDb();
	$js=$_POST['ID_JS'];
	$rq=$db->query("SELECT NO_CONT, SIZE_, TYPE_, STATUS_,BERAT, VESSEL,
       VOYAGE,
       SIZE_,
       TYPE_,
       STATUS_,
       NAMA_BLOCK,
        CASE
          WHEN SIZE_ = 40 THEN CONCAT(CONCAT (SLOT_, '/'), SLOT_ + 1)
          WHEN SIZE_ = 45 THEN CONCAT(CONCAT (SLOT_,'/'), SLOT_ + 1)
          ELSE CONCAT(SLOT_,'')
       END
          SLOT_,
       BERAT,
       ID_PEL_TUJ
  FROM tb_cont_jobslip
 WHERE ID_JOB_SLIP = '$js'");
	$req=$rq->fetchRow();
	?>
