<?php
    ...
    $query        = "SELECT MAX(a.SLOT_)+1 JML_SLOT, MAX(a.ROW_)+1 JML_ROW, b.NAME,b.POSISI, b.ID FROM YD_BLOCKING_CELL a, YD_BLOCKING_AREA b WHERE a.ID_BLOCKING_AREA = b.ID AND a.ID_BLOCKING_AREA = '$id_blok' AND b.ID_YARD_AREA='$yard_id' GROUP BY b.NAME, b.ID,b.POSISI";
				...
				<ol id="selectable">
					...
					<?
						...
						//	$c="ui-plus-default";
							//$d     = 'r';
							$l++;
						} else if (($i > $width) AND ($i % $width <> 1)){
						
								$d     = '';
								//$c     = '';
								if($cellx[$p-1]['SIZE_PLAN_ALLO']<>'')
								{
											$slot_ = $cellx[$p-1]['SLOT_'];
											$rew   = $cellx[$p-1]['ROW_'];
										...
										if(($cellx[$p-1]['SIZE_PLAN_PLC']=='20') OR ($cellx[$p-1]['SIZE_PLAN_PLC']=='40d')){
										...
										} else if ($cellx[$p-1]['SIZE_PLAN_PLC']=='40b'){
													$c="ui-plus-default";
										} else {
												$c="ui-state-default";
					...
					?> 
							<li valign="top" class="<?=$c?>" <?php if (($i%$m) == 0){$j++;	?>style="clear: both;"<?php 
							}?>><font size="1"><?=$d?></font>
...
<script>
	...
	function onstack()
	{
		...
		var url = "<?=HOME?>planning.yard_allocation_import.ajax/yard_dblink_selected";
		$.blockUI({ message: '<h1>Please Wait...</h1> <br> <img src="<?=HOME?>images/loadingF.gif" /> ' });		
		$.post( url, { xml_: xml_str,ID_VS: id_vs, STATUS_C: sts_c,ID_YARD:y_id,KATE:kate, TIER_HEAD:tier_},
		function(data) 
			  {
					//
					//alert(data);
		      		console.log(data);
					$.unblockUI({ 
						onUnblock: function(){ alert('Cell Allocated'); } 
					});
					$('#info').load("<?=HOME?>planning.yard_allocation_import.ajax/load_info?id=<?=$yard_id?>&idb=''&id_block="+id_block+"&kategori="+kate+"&type=second");
		    }
		);
		...
		function(data) 
			  {
					...
					alert(data);
		      		console.log(data);
					$.unblockUI({ 
						onUnblock: function(){ alert('Allocation saved'); } 
					});
	...
	function delete_alokasi()
	{
		...
		var url = "<?=HOME?>planning.yard_allocation_import.ajax/yard_dblink_delete";
		$.blockUI({ message: '<h1>Please Wait...</h1> ' });		
		$.post( url, { xml_: xml_str,ID_VS: id_vs, STATUS_C: sts_c, ID_BOOK: id_bk,ID_YARD:y_id, HZ:hz, KATE:kate, TIER_HEAD:tier_},
		function(data) 
			  {
					...
					//alert(data);
		      		console.log(data);
					$.unblockUI({ 
						onUnblock: function(){ alert('Allocation Deleted'); } 
					});
					$('#info').load("<?=HOME?>planning.yard_allocation_import.ajax/load_info?id=<?=$yard_id?>&idb=<?=$idb?>&id_block="+id_block+"&kategori=<?=$kate?>&type=first");   
					$('#detail_slot').load("<?=HOME?>planning.yard_allocation_import.ajax/detail_slot?id=<?=$yard_id?>&id_block="+blid); 		 
					$('#lp<?=$id_bg?>').load("<?=HOME?>planning.yard_allocation_import.ajax/load_lapangan?id=<?=$yard_id?>&id_bg=<?=$id_bg?>&id_block="+id_block+"&kategori=<?=$kate?>&no_ukk=<?=$noukk;?>");      
					//$('#detail_slot').load("<?=HOME?>planning.yard_allocation_import.ajax/detail_slot?id=<?=$yard_id?>&id_block="+id_block);   
