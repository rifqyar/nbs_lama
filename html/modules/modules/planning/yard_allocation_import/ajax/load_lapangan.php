...
<div style="margin-top:0px;border:1px solid black;width:970;height:500;overflow-y:scroll;">
<span id="select-result" style="display: none;"></span>
<span id="result"></span>
<div class="grid" style="float:left">
	 <br /><center>
                            <font size=4px><font color="red"><b>BLOK <?php echo $name?></b></font></font>
                          </center>  <br /><br /><br /><br />
						  <center>
	...
	<tr align="left" valign="top">
		<? for($y=0;$y<=$width+1;$y++){
			if ($y == 0){?>
				<td width="1" align="left" valign="center"> &nbsp </td>
			<?} else if ($y == $width+1) {?>
	...
	<tr align="center" valign="top">
			...
			<td colspan="<?=$width+1?>" rowspan="<?=$height?>" align="left" valign="top" style="padding-left: 0px; padding-right: 0px;">
				<ol id="selectable">
					<?php 
						$j = 1;
					    $z = 0;
					...
					<?
						...
						if ($id_blok == 100){
								...
								if($x==0 ||$x==1||$x==12 ||$x==13||$x==24 ||$x==25||$x==36||$x==37||$x==48||$x==49){
									$c="ui-silang-default";
							
								} else {
								
									if($cellx[$z]['SIZE_PLAN_ALLO']<>'')
									 {
											$slot_ = $cellx[$z]['SLOT_'];
											$rew   = $cellx[$z]['ROW_'];
											...
											if(($cellx[$z]['SIZE_PLAN_PLC']=='20') OR ($cellx[$z]['SIZE_PLAN_PLC']=='40d')){
											...
											} else if ($cellx[$z]['SIZE_PLAN_PLC']=='40b'){
														$c="ui-plus-default";
											} else {
												// echo "SELECT COUNT(NO_CONTAINER) JML FROM YD_PLACEMENT_YARD WHERE ID_BLOCKING_AREA = '$id_blok' AND SLOT_YARD = '$slot_' AND ROW_YARD = '$rew'";
													$c="ui-state-default";
							 ...
							 if($cellx[$i-1]['SIZE_PLAN_ALLO']<>'')
							 {
										...
										if(($cellx[$i-1]['SIZE_PLAN_PLC']=='20') OR ($cellx[$i-1]['SIZE_PLAN_PLC']=='40d')){
										//	echo "SELECT COUNT(NO_CONTAINER) JML FROM YD_PLACEMENT_YARD WHERE ID_BLOCKING_AREA = '$id_blok' AND SLOT_YARD = '$slot_' AND ROW_YARD = '$rew'";
											$querya        = "SELECT COUNT(NO_CONTAINER) JML FROM YD_PLACEMENT_YARD WHERE ID_BLOCKING_AREA = '$id_blok' AND SLOT_YARD = '$slot_' AND ROW_YARD = '$rew'";
													//print_r($querya);
											$resulta       = $db->query($querya);
		...
		<? for($z=2;$z<=$height;$z++){?>
				<tr height="10"><td align="right" valign="center" width="5"><?=$z;?></td></tr>
		<?}?>		
		...
		function(data) 
			  {
				...
				//	$('#detail_slot').load("<?=HOME?>planning.yard_allocation_import.ajax/detail_slot?id=<?=$yard_id?>&id_block="+blid); 		 
					$('#lp<?=$id_bg?>').load("<?=HOME?>planning.yard_allocation_import.ajax/load_lapangan?id=<?=$yard_id?>&id_bg=<?=$id_bg?>&id_block="+id_block+"&kategori=<?=$kate?>&no_ukk=<?=$no_ukk;?>");     
