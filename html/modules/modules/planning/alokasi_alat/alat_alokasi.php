...
function sync_mov(no_ukk,bay_area,posisi,id_alat,seq_alat,sz,act)
{
	...
	var sz_cont_    = sz;
	var keg_        = act;
	var url			= "<?=HOME?>planning.alokasi_alat.ajax/sync_mov";	
...
{
	myWindow=window.open('<?=HOME?>planning.alokasi_alat.ajax/priority_alat?id='+id_vs+'&bay_area='+id_bay_area+'&bay_no='+bay_number+'&posisi='+posisi+'&id_alat='+alat,'','width=900,height=400,scrollbars=yes,resizable=no');
	myWindow.focus();
}

function detail_cont(id_vs,id_bay_area,bay_number,posisi,alat)
{
	$('#table_detail').load("<?=HOME?>planning.alokasi_alat.ajax/detail_cont?vs="+id_vs+"&bay_area="+id_bay_area+"&alat="+alat+"&posisi="+posisi+" #cont_detail").dialog({modal:true, height:300,width:400, title:"Bay "+bay_number+" "+posisi+""});
}

function cetak_csl(no_ukk,alat)
{
	window.open('<?=HOME?>planning.alokasi_alat.print/csl_printout/?no_ukk='+no_ukk+'&alat='+alat,'_blank');
}
...
<div class="content">
	<h2><span class="graybrown">
        ...
        &nbsp;<font color="#DE7E21">Container Crane</font> Allocation<font color="#0066CC"> [ IMPORT ]</font>
        </span></h2>
	<br/>
	<hr width="870" color="#e1e0de"></hr><p><br/></p>
	<form enctype="multipart/form-data" action="<?=HOME;?>planning.alokasi_alat/alat_alokasi" method="post">
	<table>
        <tr>
			<td>VESSEL / VOYAGE</td>
			<td>&nbsp;&nbsp;:&nbsp;&nbsp;</td>
	...
	<tr>
         <?php
            ...
            foreach ($blok as $row){
				...
				$cell_address = "SELECT ID_ALAT, SEQ_ALAT, ACTIVITY FROM STW_BAY_CSL WHERE ID_BAY_AREA = '$bay_id' AND POSISI_STACK = 'ABOVE' AND TRIM(SZ_PLAN) IN ('40b','45b') AND ACTIVITY = 'I'";
                $result15     = $db->query($cell_address);
                $cell         = $result15->getAll();
				
				$query_jml = "SELECT count(*) AS JML FROM STW_BAY_CSL WHERE ID_BAY_AREA = '$bay_id' AND POSISI_STACK = 'ABOVE' AND TRIM(SZ_PLAN) IN ('40b','45b') AND ACTIVITY = 'I'";
    ...
    <tr>
         <?php
            ...
            foreach ($blok as $row){
				...
				$cell_address = "SELECT ID_ALAT, SEQ_ALAT, ACTIVITY FROM STW_BAY_CSL WHERE ID_BAY_AREA = '$id_bay_area' AND POSISI_STACK = 'ABOVE' AND TRIM(SZ_PLAN) = '20' AND ACTIVITY = 'I'";
                $result15     = $db->query($cell_address);
                $cell         = $result15->getAll();
				
				$query_jml = "SELECT count(*) AS JML FROM STW_BAY_CSL WHERE ID_BAY_AREA = '$id_bay_area' AND POSISI_STACK = 'ABOVE' AND TRIM(SZ_PLAN) = '20' AND ACTIVITY = 'I'";
                $jml15     = $db->query($query_jml);
                $jml_hsl   = $jml15->fetchRow();
				$jumlah_record = $jml_hsl['JML'];
	...
	<tr>
         <?php
            ...
            foreach ($blok as $row){
				...
				$cell_address = "SELECT ID_ALAT, SEQ_ALAT, ACTIVITY FROM STW_BAY_CSL WHERE ID_BAY_AREA = '$id_bay_area' AND POSISI_STACK = 'BELOW' AND TRIM(SZ_PLAN) = '20' AND ACTIVITY = 'I'";
                $result15     = $db->query($cell_address);
                $cell         = $result15->getAll();
				
				$query_jml = "SELECT count(*) AS JML FROM STW_BAY_CSL WHERE ID_BAY_AREA = '$id_bay_area' AND POSISI_STACK = 'BELOW' AND TRIM(SZ_PLAN) = '20' AND ACTIVITY = 'I'";
                $jml15     = $db->query($query_jml);
                $jml_hsl   = $jml15->fetchRow();
				$jumlah_record = $jml_hsl['JML'];
	...
	<tr>
         <?php
            ...
            foreach ($blok as $row){
				...
				$cell_address = "SELECT ID_ALAT, SEQ_ALAT, ACTIVITY FROM STW_BAY_CSL WHERE ID_BAY_AREA = '$bay_id' AND POSISI_STACK = 'BELOW' AND TRIM(SZ_PLAN) IN ('40b','45b') AND ACTIVITY = 'I'";
                $result15     = $db->query($cell_address);
                $cell         = $result15->getAll();
				
				$query_jml = "SELECT count(*) AS JML FROM STW_BAY_CSL WHERE ID_BAY_AREA = '$bay_id' AND POSISI_STACK = 'BELOW' AND TRIM(SZ_PLAN) IN ('40b','45b') AND ACTIVITY = 'I'";
                $jml15     = $db->query($query_jml);
                $jml_hsl   = $jml15->fetchRow();
				$jumlah_record = $jml_hsl['JML'];				
