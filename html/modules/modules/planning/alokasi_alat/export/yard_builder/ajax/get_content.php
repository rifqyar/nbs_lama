...
$query_count_content = "SELECT BLOCKING_AREA.ID AS ID_BLOCKING_AREA FROM BLOCKING_CELL INNER JOIN BLOCKING_AREA ON BLOCKING_CELL.ID_BLOCKING_AREA = BLOCKING_AREA.ID  WHERE BLOCKING_AREA.ID_YARD_AREA = '$yard_id' AND BLOCKING_CELL.INDEX_CELL = '$cell[0]'";
...
else
{
	...
	{	
		...
		$query_container 	= "SELECT * FROM TB_CONTAINER WHERE ID = '$cont_id[$i]'";
		$result_container 	= $db_>query($query_container);
		$row_container		= $result_container->fetchRow();
	...
	$html_block .= "</table> $block_name - Slot $index_slot - Row $index_row</center>";
