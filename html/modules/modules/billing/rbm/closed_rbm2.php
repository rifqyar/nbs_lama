<?php
		...
		$query_i 	= "SELECT NVL(MAX(COUNTER),0)+1 COUNT FROM LOG_RBM where NO_UKK='$id_vessel' AND KETERANGAN = 'RBM_IPC'";
		$result_i   = $db->query($query_i);
		$count_     = $result_i->fetchrow();
		$count 		= $count_['COUNT'];
		
		 $nama_file_ = 'RBM_IPC_'.$nm_kapal.'_'.$voy_in.$voy_out.'_'.$count.'.pdf';
		//$nama_file_ = 'dama_jict.pdf';
		 
		 $query_3	= "INSERT INTO LOG_RBM(TGL_UPDATE,STATUS_RBM, KETERANGAN,NAMA_FILE,NO_UKK,STATUS, COUNTER) VALUES (SYSDATE,'$kor','RBM_IPC','$nama_file_','$id_vessel','SAVE','$count')";
		 
		 //$query_4	= "INSERT INTO LOG_RBM(TGL_UPDATE,STATUS_RBM, KETERANGAN,NAMA_FILE,NO_UKK,STATUS) VALUES (SYSDATE,'$kor','RBM_JICT','$nama_file_','$id_vessel','SAVE')";
