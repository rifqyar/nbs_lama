...
$query="SELECT NO_NOTA  FROM nota_batalmuat_h WHERE TRIM(ID_BATALMUAT) = TRIM('$no_req') AND STATUS!='X'";
$hasil=$db->query($query);
$hasil_=$hasil->fetchRow();
$notanya=$hasil_['NO_NOTA'];

/*PTP*/
    $query = "select fc_nm_perusahaan('$notanya','BM') NM_PERUSAHAAN from dual";
// echo $query; die();
    $rq    = $db->query($query)->fetchRow();
    $nm_perusahaan = $rq['NM_PERUSAHAAN'];
/*PTP*/

$query="UPDATE NOTA_BATALMUAT_H SET TGL_CETAK_NOTA=SYSDATE WHERE NO_NOTA='$notanya'";
$db->query($query);


$query= "SELECT b.NO_NOTA, b.ID_BATALMUAT, b.EMKL, b.ALAMAT, b.NPWP, b.TGL_REQ, b.JENIS, b.VESSEL, b.VOYAGE, to_char(b.BAYAR,'999,999,999,999') AS TOTAL, to_char(b.ADM,'999,999,999,999') as ADM_NOTA, to_char(b.PPN,'999,999,999,999') AS PPN, to_char(b.TOTAL,'999,999,999,999') AS TAGIHAN,
		(SELECT NAME FROM TB_USER a WHERE b.PENGGUNA = a.ID) ID_USER
		 FROM NOTA_batalmuat_h b
		 WHERE TRIM(b.NO_NOTA) = TRIM('$notanya')";
...
for($pg=1; $pg<=$jum_page; $pg++) {
	...
	<table border='0'>                
                ...
                <tr>
                    <td align="left"><font size="8"><b>$data[NO_NOTA]</b></font></td>                    
                <td colspan="2"> <font size="8"> <b>$date</b></font></td>
                </tr>
                
                <tr>                    
                    <td><b>$data[ID_BATALMUAT]</b></td>
                </tr>                                   
