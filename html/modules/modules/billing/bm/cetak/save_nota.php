...
$rs=$id_vessel;
$db=getDb();
	$qh="select NO_UKK, NM_KAPAL, VOYAGE_IN, VOYAGE_OUT, 
		NM_PEMILIK, ALAMAT, NO_DO, NO_BL, NO_PEB, NO_NPWP,
		...
		to_char(PPN, '999,999,999.99') PPN FROM RBM_H WHERE NO_UKK='$rs'";
	$res=$db->query($qh);
	$rd=$res->fetchRow();
...
$nm_kpl=$rd['NM_KAPAL'];
$voyin=$rd['VOYAGE_IN'];
$voyout=$rd['VOYAGE_OUT'];
$nm_pmlk=$rd['NM_PEMILIK'];
$almt=$rd['ALAMAT'];
$no_npwp=$rd['NO_NPWP'];
$rta=$rd['TGL_JAM_TIBA'];
$rtd=$rd['TGL_JAM_BERANGKAT'];
$date_n=$rd['DATE_N'];
$tag=$rd['TAGIHAN'];
$ttl=$rd['TOTAL'];
$adm=$rd['ADM'];
...
$qd="select CASE WHEN EI='I' THEN 'DISCH' ELSE 'LOAD' END KETERANGAN, 
...
foreach ($rdt as $row)
{
	$tblx .='	<tr>
                    ...
                    <td widtd="32" align="center"><font size="8">CC</font></td>
                    <td widtd="32" align="center"><font size="8">'.$row['SIZE_'].'</font></td>
                    <td widtd="32" align="center"><font size="8">'.$row['TYPE_'].''.$row['HEIGHT'].'</font></td>
                    <td widtd="32" align="center"><font size="8">'.$row['STATUS'].'</font></td>
                    <td widtd="32" align="center"><font size="8">'.$row['HZ'].'</font></td>
                    <td widtd="32" align="center"><font size="8">'.$row['BOX'].'</font></td>
...
$query_i 	= "SELECT MAX(COUNTER) COUNT FROM LOG_RBM where NO_UKK='$no_ukk' AND KETERANGAN = 'NOTA'";
...
$date = date('dMYh:i:s');
ob_clean();
   $pdf->Output('./report/pdf_nota/NOTA_'.$nm_kpl.'_'.$voyin.'_'.$voyout.'_'.$date.'_'.$count.'.pdf', 'F');
	// $pdf->Output('http://192.168.212.31/email_rbm/dama_ict.pdf', 'F');
       exit();
