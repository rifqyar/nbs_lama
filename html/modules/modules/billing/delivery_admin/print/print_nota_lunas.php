...
$query="SELECT ID_NOTA, ID_REQ FROM nota_delivery_h WHERE TRIM(ID_REQ) = TRIM('$no_req') AND STATUS!='X'";
...
$query_dtl="SELECT TO_CHAR(a.TARIF, '999,999,999,999') AS TARIF, 
	...
	foreach($row2 as $rows) {
		                ...
		                <tr><td colspan="3" width="100"><font size="4">'.'<b>'.$rows[KETERANGAN].'</b>'.'</font></td>
                        
                            
                        <td width="10" align="left"><font size="4">'.'<b>'.$rows[JUMLAH_CONT].'</b>'.'</font></td>                              
                        <td width="50" align="left"><font size="4">'.'<b>'.$rows[SIZE_]." ".$rows[TYPE]." ".$rows[STATUS].'</b>'.'</font></td>                                
                            
                        <td width="7" align="left"><font size="4">'.'<b>'.$rows[HZ].'</b>'.'</font></td>                        
                        <td width="45" align="right"><font size="4">'.'<b>'.$rows[TARIF].'</b>'.'</font></td>                        
                        <td width="35" align="right"><font size="4">'.'<b>'.$rows[SUB_TOTAL].'</b>'.'</font></td>        
                        </tr>   
                        <tr>
                        <td colspan="8"><font size="4"><i><b>('.$rows[START_STACK].' s/d '.$rows[END_STACK].')'.$rows[JUMLAH_HARI].' hari</b></i></font></td>
                         </tr>
		';
                        }else {                            
                            $detail .= '<tr><td colspan="3" width="100"><font size="4">'.'<b>'.$rows[KETERANGAN].'</b>'.'</font></td>
...
$query_jum="SELECT COUNT(1) JUM_DETAIL FROM NOTA_DELIVERY_D A WHERE A.ID_NOTA='".$data['ID_NOTA']."'";
...
for($pg=1; $pg<=$jum_page; $pg++) {
	...
	$pdf->SetFont('helvetica', '', 6);	
        ...
        $tbl = <<<EOD
			...
			<table border='0'>                
		<tr>                    
                 <td COLSPAN="1" align="left"><font size="8"><b>$data[ID_NOTA]</b></font></td>
