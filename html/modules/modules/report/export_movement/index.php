...
$(function() { $( "#NM_KAPAL" ).autocomplete({
		minLength: 3,
...
function search_request()
{
	...
	if(keg=="E")
	{
		 ...
		 jQuery("#list_container").jqGrid({
			...
			mtype : "post",
			datatype: "json",
			colNames:['No Container','E/I', 'OPR','ISO','F/M','WGT', 'Seal No','POD',
                                  'Temp','OH|OL|OW', 'IMDG','UNNO','Status', 'Location', 'Type',
                                  'Gate In', 'Booking No', 'Customer Name', 'NPE', 'Yard Postion', 'BayPlan'], 
			colModel:[
				{name:'no_cont',index:'no_cont', width:80, align:"center"},
                                {name:'ei',index:'ei', width:40, align:"center"},
				{name:'opr',index:'opr', width:40, align:"center"},
				{name:'iso',index:'iso', width:50, align:"center"},				
				{name:'fm',index:'fm', width:50, align:"center"},				
				{name:'wgt',index:'wgt', width:50, align:"center"},				
				{name:'sealno',index:'sealno', width:110, align:"center"},				
				{name:'pod',index:'pod', width:50, align:"center"},
				{name:'temp',index:'temp', width:40, align:"center"},                                
                                {name:'oog',index:'oog', width:80, align:"center"},
                                {name:'imdg',index:'imdg', width:70, align:"center"},				
				{name:'unno',index:'unno', width:70, align:"center"},	                                
				...
				{name:'gate_in',index:'gate_in', width:110, align:"center"},
				{name:'bookingno',index:'bookingno', width:110, align:"center"},                     
                                {name:'emkl',index:'emmkl', width:220, align:"center"},                                
				{name:'npe',index:'npe', width:120, align:"center"}, 
                                {name:'yard_postion',index:'yard_position', width:120, align:"center"}, 
                                {name:'bayplan',index:'bayplan', width:120, align:"center"}
	...
	{	
		 ...
		 jQuery("#list_container").jqGrid({
			...
			colNames:['No Container','E/I', 'OPR','ISO','F/M','WGT', 'Seal No','POD',
                                  'Temp','OH|OL|OW', 'IMDG', 'UNNO','Status', 'Location', 'Type',
                                  'Gate In', 'Booking No', 'Customer Name', 'SPPB', 'Yard Postion','BayPlan'], 
			colModel:[
				{name:'no_cont',index:'no_cont', width:80, align:"center"},
                                {name:'ei',index:'ei', width:40, align:"center"},
				{name:'opr',index:'opr', width:40, align:"center"},
				...
				{name:'bookingno',index:'bookingno', width:110, align:"center"},                     
                                {name:'emkl',index:'emmkl', width:220, align:"center"},                                
				{name:'sppb',index:'sppb', width:120, align:"center"}, 
                                {name:'yard_position',index:'yard_position', width:120, align:"center"}, 
                                {name:'bayplan',index:'bayplan', width:120, align:"center"}
			],
			rowNum:10000,
			width: 880,
...
function export_excel()
{
    ...
    var keg = $( "#KEG" ).val();	
    window.open("<?=$HOME?>report.export_movement/detail_excel?vessel="+vessel+"&voyage_in="+voyage_in+"&voyage_out="+voyage_out+"&keg="+keg);
...
function export_pdf()
{
	...
	var no_ukk = $( "#NO_UKK" ).val();
	var keg = $( "#KEG" ).val();
	window.open("<?=$HOME?>report.export_movement/detail_pdf?vessel="+vessel+"&voyage_in="+voyage_in+"&voyage_out="+voyage_out+"&keg="+keg);
...
<div class="content">
	...
	<img src="<?=HOME?>images/kapal_.gif" height="7%" width="7%" style="vertical-align:middle">
		<b> <font color='#69b3e2' size='4px'>Container Movement</font> </b>
	</p>
	...
	<center>
		<table style="margin: 10px 10px 10px 10px;" border="0">
			...
			<tr>
				...
				<button onclick="export_pdf();">ExportToPDF</button>
