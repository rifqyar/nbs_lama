...
$query="select a.nota_name as nota_name, currency,
(select count(distinct(b.trx_number)) from uster.pyma_staging b where b.nota_name = a.nota_name and b.transfer_status = 'S' and b.trx_number IS NOT NULL and b.org_id in $ORG_ID and to_char(b.trx_date,'yyyy') = '$YEAR') as count_yes,
(select nvl(sum(nvl(c.amount,0)),0) from uster.pyma_staging c where c.nota_name = a.nota_name and c.transfer_status = 'S' and c.trx_number IS NOT NULL and c.org_id in $ORG_ID and to_char(c.trx_date,'yyyy') = '$YEAR') as amount_yes,
(select count(distinct(d.trx_number)) from uster.pyma_staging d where d.nota_name = a.nota_name and d.transfer_status = 'F' and d.trx_number IS NOT NULL and d.org_id in $ORG_ID and to_char(d.trx_date,'yyyy') = '$YEAR') as count_no,
(select nvl(sum(nvl(f.amount,0)),0) from uster.pyma_staging f where f.nota_name = a.nota_name and f.transfer_status = 'F' and f.trx_number IS NOT NULL and f.org_id in $ORG_ID and to_char(f.trx_date,'yyyy') = '$YEAR') as amount_no
from uster.pyma_staging a where a.trx_number IS NOT NULL and to_char(a.trx_date,'yyyy') = '$YEAR' AND ORG_ID in $ORG_ID group by a.nota_name, a.currency order by a.nota_name asc";
if($res = $db->query($query)) {
	$i=0;
	while ($row = $res->fetchRow()) {
		//$id=$row[TRX_NUMBER];
