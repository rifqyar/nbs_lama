...
<?php
	...
	$qr="SELECT a.EMKL,a.ALAMAT,a.NPWP,a.TGL_REQUEST,a.NO_SPPB,to_char(a.TGL_SPPB,'dd-mm-yyyy') TGL_SPPB,a.NO_DO,to_char(a.TGL_SP2,'dd-mm-yyyy') TGL_SP2,a.SHIP,a.DEV_VIA,a.KETERANGAN,CALL_SIGN,TO_CHAR(a.TGL_DO,'dd-mm-yyyy') TGL_DO,
		...
		A.COA as IDEMKL, TO_CHAR(a.disch_date,'dd-mm-yyyy') DISCH_DATE, a.no_ukk, regexp_replace(VESSEL, '[[:space:]]*','') xvessel, a.VESSEL, a.voyage_in, a.voyage_out, a.CUST_DSTN,a.ICUST_DSTN, a.TIPE_CONT_REQ FROM REQ_DELIVERY_H a WHERE a.ID_REQ = TRIM('$req')";
	//print_r($qr);die;
	$db=getDb();
	$er=$db->query($qr);
	$hr=$er->fetchRow();
...
{	
	//======================================= autocomplete EMKL==========================================//
	$("#tipe_req_cont").val("<?=$hr['TIPE_CONT_REQ'];?>");
    $( "#emkl" ).autocomplete({
		minLength: 3,
	...
	$('#detail_req').load("<?=HOME?>request.delivery.sp2.ajax/detail_req?id=<?=$req?>&ship=<?=$ship?>&call_sign=<?=$hr['CALL_SIGN'];?>&vin=<?=$hr['VOYAGE_IN'];?>&tipe_req_cont=<?=$hr['TIPE_CONT_REQ'];?>");
...
<div class="content">
    ...
    <tr>
		...
		<td>
                    <select id="tipe_req_cont" disabled>
