...
function create_req()
{
	...
	var tipe_req_cont=$("#tipe_req_cont").val();
	alert(tipe_req_cont);
	...
	if( tipe_req_cont=='')
	{
		alert('Masukan Tipe Request Container..');
		return false;
	}
	else if( iemkl=='')
	{
	...
	{
		//alert('e');
		$.post(url,{TIPE_REQ_CONT:tipe_req_cont, CALL_SIGN:call_sign,IDEMKL:iemkl,SPPB:nsppb, TGLSPPB:tglsppb, TGLSPCUST:tgl_spcust, NDO:ndo, TGLDO:tgldo, TGLDEL:tgldel, SHIP:ship, VIA:via, KET:ket, ID_VES_SCD:id_ves_scd, VIN:vin, VOUT:vout, VES:ves, DDSC: date_disch, SPCUST:spcust, BLNUMB:blnumb, CARGOW:cargow, ICARGOW:icargow, JENIS_SPPB:jenis_sppb},
		function(data){	
			alert(data);
			var row_data = data;
			...
			else
			{
				...
				$('#detail_req').load("<?=HOME?>request.delivery.sp2.ajax/detail_req?id="+v_req+"&tipe_req_cont="+tipe_req_cont+"&call_sign="+call_sign+"&vin="+vin,{VES:ves} );
...
function add_cont1(i,j)
{
	...
	var pli=$('#pli').val();
	var plo=$('#plo').val();
	var dischdate=$('#dischdate').val();
	var contdeldate=$('#contdeldate').val();
	alert(contdeldate);
	...
	else if((tc=='RFR')&&(plo=='')&&(stc!='EMPTY'))
	{
        ...
        alert('Entry delivery date please...');
		return false;
    }
	else 
	{
		$.post(url,{VIN :vin,VESX :vesx, NC: nc,SC:sc, TC:tc, STC:stc, HC:hc, COMM:comm, IMO:imo, ISO:iso, REQ:i, HGC:hgc, SHIP:j, CAR:car, TEMP:tmp,UKK:ukk, BOOKSL: booksl, OW:ow, OH:oh, OL:ol, UNNUMBER:unnumber, POD:pod, POL:pol, PLI:pli, PLO:plo, dischdate: dischdate,contdeldate:contdeldate},function(data){	
...
<div class="content">
	...
	<table>
         <tr>
		<td class="form-field-caption" align="right">Request Full / Empty</td>
		<td class="form-field-caption" align="right">:</td>
		<td>
                    <select id="tipe_req_cont">
                        <option>Pilih Tipe Container</option>
	...
	<tr>
		<td class="form-field-caption" align="right"><div class='OceanGoing'>No S.P.P.B</td>
		<td class="form-field-caption" align="right"><div class='OceanGoing'>:</td>
		<td><input type="text" size="20" maxlength="20" name="sppb" id="sppb" value="" class='OceanGoing' style="background-color:#FFFFFF;"/>
	 ...
	 <tr class='OceanGoing'>
		...
		<td class="form-field-caption" align="right"><div class='OceanGoing'>Date SPPB</td>
		<td class="form-field-caption" align="right"><div class='OceanGoing'>:</td>
		<td><input type="text" size="20" id="tgl_sp2b" name="tgl_sp2b" value="" class='OceanGoing'/>
		</td>	
		<td>&nbsp;</td>
