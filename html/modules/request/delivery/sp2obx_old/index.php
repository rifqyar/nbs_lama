...
function add_reqobx()
{
	$('#add_reqobx').load("<?=HOME?>request.delivery.sp2obx.ajax/obx_insert").dialog({modal:true, height:600,width:560,title: "Insert Request SP2 OBX"});
}
...
{
	$('#approve_plp').load("<?=HOME?>request.delivery.sp2obx.ajax/perp_obx?id_del="+iddel).dialog({modal:true, height:480,width:495,title: "Perpanjangan SP2 OBX"});
}

function perp_entry(iddel)
{
	var tglexp = $("#tgl_perp").val();
	var url = "<?=HOME?>request.delivery.sp2obx.ajax/update_obx";
