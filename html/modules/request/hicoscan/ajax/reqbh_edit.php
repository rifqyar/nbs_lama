...
$(document).ready(function(){
	...
	$("#vessel").autocomplete({
		minLength: 3,
		source: "request.hicoscan.auto/vesvoy",
	...
	$("#emkl").autocomplete({
		minLength: 3,
		source: "request.hicoscan.auto/pelanggan",
		focus: function( event, ui ) 
		{
		...
		select: function( event, ui ) 
		{
			...
			$("#iemkl").val(ui.item.KD_PELANGGAN);			
			return false;
		}
	})
	.data( "autocomplete" )._renderItem = function( ul, item ) 
...
{
	if($("#tipe_req").val()=="") {
		alert("pilih jenis request hicoscan terlebih dahulu!!!");
	}
...
function autocompletenya(index) {
	...
	$( "#no_cont"+index ).autocomplete({
		minLength: 4,
		source: "request.hicoscan.auto/container1?vess="+glob_vessel+"&vin="+glob_vin+"&tipe="+tipe_req,
		focus: function( event, ui ) 
		{
...
$id = $_GET['id'];
$db = getDB();
$query="SELECT NOMOR_INSTRUKSI, ID_USER, VOYAGE, VOYAGE_OUT, to_char(PAID_THRU,'DD-MM-YYYY') PAID_THRU,VESSEL, EMKL,KD_PELANGGAN, ALAMAT_EMKL, NPWP, TIPE_REQ, STATUS, SHIPPING_LINE, KET, NO_UKK FROM REQ_HICOSCAN WHERE ID_REQUEST='".$id."'";
// echo $query;die;
...
?>	
	...
	<table>
		...
		<tr>
			...
			<td class="form-field-caption" align="right"> : </td>
			<td class="form-field-caption" align="left">
				<input type="text" name="vessel" id="vessel" size="30" placeholder="Autocomplete" value="<?=$row['VESSEL']?>" /> / <input type="text" name="voyage" id="voyage" size="5" value="<?=$row['VOYAGE']?>" ReadOnly /><input type="text" name="voyage_out" id="voyage_out" size="5" value="<?=$row['VOYAGE_OUT']?>" ReadOnly />
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td class="form-field-caption" align="right"> : </td>
			<td class="form-field-caption" align="left">
				<input type="text" name="paid_thru" id="paid_thru" size="15" value="<?=$row['PAID_THRU']?>" ReadOnly />
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td class="form-field-caption" align="left">
				...
				<input type="hidden" name="iemkl" id="iemkl" size="30" placeholder="Autocomplete" value="<?=$row['KD_PELANGGAN']?>" />
			</td>
		</tr>
		<tr>
		...
		<tr>
			<td class="form-field-caption" align="right">No. DO</td>
			<td class="form-field-caption" align="right"> : </td>
			<td class="form-field-caption" align="left">
				<input type="text" name="no_do" id="no_do" size="30" />
			</td>
		</tr>
		<tr>
...
<?php
$sql2 = "SELECT * FROM REQ_HICOSCAN_D WHERE ID_REQUEST='".$id."' ORDER BY ID";
$rs2 = $db->query( $sql2 );
// echo $sql2;
$j=0;
...
?>
						<tr height="20"> 
							...
							<td valign="top">
								<input type="text" id="hz<?=$j?>" name="hz<?=$j?>" value="<?=$row2["HAZZARD"]?>" size="2" readonly />
							</td>
						</tr>
<?php
}
