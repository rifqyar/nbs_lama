...
<?php
$db = getDB();
$query="SELECT NO_CONTAINER, SIZE_CONT AS SIZE_, TYPE_CONT AS TYPE_, STATUS, HZ, HEIGHT, E_I, '' AS KD_BARANG FROM M_CYC_CONTAINER@dbint_link WHERE E_I='I' AND UPPER(CONT_LOCATION)='YARD' AND ACTIVE='Y' AND VESSEL='$vessel' and VOYAGE_IN='$vin' AND NO_CONTAINER NOT IN 
(SELECT NO_CONTAINER FROM REQ_TRANSHIPMENT_D WHERE ID_REQ='$req') AND NO_CONTAINER NOT IN (SELECT NO_CONTAINER FROM REQ_DELIVERY_D WHERE NO_UKK='$oukk')";
$query2 = $query;
...
<?php
$query="SELECT NO_CONTAINER, SIZE_CONT, TYPE_CONT, STATUS_CONT, HZ, HEIGHT, E_I, ID_CONT FROM REQ_TRANSHIPMENT_D WHERE ID_REQ='$req' ORDER BY URUT";
if($res = $db->query($query))
while ($row = $res->fetchRow()) {
	$val_cont = $row[NO_CONTAINER]."^^".$row[SIZE_CONT]."^^".$row[TYPE_CONT]."^^".$row[STATUS_CONT]."^^".$row[HZ]."^^".$row[HEIGHT]."^^".trim($row[E_I])."^^".trim($row[ID_CONT]);
?>
				<option value="<?=$val_cont?>"><?=$row[NO_CONTAINER]?> ~ <?=$row[SIZE_CONT]?>-<?=$row[TYPE_CONT]?>-<?=$row[STATUS_CONT]?></option>
<?php
}
