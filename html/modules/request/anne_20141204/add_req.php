...
function create_req()
{
	...
	{
		//alert('e');
		$.post(url,{UKK: ukk,IDEMKL:iemkl, IPOD:ipod, IPOL:ipol, SHIP:ship, NPE:npe, PEB:peb, DS:book_ship_h, CAR:icar, START_SHIFT: start_shift, END_SHIFT:end_shift, JML_SHIFT:jml_shift, FIPOD:fipod},function(data){	
			//alert(data);
			var row_data = data;
...
{	
	...
	/*$( "#carrier" ).autocomplete({
		minLength: 3,
		source: "request.anne.auto/carrier",
		focus: function( event, ui ) 
		{
	...
	}).data( "autocomplete" )._renderItem = function( ul, item ) 
	{
		return $( "<li></li>" )
		.data( "item.autocomplete", item )
		.append( "<a>" + item.LINE_OPERATOR + " <br> " + item.CODE +"</a>")
		.appendTo( ul );
...
<div class="content">
	...
	<tr>
	 ...
	 <tr>
		...
		<td class="form-field-caption" align="right">:</td>
		<td><input type="text" size="25" name="carrier" id="carrier"  /> <input type="text" size="8" id="icar" readonly="readonly"/>
			<input type="hidden" size="8" id="negt" />
		</td>-->
	 ...
	 <tr>
		...
		<td class="form-field-caption" align="right">:</td>
		<td><input type="text" size="5" id="ship" name="ship" readonly="readonly" style="background-color:#FFFFCC; font-weight:bold;text-align:center"/><br>
