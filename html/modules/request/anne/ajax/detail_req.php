...
document.getElementById('notfound').innerHTML = '';
	$( "#nc" ).autocomplete({
		...
		focus: function( event, ui )
		{
			  if (ui.item.VALIDATE == 1) {
				  $( "#nc" ).val( ui.item.NO_CONTAINER);
				  $( "#vl" ).val( ui.item.VALIDATE);
				document.getElementById('notfound').innerHTML = "<p align='right' style='font-weight:bold;font-size:200%;' style='color:red;'>Container Belum Terdaftar</p>";
			  }
			  else {
			$( "#nc" ).val( ui.item.NO_CONTAINER);
			$( "#vl" ).val(0);
			document.getElementById('notfound').innerHTML = "<p align='right' style='font-weight:bold;font-size:200%;' style='color:red;'>Container Terdaftar</p>";
			return false;
			  } 
		},
		select: function( event, ui )
		{
			...
			$( "#sc" ).val( ui.item.ISO_SIZE);
			$( "#hgc" ).val( ui.item.ISO_HEIGHT);
			$( "#tc" ).val( ui.item.ISO_TYPE);
	...
	}).data( "autocomplete" )._renderItem = function( ul, item )
	{
		...
		return $( "<li></li>" )
		.data( "item.autocomplete", item )
		.append( "<a align='left'>" + item.NO_CONTAINER +"|"+item.ISO_CODE+"|"+item.ISO_HEIGHT+"</a>")
		.appendTo( ul );
...
<tr>
		...
		<td align="left" colspan="7"><div id='notfound'></div><input type="hidden" id="vl" name="vl" /></td>
</tr>
<tr>
		...
		<td class="form-field-caption" align="right">:</td>
		<td>
			<select id="hgc" onchange="cek_iso('true')">
