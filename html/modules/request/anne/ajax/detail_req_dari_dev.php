<?php
	...
	$ship=$_GET['ship'];
	$car=$_GET['car'];
	$book_ship_h=$_GET['book_ship_h']; $ukk =$_GET['ukk'];
?>
...
function upload(form_name) {
	...
	uploadfile = document.getElementById('uploadField').value;
	
	if(uploadfile==""){
		alert("File tidak dipilih!");
	} else if (uploadfile!="" && !(uploadfile.substr(-3)=="xls")) {
		alert("File Upload Container harus berupa file excell (xls,xlsx)!!!");
		//return false;
	} else {
		question = confirm("Data upload akan disimpan, apakah sudah benar?")
		if(question !=0 ){
			alert("Ivan Ganteng");
		}
	}
