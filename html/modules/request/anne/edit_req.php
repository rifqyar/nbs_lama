...
<?php
	$req=$_GET['no_req'];	
	$db=getDb();
	$qrcek="select count(1) JML from nota_receiving_h where trim(id_req)='$req' and status in ('P','T')";
	$excek=$db->query($qrcek);
	$hscek=$excek->fetchRow();
	$jmcek=$hscek['JML'];
	//print_r($qrcek);die;
	if($jmcek>0)
	{
		print_r("REQUEST ALREADY PAID: YOU CAN'T DO FRAUD ANYMORE");die;
	}
?>
...
<?php
	...
	$query = "UPDATE REQ_RECEIVING_H SET IS_EDIT=1 WHERE ID_REQ = :v_noreq";
	$db->query($query,$param_b_var);
	
	$qr="select OI,KODE_PBM, VESSEL,NO_UKK, VOYAGE_IN,VOYAGE_OUT, PEB, NPE, TGL_REQUEST, 
	to_char(TGL_STACK,'dd-mm-yyyy') TGL_STACK, TGL_OPEN_STACK, TO_CHAR(TGL_MUAT,'dd-mm-yyyy hh24:mi:ss') TGL_MUAT, TGL_BONGKAR, 
	STATUS,SHIFT_REEFER, TO_CHAR(START_SHIFT,'dd-mm-yyyy hh24:mi:ss') START_SHIFT, END_SHIFT, 
...
function update_req()
{
	...
	var peb=$( "#peb" ).val();
	var icar=$( "#icar" ).val();
	var stshift=$( "#start_shift" ).val();
	var enshift=$( "#end_shift" ).val();
	var jmlshift=$( "#jml_shift" ).val();
	...
	{
		var url_save_flag="<?=HOME;?>request.anne.ajax/save_flag_req";
		$.post(url_save_flag,{noreq:id_req},function(data){	
			//alert(data);
		});
	
		//alert('e');
		$.post(url,{ID_REQ:id_req,UKK: ukk,IDEMKL:iemkl, IPOD:ipod, IPOL:ipol, SHIP:ship, NPE:npe, PEB:peb, CAR:icar, START_SHIFT:stshift, END_SHIFT:enshift, JMLSHIFT:jmlshift},function(data){	
			//alert(data);
			var row_data = data;
...
function add_cont1(i,j,k, book_s,ukk,vessel,voyin,voyout)
{
	...
	var url="<?=HOME;?>request.anne.ajax/add_detail_req?ukk="+ukk+"&vessel="+encodeURIComponent(vessel)+"&voyin="+voyin+"&voyout="+voyout;
	var valid=$('#rsct').val();
	if((valid=='0') || (valid==''))
	{
	...
	if (vl==1)
	{
		...
		alert (nc);
		var iso=$('#iso').val();
		var url_save_cont="<?=HOME;?>request.anne.ajax/add_new_cont";
		$.post(url_save_cont,{NC: nc,ISO:iso},function(data){	
			alert(data);
		});
	}
		$.post(url,{NC: nc,SC:sc, TC:tc, STC:stc, HC:hc, COMM:comm, IMO:imo, ISO:iso, BOOK:book_s, REQ:i, HGC:hgc, SHIP:j, CAR:car_d, TEMP:tmp, NOR:nor,WEIGHT_NPE:weight_npe},function(data){	
			alert(data);
...
{	        
	...
	$('#detail_req').load("<?=HOME?>request.anne.ajax/detail_req?id="+v_req+"&ship="+ship+"&car=null&book_ship_h="+book_ship_h+"&ukk="+ukk+"&vessel="+encodeURIComponent(ves)+"&voyin="+voy+"&voyout="+voyo);
	...
	$('#book_ship_h').keypress(function (e) {
...
<div class="content">
	...
	<tr>
		...
		<td class="form-field-caption" align="right">:</td>
		<td><input type="text" size="10" name="voy" id="voy" readonly="readonly" value="<?=$hr['VOYAGE_IN'];?>" readonly="readonly"  /><input size="10" name="voyopus" id="voyopus" type="hidden"/>
                    <input type="text" size="10" name="voyo" id="voyo" readonly="readonly" value="<?=$hr['VOYAGE_OUT'];?>" readonly="readonly"  />
		</td>
	 ...
	 <tr>
		...
		<td class="form-field-caption" align="right">:</td>
                <td><input type="text" size="20" onfocus="onfoucusshiftref()" id="start_shift" name="start_shift" value="<?=$hr['START_SHIFT']?>" style="background-color:#FFFFFF;"/> s/d <input type="text" size="20" id="end_shift" name="end_shift" value="<?=$hr['END_SHIFT']?>" style="background-color:#FFFFFF;"/><img src="<?=HOME?>images/calculator.png" width="20" onclick="calculator()" /></td>
	</tr>
	 ...
	 <tr>
		...
		<td class="form-field-caption" align="right">:</td>
		<td><input type="text" size="10" name="jml_shift" id="jml_shift" value="<?=$hr['SHIFT_REEFER']?>" style="background-color:#FFFFCC;" readonly="readonly"/>*Shift
		</td>
     </tr>
	 <tr>
