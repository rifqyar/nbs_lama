...
<?php
	$db=getDb();
	$req=$_GET['no_req'];
	$qr="SELECT SHIPPING_LINE, ALAMAT, NPWP, VESSEL_OLD, VOYAGE_IN_OLD, E_I, TGL_REQUEST, NO_UKK_OLD, USER_REQ, STATUS, NO_BC_12, NO_SURAT_TRS, VESSEL_NEW, VOYAGE_IN_NEW, NO_UKK_NEW, KD_PELANGGAN, OI, DECODE(OI,'O','Ocean Going','I','Intersuler') AS KET_OI, KETERANGAN from REQ_REEXPORT_H A WHERE ID_REQ='$req'";
	//print_r($qr);die;
	$er=$db->query($qr);
	$hr=$er->fetchRow();
?>
...
{	
	//open at first
	var v_req="<?=$req?>";
	var oukk="<?=$hr['NO_UKK_OLD']?>";
	var nukk="<?=$hr['NO_UKK_NEW']?>";
	var oi="<?=$hr['OI']?>";
	$('#detail_req').load("<?=HOME?>request.reexport.ajax/detail_req?id="+v_req+"&oukk="+oukk+"&nukk="+nukk+"&oi="+oi);
...
{	
	var url="<?=HOME;?>request.reexport.ajax/update_req";
	var url2="<?=HOME;?>request.reexport.ajax/add_detail_req";
	var emkl=$( "#emkl" ).val();
...
// function del(a,b)
// {
	// var url="<?=HOME;?>request.reexport.ajax/del_cont";
	// $.post(url,{NO_CONT:b, NO_REQ:a},function(data){	
		// alert(data);
		// $('#detail_container').load("<?=HOME?>request.reexport.ajax/list_container?id="+a);
	// });
// }
</script>
...
<div class="content">
	...
	<tr>
	 ...
	 <tr>
		...
		<td class="form-field-caption" align="right">:</td>
		<td><input type="text" size="25" name="ovessel" id="ovessel" value="<?=$hr['VESSEL_OLD'];?>" readonly="readonly" style="background-color:#FFFFCC;" />
		<input type="text" size="10" name="ovoy" id="ovoy" value="<?=$hr['VOYAGE_IN_OLD'];?>" readonly="readonly"/> <input type="hidden" name="oukk" id="oukk" value="<?=$hr['NO_UKK_OLD'];?>" /> 
		</td>
	</tr>
	 <tr>
		<td class="form-field-caption" align="right">New Vessel</td>
		<td class="form-field-caption" align="right">:</td>
		<td><input type="text" size="25" name="nvessel" id="nvessel" value="<?=$hr['VESSEL_NEW'];?>" readonly="readonly" style="background-color:#FFFFCC;" />
		<input type="text" size="10" name="nvoy" id="nvoy" value="<?=$hr['VOYAGE_IN_NEW'];?>" readonly="readonly"/> <input type="hidden" name="nukk" id="nukk" value="<?=$hr['NO_UKK_NEW'];?>" /> 
		</td>
	</tr>
	<tr>
