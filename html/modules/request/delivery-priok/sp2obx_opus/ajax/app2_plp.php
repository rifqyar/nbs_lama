...
$list_param = $user."^".$idplp."^".$custno."^".$custnm."^".$custadr."^".$custtax."^".$tgldel."^".$no_ukk;
...
$get_reqdel = "SELECT TRIM(NO_REQUEST) NO_REQ FROM BIL_DELOB_H WHERE ID_DEL = '$delno'";
$delob = $db->query($get_reqdel)->fetchRow();
$iddel = $delob['NO_REQ'];
...
for($i=0; $i<$jmlarr; $i++) 
{
	...
	$list_params = array("v_nc"=>"$list_no",
						 ...
						 "v_ukk"=>"$no_ukk",
						 "v_msg"=>"");
	// print_r($list_params);die;
	$ict_appd = "begin proc_add_cont_obx_opus(:v_nc,:v_req,:v_ukk,:v_msg); end;";
