		...
		<table>
			<tr>
				...
				Filter: <input type="text" id="box1Filter" /><button type="button" id="box1Clear">X</button><br/><br/>
						...
						<?php								
								$query = "SELECT ID_BARANG, LINE_NUMBER FROM OBX_APPROVAL_D WHERE TRIM(ID_PLP) = TRIM('$id_plp') AND FLAG_SP2OBX = '0' ORDER BY LINE_NUMBER ASC";
								if($res = $db->query($query))
									while ($row = $res->fetchRow()) {
						?>
									<option value="<?=$row['LINE_NUMBER']?>"><?=$row['ID_BARANG']?></option>
						<?php
									}
						?>
						</select><br />
