...
class XmlParser {
			...
			switch ($this->ch) {		
				...
				case '%':
					if ($this->ptr-$startptr==1) {
						return $this->parseSSS($startline,$startcol,$this->ch);
					} else {
						return false;
					}
					break;
	...
	function parseSSS($startline,$startcol,$stopper) {
		$buffer = '';
		while ($this->ptr < $this->len) {
			$this->last = $this->ch;
			$this->ch   = $this->data[ $this->ptr++ ];
			...
			switch ($this->ch) {		
				...
				default:
					...
					if (strpos(strrev($buffer),'>'.$stopper)===0) {
						$this->tags[] 	= array('SCRIPT','<'.$stopper.$buffer);
						return true;
					} 
			}			
		}		
